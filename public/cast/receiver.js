const BUILD = "CAF-VISUAL-X01-2026-03-19-4";
...
function avatarHtml(player, size = 116, smallText = false) {
  ...
  return `
    <div class="avatar-frame avatar-fallback" style="width:${size}px;height:${size}px;font-size:${Math.round(size * (smallText ? 0.28 : 0.32))}px;">
...
function graphHtml(players) {
  ...
  const lines = series.map((s, idx) => {
    ...
    return `
      <path d="${d}" fill="none" stroke="${color}" stroke-width="9" stroke-linecap="round" stroke-linejoin="round" opacity="0.15" filter="url(#glow-${idx % colors.length})" />
...
function renderSnapshot(payload) {
  ...
  contentEl.innerHTML = `
    <div class="receiver-shell">
      <div class="cast-game-title">${esc(String(gameTitle).toUpperCase())}</div>
