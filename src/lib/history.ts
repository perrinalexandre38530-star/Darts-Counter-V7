...
function toHeaderRecord(rec: any) {
  ...
  // CRICKET HEADER SCORE FIX:
  // History cards are rendered from the light header. Keep a trustworthy scoreLine
  // and finalScores there even when the heavy payload lives in history_details.
  try {
    const mode = String(
      out?.kind ||
      out?.game?.mode ||
      out?.summary?.mode ||
      out?.payload?.mode ||
      out?.payload?.game?.mode ||
      ""
    ).toLowerCase();
    if (mode.includes("cricket")) {
      const payloadPlayers = Array.isArray(out?.payload?.players) ? out.payload.players : [];
      const headerPlayers = Array.isArray(out?.players) ? out.players : [];
      const players = payloadPlayers.length ? payloadPlayers : headerPlayers;
      const derivedScores: Record<string, number> = {};
      const line: string[] = [];
      for (const pl of players) {
        const pid = String(pl?.id ?? pl?.playerId ?? pl?.profileId ?? "").trim();
        const rawScore =
          pl?.legStats?.totalPoints ??
          pl?.cricketStats?.totalPoints ??
          pl?.score ??
          out?.summary?.finalScores?.[pid];
        const score = Number(rawScore);
        if (pid && Number.isFinite(score)) derivedScores[pid] = score;
        const name = String(pl?.name ?? pl?.displayName ?? "").trim();
        if (name && Number.isFinite(score)) line.push(`${name} ${score}`);
      }
      const existingFinal =
        out?.summary?.finalScores && typeof out.summary.finalScores === "object"
          ? out.summary.finalScores
          : {};
      const existingLine = String(out?.summary?.scoreLine || "").trim();
      out.summary = {
        ...(out.summary || {}),
        finalScores: { ...derivedScores, ...existingFinal },
        ...(existingLine || line.length ? { scoreLine: existingLine || line.join(" • ") } : {}),
      };
    }
  } catch {}
...
export async function get(id: string): Promise<SavedMatch | null> {
  ...
  try {
    ...
    // History.get() is intentionally READ-ONLY.
    // A previous implementation tried to compare every finished match against the
    // local revision vault on each read, then called upsert() from inside get().
    // StatsHub can hydrate dozens of matches at once, so a simple page read could
    // trigger dozens of IndexedDB scans + writes + dc-history-updated events and
    // enter a repair/reload loop. Integrity repair now happens only on write/import
    // paths (upsert / snapshot merge), never while rendering a page.
...
export async function upsert(rec: SavedMatch): Promise<void> {
  ...
  try {
    ...
    payloadEffective = stripAvatarDataFromPayload(enriched.payload);
    if (enriched.record?.summary) safe.summary = enriched.record.summary;
    if (enriched.record?.telemetryVersion) (safe as any).telemetryVersion = enriched.record.telemetryVersion;
  } catch (e) {
    console.warn("[history.upsert] canonical darts telemetry enrichment failed:", e);
  }
  ...
  try {
    ...
    const telemetryLooksRich = (payload: any) => !!(
      payload && typeof payload === "object" && (
        ...
        (Array.isArray(payload?.dartTelemetry?.visits) && payload.dartTelemetry.visits.length > 0) ||
        (Array.isArray(payload?.visitHistory) && payload.visitHistory.length > 0) ||
        (Array.isArray(payload?.visits) && payload.visits.length > 0) ||
        (Array.isArray(payload?.dartLog) && payload.dartLog.length > 0) ||
        (Array.isArray(payload?.cricketDartLog) && payload.cricketDartLog.length > 0) ||
        (Array.isArray(payload?.events) && payload.events.some((event: any) =>
          event?.dart != null || event?.throw != null || Array.isArray(event?.darts)
        )) ||
        (Array.isArray(payload?.match?.visitLog) && payload.match.visitLog.length > 0) ||
        (Array.isArray(payload?.stats?.global?.visitLog) && payload.stats.global.visitLog.length > 0)
      )
    );
    const needsGenericTelemetryMerge =
      !!payloadEffective &&
      !!prevPayloadObj &&
    ...
    try {
      if (needsMerge && payloadEffective && typeof payloadEffective === "object") {
        ...
        payloadEffective = stripAvatarDataFromPayload(enrichedMerged.payload);
        if (enrichedMerged.record?.summary) safe.summary = enrichedMerged.record.summary;
        if (enrichedMerged.record?.telemetryVersion) (safe as any).telemetryVersion = enrichedMerged.record.telemetryVersion;
      }
    } catch (e) {
      console.warn("[history.upsert] telemetry re-enrichment after merge failed:", e);
    }
    ...
    await withStores([STORE_HEADERS, STORE_DETAILS], "readwrite", async (stores) => {
      ...
      const headerReq = headers.put({
        ...toHeaderRecord({ ...safe, payload: payloadEffective }),
