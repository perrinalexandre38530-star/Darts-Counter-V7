...
export function scorePenduVisit(ds:GameDart[]){return(ds||[]).slice(0,3).reduce((s,d)=>s+scoreDart(d),0);}
...
export function isPenduChallengeSatisfied(c:PenduChallenge,darts:GameDart[],config:PenduConfigPayload){if(c.kind==="score"){const sc=scorePenduVisit(darts);return config.rules.executionMode==="flex"?sc>=c.target:sc===c.target;}return(darts||[]).some(d=>{if(!d||Number(d.number||0)!==c.number)return false;if(config.rules.executionMode==="strict")return d.bed===c.bed;const rank:any={S:1,D:2,T:3};return Number(rank[d.bed]||0)>=Number(rank[c.bed]||0);});}
...
export function playPenduVisit(input:PenduState,dartsRaw:GameDart[]):PenduState{
 const state=clonePenduState(input);if(state.phase==="finished")return state;const player=state.players[state.activePlayerIndex];if(!player)return state;const darts=(dartsRaw||[]).slice(0,3);const success=isPenduChallengeSatisfied(state.challenge,darts,state.config);const stats=state.statsByPlayer[player.id]||(state.statsByPlayer[player.id]=blankStats());stats.visits+=1;stats.darts+=darts.length;stats.bestVisit=Math.max(stats.bestVisit,scorePenduVisit(darts));const events:string[]=[];
 ...
 state.visits.push({id:id("pendu-visit"),playerId:player.id,leg:state.legIndex+1,turn:state.turnIndex+1,phase:state.phase,challenge:clone(state.challenge),darts,score:scorePenduVisit(darts),success,events});state.turnIndex+=1;
