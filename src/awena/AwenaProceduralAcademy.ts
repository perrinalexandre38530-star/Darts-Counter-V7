...
const PROCEDURES: AwenaProcedureEntry[] = [
  ...
  {
    id: "petanque-match-config", domain: "game_setup", title: "Configurer une partie de Pétanque",
    aliases: ["configurer petanque", "partie petanque 1v1", "doublette triplette quadrette", "boules par joueur petanque"],
    route: "petanque_config",
    purpose: "Préparer le format Pétanque, les équipes/joueurs et les variantes avant de lancer le comptage.",
    prerequisites: [],
    steps: ["Ouvre Pétanque puis Configuration.", "Choisis le format : simple 1V1, chacun pour soi à 3, doublette, triplette, quadrette ou variante/handicap selon ce que l’écran propose.", "Sélectionne les joueurs ou équipes et vérifie le nombre de boules par joueur affiché.", "Contrôle la cible de score et les variantes actives.", "Si tu crées une équipe, vérifie son nom/logo puis enregistre.", "Relis le récapitulatif et démarre la partie."],
    verify: ["Le format affiché correspond au nombre de participants.", "Chaque joueur/équipe apparaît une seule fois et la cible de score est correcte."],
    troubleshooting: ["Bouton de départ indisponible : cherche un participant obligatoire manquant.", "Format incohérent : reviens au choix de format avant de modifier les joueurs."],
    warnings: [],
    related: ["Événements Pétanque", "Stats Pétanque"],
  },
  {
    id: "petanque-events", domain: "game_setup", title: "Saisir les événements détaillés de Pétanque",
    aliases: ["carreau trou bec petanque application", "saisir tir reussi petanque", "evenements petanque"],
    route: "petanque_play",
    purpose: "Enregistrer non seulement le score de la mène mais aussi les événements techniques utiles aux statistiques.",
    prerequisites: [],
    steps: ["Repère l’équipe/joueur actif et la mène en cours.", "Saisis le score de la mène selon l’interface.", "Lorsque l’écran le propose, renseigne les événements réels : carreau, tir réussi, trou, bec, but annulé/pointé ou poussée.", "Vérifie la timeline/journal avant de valider la mène suivante.", "Utilise l’annulation si un événement a été attribué au mauvais joueur."],
    verify: ["Le total de points et les événements de la mène correspondent à ce qui s’est réellement passé."],
    troubleshooting: ["Statistique technique absente plus tard : vérifie qu’elle a bien été saisie pendant le match.", "Erreur de joueur : corrige avant de continuer la mène suivante."],
    warnings: ["N’invente pas un événement uniquement pour corriger le score : utilise la correction prévue."],
    related: ["Historique Pétanque"],
  },
  {
    id: "petanque-tournament-guide", domain: "competition", title: "Créer un tournoi de Pétanque",
    aliases: ["tournoi petanque", "creer competition petanque", "petanque tournoi"],
    route: "petanque_tournaments",
    purpose: "Organiser une compétition Pétanque depuis le module dédié plutôt qu’une simple partie isolée.",
    prerequisites: [],
    steps: ["Ouvre la section Tournois Pétanque.", "Crée un tournoi et renseigne son identité/participants selon les champs disponibles.", "Choisis le format de compétition et les équipes si nécessaire.", "Vérifie le calendrier/tableau généré avant le premier match.", "Ouvre le match prévu, saisis le score, puis reviens au tournoi.", "Contrôle que le résultat fait progresser le classement/tableau."],
    verify: ["Le tournoi conserve les participants et les résultats après navigation."],
    troubleshooting: ["Si un match n’avance pas le tableau, vérifie qu’il a été validé depuis l’écran de score du tournoi."],
    warnings: [],
    related: ["Tournoi général", "Stats Pétanque"],
  },
  {
    id: "pingpong-config-official", domain: "game_setup", title: "Configurer un match de Ping-Pong",
    aliases: ["config ping pong", "preset officiel pingpong", "deuce service ping pong application"],
    route: "pingpong_config",
    purpose: "Régler le score par set, le nombre de sets gagnants, le service et les changements de côté.",
    prerequisites: [],
    steps: ["Choisis les joueurs, y compris A2/B2 si tu utilises un format double prévu.", "Sélectionne le preset Officiel, Fun ou Custom.", "Règle les points par set et le nombre de sets gagnants.", "Vérifie l’option d’écart de 2 et la règle de service.", "Choisis la fréquence de changement de serveur et le comportement au deuce.", "Vérifie les changements de côté et le set décisif.", "Relis les règles affichées puis démarre."],
    verify: ["Le résumé de configuration affiche points, sets et service attendus."],
    troubleshooting: ["Service qui semble alterner au mauvais rythme : vérifie preset + fréquence + règle de deuce."],
    warnings: [],
    related: ["Training Ping-Pong", "Stats Ping-Pong"],
  },
  {
    id: "pingpong-training-guide", domain: "game_setup", title: "Utiliser le Training Ping-Pong",
    aliases: ["training ping pong", "entrainement ping pong", "reussite rate pingpong"],
    route: "pingpong_training",
    purpose: "Enregistrer une séance de réussite chronométrée et suivre séries/records.",
    prerequisites: [],
    steps: ["Ouvre Ping-Pong > Training.", "Choisis l’objectif temps : libre ou durée proposée.", "Lance la séance.", "À chaque tentative, utilise + Réussi ou + Raté selon le résultat réel.", "Surveille la série en cours et la meilleure série.", "Termine la séance et consulte le record/résumé."],
    verify: ["Le compteur Réussite et la série évoluent à chaque action."],
    troubleshooting: ["Une série incohérente vient souvent d’un +Réussi/+Raté saisi à l’envers ; corrige immédiatement si l’écran permet l’annulation."],
    warnings: [],
    related: ["Statistiques Training"],
  },
  {
    id: "babyfoot-config-guide", domain: "game_setup", title: "Configurer une partie de Baby-foot",
    aliases: ["config baby foot", "regle bar babyfoot", "demi pissette gamelle application"],
    route: "babyfoot_config",
    purpose: "Choisir format, score/sets, chrono et règles de bar/compétition de la partie.",
    prerequisites: [],
    steps: ["Ouvre Baby-foot > Configuration.", "Choisis Sans équipe ou Équipe existante selon le format.", "Active ou non le jeu en sets puis règle le score cible par set.", "Configure le chrono et la prolongation si tu joues au temps.", "Choisis la règle de compétition ou les règles de bar proposées.", "Vérifie individuellement Demi, Pissette, Gamelle, Pêche offensive/défensive et leurs effets affichés.", "Relis le récapitulatif puis lance la partie."],
    verify: ["Les règles actives sont visibles avant le départ et correspondent à ce que les joueurs ont décidé."],
    troubleshooting: ["Désaccord sur une règle de bar : fige le réglage avant la partie et ne le change pas en cours de match."],
    warnings: [],
    related: ["Équipes Baby-foot", "Stats Baby-foot"],
  },
  {
    id: "babyfoot-team-guide", domain: "identity", title: "Créer ou modifier une équipe Baby-foot",
    aliases: ["equipe babyfoot", "team baby foot", "modifier equipe babyfoot"],
    route: "babyfoot_teams",
    purpose: "Créer une équipe réutilisable avec joueurs et identité visuelle.",
    prerequisites: [],
    steps: ["Ouvre Baby-foot > Équipes.", "Crée une nouvelle équipe ou ouvre une équipe existante.", "Choisis les profils joueurs.", "Renseigne le nom et les éléments d’identité proposés, par exemple pays/région/logo.", "Enregistre.", "Reviens à Configuration et sélectionne Équipe existante."],
    verify: ["L’équipe apparaît dans le sélecteur de configuration avec les bons joueurs."],
    troubleshooting: ["Équipe absente : vérifie l’enregistrement et que les profils existent toujours."],
    warnings: [],
    related: ["Profils locaux"],
  },
  {
    id: "babyfoot-league-guide", domain: "competition", title: "Utiliser la ligue Baby-foot",
    aliases: ["ligue babyfoot", "babyfoot league", "championnat baby foot"],
    route: "babyfoot_league",
    purpose: "Suivre une organisation de matchs Baby-foot au-delà d’une seule rencontre.",
    prerequisites: [],
    steps: ["Ouvre le module Ligue Baby-foot.", "Vérifie les équipes participantes et le calendrier/état affiché.", "Ouvre le match à jouer.", "Saisis ou joue la rencontre selon le parcours prévu.", "Valide le résultat.", "Reviens à la ligue pour contrôler classement/progression."],
    verify: ["Le résultat validé est répercuté dans l’état de la ligue."],
    troubleshooting: ["Si le classement ne change pas, vérifie que le match a été finalisé et non simplement quitté."],
    warnings: [],
    related: ["Stats équipes Baby-foot"],
  },
  {
    id: "molkky-config-guide", domain: "game_setup", title: "Configurer une partie de Mölkky",
    aliases: ["config molkky", "molkky classique rapide personnalise", "objectif 50 application"],
    route: "molkky_config",
    purpose: "Choisir preset, cible et règles de pénalité Mölkky avant la partie.",
    prerequisites: [],
    steps: ["Ouvre Mölkky puis choisis Classique, Rapide ou Personnalisé.", "Vérifie la cible de score ; le classique utilise 50 exact, tandis que d’autres presets peuvent modifier la cible.", "Contrôle la règle de dépassement et la pénalité prévue.", "Vérifie la limite de MISS/élimination si l’option est active.", "Sélectionne les joueurs.", "Relis les règles affichées et démarre."],
    verify: ["La cible et les options affichées correspondent au preset choisi."],
    troubleshooting: ["Une valeur change quand tu changes de preset : c’est normal si le preset réapplique ses paramètres ; repasse en Personnalisé pour ajuster."],
    warnings: [],
    related: ["Stats Mölkky"],
  },
  {
    id: "molkky-score-guide", domain: "game_setup", title: "Saisir correctement un tour de Mölkky",
    aliases: ["saisie molkky", "une quille plusieurs quilles", "miss molkky application"],
    route: "molkky_play",
    purpose: "Entrer le résultat d’un lancer en respectant la différence entre une seule quille et plusieurs quilles.",
    prerequisites: [],
    steps: ["Repère le joueur actif.", "Si une seule quille est tombée, saisis le numéro de cette quille.", "Si plusieurs quilles sont tombées, saisis le nombre de quilles tombées.", "Si aucune quille valable ne tombe, utilise MISS.", "Vérifie le nouveau total, notamment près de la cible exacte.", "Utilise Undo immédiatement en cas d’erreur."],
    verify: ["Le total augmente selon la règle du lancer et le joueur suivant devient actif."],
    troubleshooting: ["Score inattendu : vérifie si tu as saisi un numéro de quille alors que plusieurs sont tombées, ou l’inverse."],
    warnings: [],
    related: ["Règle 50 exact"],
  },
  {
    id: "dice-duel-config", domain: "game_setup", title: "Configurer Dice Duel / Dice Race",
    aliases: ["config dice duel", "config dice race", "cible des sets"],
    route: "dice_config",
    purpose: "Régler joueurs, cible, nombre de dés et sets pour les modes de dés simples.",
    prerequisites: [],
    steps: ["Choisis le mode de dés depuis le menu.", "Sélectionne les joueurs.", "Règle la cible de points.", "Vérifie le nombre de dés utilisé par le mode.", "Choisis le format en sets.", "Démarre puis utilise le bouton de lancer/saisie prévu."],
    verify: ["La config affiche la bonne cible, le bon nombre de dés et les bons joueurs."],
    troubleshooting: ["Score trop rapide/lent par rapport à la partie voulue : ajuste la cible avant de relancer un match."],
    warnings: [],
    related: ["Yam’s", "Farkle", "Poker Dice"],
  },
  {
    id: "yams-config-guide", domain: "game_setup", title: "Configurer une partie de Yam’s",
    aliases: ["config yams", "relances yams application", "manches yams"],
    route: "dice_yams_config",
    purpose: "Régler joueurs, relances, manches et sets du mode Yam’s.",
    prerequisites: [],
    steps: ["Ouvre Dice > Yam’s.", "Choisis les joueurs.", "Règle le nombre de relances autorisées.", "Choisis le nombre de manches.", "Configure les sets.", "Démarre et vérifie la scorecard avant de lancer."],
    verify: ["La scorecard/partie reprend les valeurs choisies."],
    troubleshooting: ["Une partie déjà lancée garde sa configuration ; reviens au setup pour changer relances/manches."],
    warnings: [],
    related: ["Scorecard Yam’s"],
  },
  {
    id: "farkle-config-guide", domain: "game_setup", title: "Configurer Farkle",
    aliases: ["config farkle", "hot dice farkle application", "bank bust farkle"],
    route: "dice_farkle_config",
    purpose: "Préparer la cible, les dés et les options push-your-luck du Farkle.",
    prerequisites: [],
    steps: ["Ouvre Dice > Farkle.", "Lis les règles intégrées du mode.", "Choisis les joueurs.", "Règle la cible et le nombre de dés.", "Configure les sets.", "Vérifie les options expérimentales/placeholder indiquées par l’écran avant de les considérer comme finalisées.", "Démarre la partie."],
    verify: ["La cible et le format sont affichés avant le premier lancer."],
    troubleshooting: ["Si une option porte explicitement la mention placeholder, ne la présente pas comme une règle définitivement implémentée."],
    warnings: ["Le push-your-luck implique de choisir quand bank/valider selon le moteur du mode."],
    related: ["Dice Race"],
  },
  {
    id: "x01-end-guide", domain: "stats", title: "Lire l’écran de fin X01",
    aliases: ["fin x01 stats", "x01 end avg3 first9 checkout", "resume x01"],
    route: "x01_end",
    purpose: "Comprendre les KPI de fin de partie X01 et vérifier l’enregistrement.",
    prerequisites: [],
    steps: ["Vérifie d’abord le gagnant et le score collectif/legs.", "Lis les volumes : darts, points, score par visite.", "Analyse AVG3D et Avg/1D.", "Regarde First9 pour le scoring d’ouverture.", "Regarde Best CO, CO hits, Darts CO et checkout pour la finition.", "Analyse Bust/Miss/Singles/Doubles/Triples si ces données ont été enregistrées.", "Utilise Partager cette partie si besoin puis vérifie l’Historique."],
    verify: ["Le gagnant et les KPI correspondent à la partie jouée."],
    troubleshooting: ["KPI incohérent : ouvre le détail/historique de la partie avant de conclure à un calcul global faux."],
    warnings: [],
    related: ["Stats X01", "Partager une partie"],
  },
  {
    id: "x01-undo-guide", domain: "game_setup", title: "Corriger une erreur X01 avec Undo",
    aliases: ["undo x01", "annuler score x01", "double saisie x01"],
    route: "x01_play_v3",
    purpose: "Revenir proprement sur une erreur de saisie sans fausser score, tour et statistiques.",
    prerequisites: [],
    steps: ["Arrête toute nouvelle saisie dès que tu vois l’erreur.", "Utilise Undo/Annuler la dernière action.", "Vérifie le score restauré et le joueur actif.", "Vérifie que les indicateurs de volée/tirs ont également reculé.", "Saisis ensuite la bonne action une seule fois."],
    verify: ["Score, joueur actif et détail de volée retrouvent l’état précédent."],
    troubleshooting: ["Si tu as déjà joué plusieurs actions après l’erreur, utilise les Undo successifs permis au lieu d’inventer une compensation."],
    warnings: ["Évite les clics répétés si l’interface tarde à répondre."],
    related: ["Historique X01"],
  },
  {
    id: "training-x01-guide", domain: "game_setup", title: "Configurer Training X01",
    aliases: ["training x01", "entrainement x01", "301 501 training"],
    route: "training_x01",
    purpose: "Lancer un entraînement X01 comparable entre plusieurs joueurs.",
    prerequisites: [],
    steps: ["Ouvre Training > X01.", "Choisis le score de départ 301/501/701/901 proposé.", "Choisis la règle de sortie Simple/Double/Master selon l’écran.", "Sélectionne le ou les joueurs/équipes.", "Vérifie la méthode de saisie, dont la commande vocale de scores si disponible.", "Démarre : tous les participants doivent utiliser la même configuration.", "À la fin, consulte le comparatif sauvegardé dans Stats Training."],
    verify: ["Tous les participants jouent la même variante et les résultats sont séparés par configuration dans les stats."],
    troubleshooting: ["Comparaison incohérente : vérifie que les joueurs ont bien utilisé la même config de départ/sortie."],
    warnings: [],
    related: ["Stats Training"],
  },
  {
    id: "training-mode-guide", domain: "game_setup", title: "Utiliser les autres Trainings Fléchettes",
    aliases: ["training precision gauntlet", "time attack darts", "repeat master ghost doubleio"],
    route: "training_mode",
    purpose: "Lancer un module d’entraînement spécialisé avec un participant valide.",
    prerequisites: [],
    steps: ["Ouvre Training.", "Choisis le module souhaité, par exemple précision, time attack, repeat master, ghost ou double-oriented selon les modes disponibles.", "Sélectionne un participant Training valide.", "Lis l’objectif spécifique affiché par le module.", "Lance la séance et saisis les résultats réels.", "Termine la séance pour enregistrer les statistiques prévues."],
    verify: ["Le joueur sélectionné apparaît et la séance démarre avec l’objectif du module."],
    troubleshooting: ["Message Aucun participant Training valide : reviens au sélecteur et choisis/crée un profil utilisable."],
    warnings: [],
    related: ["Training X01"],
  },
  {
    id: "firefighter-config-guide", domain: "game_setup", title: "Configurer Darts Firefighter",
    aliases: ["config darts firefighter", "mission feu foret darts", "canadair firefighter config"],
    route: "darts_firefighter_config",
    purpose: "Préparer une mission Firefighter complète : scénario, brigade, territoire, feu, propagation et moyens.",
    prerequisites: [],
    steps: ["Choisis une mission préconfigurée ou Personnaliser.", "Dans Mission, fixe l’objectif/scénario.", "Dans Brigade, sélectionne joueurs/Bots et dartsets.", "Dans Territoire, choisis carte, zones actives et secteurs.", "Dans Départ du feu, règle foyers, fumée et zones critiques.", "Dans Propagation, règle difficulté, vitesse, vent et prévisions.", "Dans Moyens, vérifie eau, Bull, DBULL/Canadair et jauges.", "Dans Partie, règle volée, saisie, MISS et bonus.", "Ouvre Résumé et vérifie tout avant de démarrer."],
    verify: ["Le résumé liste la brigade, les zones actives, les niveaux de danger et les moyens choisis."],
    troubleshooting: ["Mission trop dure : baisse propagation/difficulté ou augmente les moyens avant de relancer.", "Zone/secteur incohérent : reviens à Territoire avant de toucher au moteur de partie."],
    warnings: [],
    related: ["Jouer Darts Firefighter"],
  },
  {
    id: "firefighter-play-guide", domain: "game_setup", title: "Jouer Darts Firefighter",
    aliases: ["comment jouer darts firefighter", "territoire selectionne canadair", "actions firefighter darts"],
    route: "darts_firefighter_play",
    purpose: "Choisir une zone, lire le danger et appliquer l’action de fléchette appropriée.",
    prerequisites: [],
    steps: ["Repère le pompier actif et l’état global de la mission.", "Sélectionne le territoire concerné ; lis feu, fumée, protection, menace et zones adjacentes.", "Regarde l’objectif/action conseillée.", "Saisis le tir réel : Simple/Double/Triple appliquent la puissance prévue au secteur.", "Utilise BULL/DBULL uniquement selon les moyens configurés ; DBULL peut déclencher le Canadair.", "Vérifie l’intervention réalisée et la propagation avant le tour suivant."],
    verify: ["Le niveau de danger/protection évolue et le journal confirme l’intervention."],
    troubleshooting: ["Aucune cible : sélectionne d’abord un territoire valide.", "Résultat inattendu : vérifie secteur, multiplicateur et règle de propagation de la mission."],
    warnings: [],
    related: ["Configuration Firefighter"],
  },
  {
    id: "darts-poker-config-guide", domain: "game_setup", title: "Configurer Darts Poker",
    aliases: ["config darts poker", "pouvoirs poker darts", "joker dbull poker darts"],
    route: "darts_poker_config",
    purpose: "Régler joueurs/Bots, manches, visibilité et pouvoirs du Darts Poker.",
    prerequisites: [],
    steps: ["Lis les règles intégrées : 6 fléchettes une par une pour construire une meilleure main de 5 cartes.", "Sélectionne joueurs/Bots et leur niveau.", "Choisis le nombre de manches et l’ordre.", "Règle la visibilité des mains adverses.", "Active/désactive les pouvoirs Double/Triple, Joker DBULL et contrats bonus selon la partie voulue.", "Choisis la méthode de saisie.", "Relis le classement des mains puis démarre."],
    verify: ["Le récapitulatif indique les pouvoirs et le format voulus."],
    troubleshooting: ["Une main surprenante : rappelle-toi que le moteur conserve automatiquement la meilleure combinaison de 5 cartes parmi les cartes acquises."],
    warnings: [],
    related: ["Règles Darts Poker"],
  },
  {
    id: "cargo-config-guide", domain: "game_setup", title: "Configurer Cargo",
    aliases: ["config cargo darts", "contrats palettes cargo", "serie stricte cargo darts"],
    route: "cargo_config",
    purpose: "Préparer les contrats, séries, risques et format équipe/convoi du mode Cargo.",
    prerequisites: [],
    steps: ["Choisis les chauffeurs/joueurs/Bots et leur niveau.", "Choisis Individuel ou Équipes/Convoi.", "Sélectionne la variante de mission logistique.", "Règle le nombre de tours, contrats visibles et ordre.", "Choisis la règle de série : segment exact S/D/T ou même numéro selon les options.", "Décide si une série peut continuer entre les tours.", "Configure la conséquence d’une erreur : sécuriser, valider partiellement ou perdre la charge.", "Relis les risques/bonus puis démarre."],
    verify: ["Le résumé montre format, variante, contrats et règle de série."],
    troubleshooting: ["Progression perdue : vérifie la conséquence d’erreur configurée avant de soupçonner un bug."],
    warnings: [],
    related: ["Règles Cargo"],
  },
  {
    id: "ocean-control-config-guide", domain: "game_setup", title: "Configurer Ocean Control",
    aliases: ["config ocean control", "flotte sonar dbull", "mode tactique ocean darts"],
    route: "ocean_control_config",
    purpose: "Régler flotte, organisation et systèmes d’armes du mode Ocean Control.",
    prerequisites: [],
    steps: ["Choisis joueurs ou équipes.", "Sélectionne le format de flotte : Escarmouche, Standard ou Armada selon l’écran.", "Choisis le système d’armes Classique ou Tactique.", "Vérifie l’effet des Simple/Double/Triple.", "Active/configure Sonar au BULL et frappe DBULL selon la partie.", "Lis la règle concernant un tir déjà effectué.", "Relis l’objectif : détruire la flotte adverse, puis démarre."],
    verify: ["La flotte, le mode tactique et les pouvoirs BULL/DBULL correspondent au récapitulatif."],
    troubleshooting: ["Tir consommé sans effet : vérifie si la zone avait déjà été visée et la règle correspondante."],
    warnings: [],
    related: ["Règles Ocean Control"],
  },
  {
    id: "messages-guide", domain: "online", title: "Utiliser Messages et invitations",
    aliases: ["messages application", "messenger multisports", "invitations salon online messages"],
    route: "messages",
    purpose: "Centraliser conversations, invitations, annonces et parties partagées.",
    prerequisites: [],
    steps: ["Ouvre Messages.", "Choisis le type : amis, groupes/salons, annonces, invitations Online ou parties reçues selon les onglets/cartes.", "Ouvre la conversation ou notification concernée.", "Réponds, accepte/refuse ou ouvre la partie selon l’action disponible.", "Vérifie l’état en ligne/absent/déconnecté du contact lorsque pertinent."],
    verify: ["Le message/action apparaît dans le bon fil ou la notification change d’état."],
    troubleshooting: ["Invitation introuvable : vérifie le bon onglet et la session Online."],
    warnings: [],
    related: ["Amis", "Online"],
  },
  {
    id: "avatar-ai-guide", domain: "identity", title: "Créer un avatar IA",
    aliases: ["avatar ia", "creer avatar ai", "galerie avatar"],
    route: "avatar",
    purpose: "Créer ou choisir un avatar pour un profil en utilisant les options disponibles de la galerie/IA.",
    prerequisites: [],
    steps: ["Ouvre Profils puis Avatar.", "Choisis la galerie existante ou le parcours Avatar IA.", "Sélectionne le style/caricature proposé et les paramètres visuels disponibles.", "Génère ou choisis l’avatar.", "Vérifie l’aperçu avant de l’associer au profil.", "Enregistre puis reviens au profil pour contrôler l’affichage."],
    verify: ["Le nouvel avatar reste visible après navigation et rechargement du profil."],
    troubleshooting: ["Avatar visible puis disparu : vérifie persistance/cache média avant de régénérer."],
    warnings: ["Évite de créer plusieurs variantes inutiles si le problème est seulement un cache d’affichage."],
    related: ["Profils"],
  },
  {
    id: "bots-profile-guide", domain: "identity", title: "Gérer les Bots IA / CPU",
    aliases: ["profils bots", "creer bot cpu", "bots ia application"],
    route: "profiles_bots",
    purpose: "Comprendre la différence entre Bots IA intégrés et Bots CPU/profils créés par l’utilisateur, puis les sélectionner dans les modes compatibles.",
    prerequisites: [],
    steps: ["Ouvre Profils > Bots.", "Parcours les Bots IA intégrés disponibles pour le mode concerné.", "Si l’application permet des Bots CPU personnalisés, crée ou modifie leur profil/niveau.", "Vérifie avatar, nom, niveau et dartset.", "Enregistre.", "Dans une configuration de mode compatible, ajoute le Bot depuis le sélecteur de participants."],
    verify: ["Le Bot apparaît dans la liste et son niveau est celui attendu."],
    troubleshooting: ["Bot absent dans un mode : vérifie que ce mode déclare réellement la prise en charge des Bots."],
    warnings: [],
    related: ["X01 Bots"],
  },
  {
    id: "leaderboard-guide", domain: "stats", title: "Lire un classement / leaderboard",
    aliases: ["lire leaderboard", "classement stats", "stats leaderboards"],
    route: "stats_leaderboards",
    purpose: "Comparer les joueurs sans surinterpréter un classement construit sur peu de matchs.",
    prerequisites: [],
    steps: ["Ouvre Stats puis le leaderboard du sport/mode voulu.", "Choisis la métrique : victoires, win rate, moyenne, points ou autre critère disponible.", "Vérifie la période et les filtres.", "Regarde le nombre de matchs/observations à côté du rang.", "Ouvre le joueur ou les matchs sources si tu veux comprendre le classement."],
    verify: ["Le critère et la période sont clairement identifiés."],
    troubleshooting: ["Un joueur avec peu de matchs domine au pourcentage : compare aussi le volume avant de conclure."],
    warnings: [],
    related: ["Filtres Stats"],
  },
  {
    id: "stats-empty-guide", domain: "stats", title: "Diagnostiquer des statistiques vides",
    aliases: ["stats vides", "aucune stats", "statistiques a zero"],
    route: "statsHub",
    purpose: "Comprendre pourquoi un écran Stats n’affiche rien sans supposer une perte de données.",
    prerequisites: [],
    steps: ["Vérifie le profil/joueur sélectionné.", "Retire temporairement les filtres de période/mode.", "Vérifie qu’au moins une partie du mode existe dans Historique.", "Ouvre une partie source et contrôle qu’elle contient les champs nécessaires à la statistique.", "Reviens aux Stats et force seulement un rafraîchissement non destructif si prévu."],
    verify: ["Les matchs attendus sont visibles dans Historique et les filtres couvrent leur date/mode."],
    troubleshooting: ["Historique présent mais KPI absent : le provider/stat peut ne pas exploiter ce champ.", "N’efface pas IndexedDB ou les profils pour résoudre un écran à zéro."],
    warnings: ["Sauvegarde avant toute restauration."],
    related: ["Historique"],
  },
  {
    id: "spectator-guide", domain: "online", title: "Utiliser le mode spectateur",
    aliases: ["spectateur online", "spectator mode", "voir partie online"],
    route: "spectator",
    purpose: "Rejoindre une partie ou un affichage Online en lecture lorsque le parcours spectateur est disponible.",
    prerequisites: [],
    steps: ["Ouvre le lien/code/entrée spectateur fourni par la room.", "Vérifie que la session Online est joignable.", "Rejoins en mode spectateur plutôt qu’en joueur si ce choix est proposé.", "Laisse l’écran ouvert pour recevoir les snapshots de partie."],
    verify: ["Les scores/joueurs se mettent à jour sans que tu contrôles la partie."],
    troubleshooting: ["Écran figé : vérifie d’abord que l’hôte/room continue de publier et que le réseau est actif."],
    warnings: [],
    related: ["Viewer tablette", "Online"],
  },
  {
    id: "tournament-roadmap-guide", domain: "competition", title: "Comprendre la Roadmap Tournois",
    aliases: ["roadmap tournoi", "fonctions tournoi a venir", "swiss double elim export"],
    route: "tournament_roadmap",
    purpose: "Distinguer les fonctions de tournoi déjà disponibles des éléments explicitement indiqués comme à venir.",
    prerequisites: [],
    steps: ["Ouvre Roadmap Tournois.", "Lis la section Fonctions de base pour les capacités déjà présentes.", "Repère les éléments marqués À venir.", "N’utilise pas un élément Roadmap comme preuve qu’une fonction est déjà utilisable.", "Pour lancer un tournoi actuel, reviens aux écrans de création/liste."],
    verify: ["Les fonctions réellement disponibles ont un écran/une action utilisable, contrairement aux éléments seulement annoncés."],
    troubleshooting: ["Si tu cherches Swiss, Double Elim, Export ou Stats tournoi, vérifie leur statut actuel sur la Roadmap avant de les promettre."],
    warnings: [],
    related: ["Créer un tournoi"],
  },
  {
    id: "mode-not-ready-guide", domain: "troubleshooting", title: "Comprendre un mode non prêt",
    aliases: ["mode not ready", "mode bientot disponible", "soon jeu"],
    route: "mode_not_ready",
    purpose: "Expliquer pourquoi une carte ou un concept peut exister dans le catalogue sans moteur finalisé.",
    prerequisites: [],
    steps: ["Lis le message de statut de l’écran.", "Reviens au menu du sport.", "Choisis un mode déclaré jouable/ready.", "Si tu demandes à Awéna les règles du concept non prêt, elle peut les expliquer mais doit signaler qu’il n’est pas finalisé dans l’application."],
    verify: ["Le mode jouable ouvre une configuration réelle au lieu de l’écran non prêt."],
    troubleshooting: ["Ne tente pas de forcer une route Play d’un mode non finalisé."],
    warnings: [],
