...
let diagEntriesCache: any[] | null = null;
let diagFlushTimer: number | null = null;

function flushDiagStore() {
  try {
    const next = Array.isArray(diagEntriesCache) ? diagEntriesCache : [];
    window.localStorage.setItem(GOOGLE_CAST_DIAG_KEY, JSON.stringify(next));
  } catch {}
}

function scheduleDiagFlush(immediate = false) {
  if (typeof window === "undefined") return;
  if (immediate) {
    if (diagFlushTimer != null) {
      window.clearTimeout(diagFlushTimer);
      diagFlushTimer = null;
    }
    flushDiagStore();
    return;
...
function pushDiag(entry: string, extra?: any) {
  try {
    if (!Array.isArray(diagEntriesCache)) {
      const prev = JSON.parse(window.localStorage.getItem(GOOGLE_CAST_DIAG_KEY) || "[]");
      diagEntriesCache = Array.isArray(prev) ? prev : [];
    }
    const now = new Date().toISOString();
    diagEntriesCache.push({ now, entry, extra: extra == null ? null : extra });
    while (diagEntriesCache.length > 200) diagEntriesCache.shift();
    scheduleDiagFlush(false);
  } catch {}
}
...
export function getGoogleCastDiagLog() {
  try {
    if (Array.isArray(diagEntriesCache)) return diagEntriesCache.slice();
    const data = JSON.parse(window.localStorage.getItem(GOOGLE_CAST_DIAG_KEY) || "[]");
    diagEntriesCache = Array.isArray(data) ? data : [];
    return Array.isArray(diagEntriesCache) ? diagEntriesCache.slice() : [];
  } catch {
    return [];
  }
}
