...
function avatarRing(accent: string): React.CSSProperties {
  return {
    width: 72,
    height: 72,
    borderRadius: 999,
...
function SideBlock({ visual, accent, align }: { visual: SideVisual; accent: string; align: "left" | "right" }) {
  ...
  return (
    <div style={{ minWidth: 0, display: "grid", justifyItems: align === "left" ? "start" : "end", textAlign: align, gap: 8 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "minmax(82px,1fr) auto minmax(82px,1fr)", gap: 10, alignItems: "center" }}>
            ...
            >
              ...
              <div style={{ marginTop: 6, display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}>
                <span style={{ fontSize: 44, lineHeight: 0.95, fontWeight: 1100, color: green, textShadow: `0 0 16px ${green}66` }}>{scoreA}</span>
                <span style={{ fontSize: 20, lineHeight: 1, fontWeight: 1100, color: "rgba(255,255,255,0.92)" }}>–</span>
                <span style={{ fontSize: 44, lineHeight: 0.95, fontWeight: 1100, color: pink, textShadow: `0 0 16px ${pink}66` }}>{scoreB}</span>
              </div>
