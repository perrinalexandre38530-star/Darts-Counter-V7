...
import { getNativeTrack, nativeTrackingStatus } from "../activity/nativeActivityTracking";
import {
...
export default function RunningActiveSessionDock({
  ...
  React.useEffect(() => {
    const primary = sessions.find((row) => row.mode === "native-gps" && !row.paused);
    if (!primary) return;
    let alive = true;
    const refresh = async () => {
      try {
  ...
  return <div style={{ position: "fixed", right: "calc(env(safe-area-inset-right, 0px) + 4px)", top: "46%", transform: "translateY(-50%)", zIndex: 72, display: "grid", justifyItems: "end", gap: 7, pointerEvents: "none" }}>
    {expanded ? <div style={{ width: 230, padding: 7, borderRadius: 16, border: "1px solid rgba(255,255,255,.12)", background: "rgba(7,9,14,.96)", boxShadow: "0 16px 38px rgba(0,0,0,.52)", backdropFilter: "blur(16px)", pointerEvents: "auto" }}>
      {sessions.map((session) => <button key={session.id} type="button" onClick={() => openSession(session)} style={{ width: "100%", minHeight: 48, padding: "7px 9px", border: 0, borderRadius: 12, background: "transparent", color: "#fff", textAlign: "left", display: "grid", gridTemplateColumns: "28px 1fr auto", gap: 8, alignItems: "center", cursor: "pointer" }}>
        ...
        <span style={{ minWidth: 0 }}><b style={{ display: "block", fontSize: 8.8, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{session.title}</b><small style={{ display: "block", marginTop: 2, fontSize: 7.4, opacity: .6 }}>{session.paused ? "PAUSE" : "EN COURS"} · {formatDuration(runningActiveElapsedMs(session))}</small></span>
        <span style={{ color: accent, fontSize: 8.2, fontWeight: 1000 }}>{formatDistance(Number(session.lastDistanceM || 0))}</span>
      </button>)}
      <div style={{ padding: "4px 8px 2px", fontSize: 6.8, opacity: .45, textAlign: "center" }}>{sessions.length}/3 SESSIONS · 1 TRACKING GPS/CAPTEURS À LA FOIS</div>
    </div> : null}
