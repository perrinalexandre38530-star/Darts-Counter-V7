...
function matchFormat(rec:any){const s=rec?.summary||rec?.payload?.summary||{};const mode=s?.legVictoryMode||rec?.payload?.config?.legVictoryMode||"best_of";const target=n(s?.legVictoryTarget??rec?.payload?.config?.legVictoryTarget??s?.legsBestOf??rec?.payload?.config?.legsBestOf,3);return mode==="first_to"?`FT${target}`:`BO${target}`;}
...
export default function AttrapeMoiStatsTabFull({records,playerId}:Props){
  ...
  const scoreEscapes=sum("scoreEscapeCredits")||sum("scoreEscapes"); const roundEscapes=sum("roundEscapeCredits")||Math.max(0,escapes-scoreEscapes);
  ...
  const evolution=rows.map(({rec,pl})=>({t:playedAt(rec),avg3:n(pl?.avg3),runner:n(pl?.runnerAvg3),chaser:n(pl?.chaserAvg3),best:n(pl?.bestVisit),points:n(pl?.points),win:pl?.winner||pl?.win?100:0,captures:n(pl?.captureCredits),escapes:n(pl?.escapeCredits),scoreEscapes:n(pl?.scoreEscapeCredits),roundEscapes:n(pl?.roundEscapeCredits)}));
  ...
  return <div>
    ...
    {tab==="resume"?<>
      <div style={{display:"grid",gridTemplateColumns:"repeat(2,minmax(0,1fr))",gap:9}}>
        ...
        <Kpi label="Sets gagnés" value={setsWon} sub={`${legsWon} manches`} color={C.gold}/><Kpi label="Captures / Évasions" value={`${captures} / ${escapes}`} sub={`${captureRate}% cap. · ${escapeRate}% év.`} color={C.chaser}/><Kpi label="Évasions au score" value={scoreEscapes} sub={`${escapes ? r1(scoreEscapes/escapes*100) : 0}% des évasions`} color={C.runner}/><Kpi label="Évasions aux rounds" value={roundEscapes} sub={`${escapes ? r1(roundEscapes/escapes*100) : 0}% des évasions`} color={C.runner}/>
    ...
    {tab==="roles"?<div style={{display:"grid",gap:10}}>
      <div style={{...card,padding:13}}><div style={{display:"grid",gridTemplateColumns:"repeat(2,minmax(0,1fr))",gap:8}}><div style={{padding:11,borderRadius:14,background:`${C.runner}0d`,border:`1px solid ${C.runner}33`}}><div style={{color:C.runner,fontWeight:1100}}>FUYARD</div><div style={{marginTop:8,display:"grid",gridTemplateColumns:"repeat(2,minmax(0,1fr))",gap:7}}><Kpi label="AVG/3" value={runnerAvg} color={C.runner}/><Kpi label="Best" value={runnerBest} color={C.gold}/><Kpi label="Points" value={runnerPoints} color="#fff"/><Kpi label="Victoires" value={runnerWins} sub={`${escapes} évasions`} color={C.runner}/><Kpi label="Évasion score" value={scoreEscapes} color={C.runner}/><Kpi label="Évasion rounds" value={roundEscapes} color={C.runner}/></div></div><div style={{padding:11,borderRadius:14,background:`${C.chaser}0d`,border:`1px solid ${C.chaser}33`}}><div style={{color:C.chaser,fontWeight:1100}}>CHASSEUR</div><div style={{marginTop:8,display:"grid",gridTemplateColumns:"repeat(2,minmax(0,1fr))",gap:7}}><Kpi label="AVG/3" value={chaserAvg} color={C.chaser}/><Kpi label="Best" value={chaserBest} color={C.gold}/><Kpi label="Points" value={chaserPoints} color="#fff"/><Kpi label="Victoires" value={chaserWins} sub={`${captures} captures`} color={C.chaser}/></div></div></div></div>
