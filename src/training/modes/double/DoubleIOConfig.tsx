...
export default function DoubleIOConfig({ profiles, onStart, onExit }: { profiles?: Profile[]; onStart:(cfg:any)=>void; onExit:()=>void; }) {
  ...
  return (
    <TrainingShell
      header={<TrainingHeader title="Double In/Out" onBack={onExit} rules={<p>Travaille les doubles : entrée, sortie, ou les deux.</p>} />}
      body={
        <div>
          ...
          <TrainingOptionCard title="Double In" subtitle="Commencer sur un double" active={mode==="DI"} onClick={()=>setMode("DI")} />
          <TrainingOptionCard title="Double Out" subtitle="Finir sur un double" active={mode==="DO"} onClick={()=>setMode("DO")} />
          <TrainingOptionCard title="Double In + Double Out" subtitle="Entrée et sortie en double" active={mode==="DIDO"} onClick={()=>setMode("DIDO")} />
