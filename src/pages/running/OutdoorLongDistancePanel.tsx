...
export default function OutdoorLongDistancePanel({ route, sport, extras, lang, accent, textSoft, onPrefsChange }: Props) {
  ...
  const copy = lang.startsWith("fr") ? {
    title: "MODE LONGUE DISTANCE", sub: "Roadbook indicatif pour préparer les sorties longues. Le profil batterie pilote réellement l’échantillonnage GPS Android.", time: "DURÉE EST.", stages: "ÉTAPES", battery: "PROFIL BATTERIE", reminders: "RAPPELS", hydration: "RAPPEL BOIRE", fuel: "RAPPEL RAVITO", off: "OFF", normal: "NORMAL", eco: "ÉCO", ultra: "ULTRA", roadbook: "ROADBOOK", gpsEvery: "GPS",
  } : lang.startsWith("es") ? {
    title: "MODO LARGA DISTANCIA", sub: "Roadbook orientativo. El perfil de batería controla realmente el muestreo GPS Android.", time: "TIEMPO EST.", stages: "ETAPAS", battery: "PERFIL BATERÍA", reminders: "RECORDATORIOS", hydration: "RECORDAR BEBER", fuel: "RECORDAR COMER", off: "OFF", normal: "NORMAL", eco: "ECO", ultra: "ULTRA", roadbook: "ROADBOOK", gpsEvery: "GPS",
  } : {
    title: "LONG DISTANCE MODE", sub: "Indicative roadbook. The battery profile now controls the native Android GPS sampling rate.", time: "EST. TIME", stages: "STAGES", battery: "BATTERY PROFILE", reminders: "REMINDERS", hydration: "DRINK REMINDER", fuel: "FUEL REMINDER", off: "OFF", normal: "NORMAL", eco: "ECO", ultra: "ULTRA", roadbook: "ROADBOOK", gpsEvery: "GPS",
  };
  ...
  return <RunningSurface accent={accent} style={{ marginTop: 8 }}>
    ...
    <div style={{ display: "grid", gridTemplateColumns: "repeat(4,minmax(0,1fr))", gap: 5, marginTop: 8 }}>
      <Mini label={copy.time} value={formatDuration(plan.expectedMs)} accent={accent}/><Mini label={copy.stages} value={String(plan.stages.length)} accent={accent}/><Mini label={copy.battery} value={batteryLabel(prefs.batteryMode)} accent={accent}/><Mini label={copy.gpsEvery} value={`~${gpsIntervalSecForBatteryMode(prefs.batteryMode)} s`} accent={accent}/>
    </div>
