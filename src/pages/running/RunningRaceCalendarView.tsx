...
export default function RunningRaceCalendarView({ lang, accent, textSoft }: Props) {
  ...
  return <Section title={copy.title}>
    ...
    <div style={{ display: "grid", gap: 8, marginTop: 12 }}>
      {races.length ? races.map((race) => { const days = raceDaysLeft(race); const pace = race.targetTimeMs / 1000 / (race.distanceM / 1000); return <div key={race.id} className="card" style={{ padding: 10, borderColor: race.primary ? `${accent}66` : undefined }}>
...
const inputStyle: React.CSSProperties = { width: "100%", minHeight: 37, borderRadius: 10, border: "1px solid rgba(255,255,255,.1)", background: "rgba(0,0,0,.2)", color: "#fff", padding: "7px 8px", boxSizing: "border-box", fontSize: 9.5, outline: "none" };
