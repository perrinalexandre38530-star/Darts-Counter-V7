...
export default function RunningTerrain3DMap({ points, accent, lang, textSoft = "#a8a8b3", height = "clamp(320px,58svh,620px)", fullscreen = false, routeName, places = [], activePointIndex = null, onActivePointChange, onPlaceSelect, onFallback2D, showReplay = true, preferCompat: _legacyPreferCompat = false }: Props) {
  ...
  return <div className="running-map-shell" style={{ position: "relative", width: "100%", height: fullscreen ? "100%" : height, minHeight: fullscreen ? 0 : 300, overflow: "hidden", borderRadius: fullscreen ? 0 : 20, background: "#101821", border: fullscreen ? 0 : "1px solid rgba(255,255,255,.09)", boxShadow: fullscreen ? undefined : "0 22px 56px rgba(0,0,0,.30)" }}>
    ...
    {status !== "ready" ? <div style={{ position: "absolute", inset: 0, zIndex: 18 }}><RunningTerrain3DCompat points={safePoints} accent={accent} lang={lang} textSoft={textSoft} height="100%" fullscreen={fullscreen} activePointIndex={effectiveIndex} onActivePointChange={onActivePointChange}/><div style={{ position: "absolute", left: 10, bottom: fullscreen ? "max(12px,env(safe-area-inset-bottom))" : 10, zIndex: 30, padding: "6px 9px", borderRadius: 999, background: "rgba(5,8,13,.86)", border: `1px solid ${status === "error" ? "rgba(255,184,92,.45)" : `${accent}44`}`, color: status === "error" ? "#ffca78" : accent, fontSize: 7.2, fontWeight: 1000, backdropFilter: "blur(10px)" }}>{status === "error" ? pickText(lang, "3D GPS active · relief DEM indisponible", "GPS 3D active · DEM terrain unavailable", "3D GPS activo · relieve DEM no disponible") : pickText(lang, "3D immédiate · chargement du relief HD…", "Instant 3D · loading HD terrain…", "3D inmediata · cargando relieve HD…")}</div>{status === "error" && onFallback2D ? <button className="btn" onClick={onFallback2D} style={{ position: "absolute", right: 10, bottom: fullscreen ? "max(12px,env(safe-area-inset-bottom))" : 10, zIndex: 31, minHeight: 34, padding: "5px 9px", color: accent, borderColor: `${accent}55`, background: "rgba(5,8,13,.88)", fontSize: 7.2, fontWeight: 1000 }}>{pickText(lang, "REVENIR EN 2D", "BACK TO 2D", "VOLVER A 2D")}</button> : null}</div> : null}
