...
import { connectHeartRateSensor, connectRunningCadenceSensor, detectFitnessConnectorCapabilities, disconnectRunningSensor, getRunningSensorSnapshot, subscribeRunningSensors, type RunningSensorKind, type RunningSensorSnapshot } from "../../activity/runningSensors";
...
export default function RunningConnectionsPanel({ lang, accent, textSoft, compact = false }: { lang: string; accent: string; textSoft: string; compact?: boolean }) {
  ...
  const copy = lang === "fr" ? {
    title: "CONNEXIONS & CAPTEURS", sub: "Capteurs BLE utilisables dès maintenant sur navigateur compatible. Les passerelles santé/cloud sont préparées sans activer Android prématurément.", hr: "CEINTURE CARDIO", foot: "FOOTPOD / CADENCE", connect: "CONNECTER", disconnect: "DÉCONNECTER", live: "LIVE", unavailable: "BLE NON DISPONIBLE", health: "HEALTH CONNECT", apple: "APPLE HEALTH", garmin: "GARMIN CONNECT", files: "FICHIERS SPORT", native: "Bridge natif requis", detected: "Bridge détecté", cloud: "API cloud / OAuth requis", configured: "API configurée", filesReady: "GPX / TCX actifs · FIT à câbler", bpm: "bpm", spm: "pas/min",
  } : lang === "es" ? {
    title: "CONEXIONES Y SENSORES", sub: "Sensores BLE utilizables ahora en navegadores compatibles. Los puentes salud/cloud quedan preparados sin activar Android todavía.", hr: "BANDA CARDÍACA", foot: "FOOTPOD / CADENCIA", connect: "CONECTAR", disconnect: "DESCONECTAR", live: "LIVE", unavailable: "BLE NO DISPONIBLE", health: "HEALTH CONNECT", apple: "APPLE HEALTH", garmin: "GARMIN CONNECT", files: "ARCHIVOS DEPORTIVOS", native: "Requiere puente nativo", detected: "Puente detectado", cloud: "Requiere API cloud / OAuth", configured: "API configurada", filesReady: "GPX / TCX activos · FIT pendiente", bpm: "bpm", spm: "pas/min",
  } : {
    title: "CONNECTIONS & SENSORS", sub: "BLE sensors can work now on compatible browsers. Health/cloud bridges are prepared without prematurely enabling Android.", hr: "HEART RATE STRAP", foot: "FOOTPOD / CADENCE", connect: "CONNECT", disconnect: "DISCONNECT", live: "LIVE", unavailable: "BLE UNAVAILABLE", health: "HEALTH CONNECT", apple: "APPLE HEALTH", garmin: "GARMIN CONNECT", files: "SPORT FILES", native: "Native bridge required", detected: "Bridge detected", cloud: "Cloud API / OAuth required", configured: "API configured", filesReady: "GPX / TCX active · FIT next", bpm: "bpm", spm: "steps/min",
  };
  ...
  return <div>
    ...
    {!compact ? <div style={{ display: "grid", gridTemplateColumns: "repeat(2,minmax(0,1fr))", gap: 7, marginTop: 9 }}>
      <Connector icon="♥" title={copy.health} status={capabilities.healthConnectBridge ? copy.detected : copy.native} accent={accent} active={capabilities.healthConnectBridge}/>
