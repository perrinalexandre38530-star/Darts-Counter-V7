...
export default function OutdoorRouteLiveMap({ route, track, sport, lang, accent, textSoft, liveDistanceM, elapsedMs, liveElevationGainM = 0, extras, reroute = null, rerouteBusy = false, onClose }: Props) {
  ...
  return <div role="dialog" aria-label={copy.nav} style={{ position: "fixed", inset: 0, zIndex: 220, background: "#0b1218", overflow: "hidden" }}>
    ...
    <div style={{ position: "absolute", right: 10, top: "38%", zIndex: 12, display: "grid", gap: 7 }}>
      <button className="btn" onClick={() => setZoomDelta((value) => Math.min(4, value + 1))} style={{ width: 42, height: 42, minHeight: 42, padding: 0, background: "rgba(7,10,15,.90)", fontSize: 18 }}>+</button>
      <button className="btn" onClick={() => setZoomDelta((value) => Math.max(-5, value - 1))} style={{ width: 42, height: 42, minHeight: 42, padding: 0, background: "rgba(7,10,15,.90)", fontSize: 18 }}>−</button>
      <button className="btn" onClick={() => { setFollow((value) => !value); setZoomDelta(0); }} style={{ width: 42, minHeight: 42, padding: 3, background: "rgba(7,10,15,.90)", color: follow ? accent : undefined, borderColor: follow ? `${accent}66` : undefined, fontSize: 7, fontWeight: 1000 }}>{follow ? "◎" : "▣"}<span style={{ display: "block", marginTop: 2, fontSize: 5.5 }}>{follow ? copy.follow : copy.overview}</span></button>
    </div>
