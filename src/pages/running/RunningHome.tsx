...
export default function RunningHome({ store, go }: Props) {
    ...
    <div style={{ width: "100%", maxWidth: PAGE_MAX_WIDTH }}>
      ...
      <div style={{ borderRadius: 28, padding: 18, marginBottom: 16, background: "linear-gradient(135deg, rgba(8,10,20,0.98), rgba(14,18,34,0.98))", border: `1px solid ${(theme as any).borderSoft ?? "rgba(255,255,255,0.10)"}`, boxShadow: "0 20px 40px rgba(0,0,0,0.7)", display: "flex", flexDirection: "column", alignItems: "center" }}>
        <div style={{ display: "inline-flex", padding: "5px 18px", borderRadius: 999, border: `1px solid ${accent}`, background: "linear-gradient(135deg, rgba(0,0,0,0.9), rgba(255,255,255,0.06))", marginBottom: 10 }}>
          <span style={{ fontSize: 13, fontWeight: 700, letterSpacing: 1.1, textTransform: "uppercase", color: accent }}>{copy.welcome}</span>
        </div>
        <div ref={wrapRef} style={{ width: "100%", overflow: "hidden" }}>
          <div ref={textRef} style={{ width: "fit-content", marginInline: "auto", fontSize: 32, fontWeight: 900, letterSpacing: 3, textAlign: "center", textTransform: "uppercase", whiteSpace: "nowrap", backgroundImage: `linear-gradient(120deg, ${accent}, #ffffff, ${accent})`, backgroundSize: "200% 100%", WebkitBackgroundClip: "text", color: "transparent", animation: "dcTitlePulse 3.6s ease-in-out infinite, dcTitleShimmer 7s linear infinite", transform: `scale(${scale})`, transformOrigin: "center" }}>{copy.title}</div>
        </div>
      </div>
    ...
    <div style={{ ...sectionWrap, marginTop: 11 }}><div className="card" style={{ padding: 13, borderColor: `${accent}38`, background: `linear-gradient(145deg,${accent}0d,rgba(255,255,255,.02))` }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8, marginBottom: 10 }}><SectionTitle text={pickLegacyLocalizedText(lang, "PERFORMANCE CENTER", "PERFORMANCE CENTER", "CENTRO DE RENDIMIENTO")} accent={accent}/><span style={{ fontSize: 8.5, color: textSoft }}>{pickLegacyLocalizedText(lang, "Synthèse de ta semaine", "Weekly snapshot", "Resumen semanal")}</span></div>
      <div style={{ display: "grid", gridTemplateColumns: "92px 1fr", gap: 12, alignItems: "center" }}><PerformanceRing value={performanceScore} accent={accent} label={pickLegacyBilingualText(lang, "SCORE", "SCORE")}/><div style={{ display: "grid", gridTemplateColumns: "repeat(2,minmax(0,1fr))", gap: 7 }}><PulseKpi label={pickLegacyLocalizedText(lang, "PRÉPARATION", "READINESS", "PREPARACIÓN")} value={`${trainingStatus.freshnessScore}%`} accent={trainingStatus.freshnessScore >= 70 ? "#71ff9a" : trainingStatus.freshnessScore >= 45 ? accent : "#ff8a67"}/><PulseKpi label={pickLegacyLocalizedText(lang, "RÉGULARITÉ", "CONSISTENCY", "REGULARIDAD")} value={`${consistencyScore}%`} accent={accent}/><PulseKpi label={pickLegacyLocalizedText(lang, "OBJECTIF SEMAINE", "WEEK GOAL", "OBJETIVO SEMANAL")} value={`${Math.round(goalPct)}%`} accent={accent}/><PulseKpi label={copy.streak} value={`${stats.activeWeekStreak} ${copy.weeks}`} accent={stats.activeWeekStreak ? "#71ff9a" : textSoft}/></div></div>
    </div></div>
...
{activitySport === "running" ? <>
    <div style={{ ...sectionWrap, marginTop: 12 }}><button type="button" onClick={() => go("running_plan", { focus: "goal" })} className="card" style={{ width: "100%", padding: 13, display: "grid", gridTemplateColumns: "54px 1fr auto", gap: 11, alignItems: "center", color: "inherit", textAlign: "left", cursor: "pointer", borderColor: `${raceGoalSnapshot ? accent : textSoft}35`, background: raceGoalSnapshot ? `linear-gradient(135deg,${accent}12,rgba(255,255,255,.02))` : "rgba(255,255,255,.02)" }}><div style={{ width: 52, height: 52, borderRadius: 15, display: "grid", placeItems: "center", fontSize: 25, background: `${accent}12`, border: `1px solid ${accent}32` }}>🏁</div><div><div style={{ fontSize: 9, color: accent, fontWeight: 1000, letterSpacing: .8 }}>{pickLegacyLocalizedText(lang, "OBJECTIF DE COURSE", "RACE GOAL", "OBJETIVO DE CARRERA")}</div>{raceGoalSnapshot ? <><div style={{ marginTop: 3, fontSize: 13, fontWeight: 1000 }}>{distanceGoalLabel(raceGoalSnapshot.goal.distanceM)} · J−{raceGoalSnapshot.daysLeft}</div><div style={{ marginTop: 3, fontSize: 9, color: textSoft }}>{pickLegacyLocalizedText(lang, "Cible", "Target", "Objetivo")} {formatDuration(raceGoalSnapshot.goal.targetTimeMs)} · {formatPace(raceGoalSnapshot.targetPaceSecPerKm)}/km{raceGoalSnapshot.predictedMs ? ` · ${pickLegacyLocalizedText(lang, "projection", "prediction", "predicción")} ${formatDuration(raceGoalSnapshot.predictedMs)}` : ""}</div></> : <><div style={{ marginTop: 3, fontSize: 12, fontWeight: 1000 }}>{pickLegacyLocalizedText(lang, "DONNE UNE DATE À TA PROGRESSION", "GIVE YOUR PROGRESS A DATE", "DA UNA FECHA A TU PROGRESO")}</div><div style={{ marginTop: 3, color: textSoft, fontSize: 9.5 }}>{pickLegacyLocalizedText(lang, "5K · 10K · Semi · Marathon · chrono cible", "5K · 10K · Half · Marathon · target time", "5K · 10K · Media · Maratón · tiempo objetivo")}</div></>}</div><div style={{ textAlign: "right" }}>{raceGoalSnapshot?.readinessPct != null ? <div style={{ color: accent, fontSize: 17, fontWeight: 1000 }}>{raceGoalSnapshot.readinessPct}%</div> : null}<div style={{ color: accent, fontSize: 18, fontWeight: 1000 }}>›</div></div></button></div>
</> : null}
    ...
    <div style={{ ...sectionWrap, marginTop: 12 }}><div className="card" style={{ padding: 14, borderColor: `${accent}34` }}>
      ...
      <div style={{ marginTop: 10, display: "flex", justifyContent: "space-between", gap: 8, alignItems: "center", padding: "9px 10px", borderRadius: 12, background: "rgba(255,255,255,.025)", border: "1px solid rgba(255,255,255,.06)" }}><span style={{ color: textSoft, fontSize: 9.5 }}>{pickLegacyLocalizedText(lang, "Équilibre charge récente / base", "Recent load / baseline balance", "Equilibrio carga reciente / base")}</span><b style={{ color: trainingStatus.loadLabel === "high" ? "#ff8a67" : trainingStatus.loadLabel === "balanced" ? "#71ff9a" : accent, fontSize: 9.5 }}>{trainingStatus.loadLabel === "high" ? copy.high : trainingStatus.loadLabel === "balanced" ? copy.balanced : copy.low}{trainingStatus.loadRatio != null ? ` · ${trainingStatus.loadRatio.toFixed(2)}×` : ""}</b></div>
    </div></div>
...
{activitySport === "running" ? <>
    <div style={{ ...sectionWrap, marginTop: 12 }}><div className="card" style={{ padding: 14, background: `linear-gradient(145deg,${accent}10,rgba(255,255,255,.02))`, borderColor: `${accent}38` }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}><SectionTitle text={copy.program} accent={accent}/><button className="btn" onClick={() => go("running_plan")} style={{ minHeight: 34, fontSize: 9, fontWeight: 1000 }}>{activePlan ? copy.open : copy.createProgram}</button></div>
      {activePlan ? <><div style={{ display: "grid", gridTemplateColumns: "1fr auto", gap: 10, alignItems: "center" }}><div><div style={{ fontWeight: 1000, fontSize: 15 }}>{activePlan.goal === "first-5k" ? "🌱 FIRST 5K" : activePlan.goal === "faster-5k" ? "⚡ FASTER 5K" : activePlan.goal === "10k" ? "🎯 10K" : "🛣️ HALF MARATHON"}</div><div style={{ color: textSoft, fontSize: 9.5, marginTop: 4 }}>{copy.weekGoal ? `${pickLegacyLocalizedText(lang, "Semaine", "Week", "Semana")} ${currentPlanWeek}/${planDurationWeeks(activePlan.goal)} · ${planCompletion}%` : ""}</div></div><div style={{ minWidth: 54, textAlign: "right", color: accent, fontWeight: 1000, fontSize: 20 }}>{planCompletion}%</div></div><Progress value={planCompletion} accent={accent}/>{nextWorkout ? <button type="button" onClick={() => go("running_plan")} style={{ width: "100%", marginTop: 10, border: "1px solid rgba(255,255,255,.07)", background: "rgba(255,255,255,.025)", color: "inherit", borderRadius: 13, padding: 10, display: "grid", gridTemplateColumns: "36px 1fr auto", gap: 9, alignItems: "center", textAlign: "left", cursor: "pointer" }}><span style={{ width: 34, height: 34, borderRadius: 10, display: "grid", placeItems: "center", background: `${accent}12` }}>▶</span><span><small style={{ color: textSoft, fontSize: 8.5 }}>{copy.nextWorkout}</small><b style={{ display: "block", marginTop: 2, fontSize: 10.5 }}>{nextWorkout.title}</b><small style={{ display: "block", marginTop: 2, color: textSoft, fontSize: 8.5 }}>{nextWorkout.subtitle}</small></span><b style={{ color: accent }}>›</b></button> : null}</> : <div style={{ padding: "8px 0 2px", color: textSoft, fontSize: 10, lineHeight: 1.45 }}>{copy.noProgram}</div>}
    </div></div>
</> : null}
    ...
    <div style={{ ...sectionWrap, marginTop: 14 }}><div className="card" style={{ padding: 14 }}><SectionTitle text={copy.start} accent={accent}/>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(2,minmax(0,1fr))", gap: 9 }}>
        <RunAction icon={OUTDOOR_SPORT_PROFILES[activitySport].icon} title={activitySport === "running" ? copy.free : outdoorSportLabel(activitySport, lang).toUpperCase()} subtitle={activitySport === "treadmill" ? (pickLegacyLocalizedText(lang, "Sans GPS · vitesse · inclinaison · splits", "No GPS · speed · incline · splits", "Sin GPS · velocidad · inclinación · splits")) : activitySport === "trail" ? (pickLegacyLocalizedText(lang, "GPS · relief · parcours · Ghost", "GPS · elevation · routes · Ghost", "GPS · relieve · rutas · Ghost")) : activitySport === "hiking" ? (pickLegacyLocalizedText(lang, "GPS · D+ · temps de mouvement · carte", "GPS · elevation · moving time · map", "GPS · D+ · tiempo en movimiento · mapa")) : copy.freeSub} accent={accent} onClick={() => go("games", presetParams("free", null, activitySport))} featured/>
        <RunAction icon="🌱" title={copy.easy} subtitle={copy.easySub} accent={accent} onClick={() => go("games", presetParams("easy", null, activitySport))}/>
        <RunAction icon={activitySport === "trail" || activitySport === "hiking" ? "⛰️" : "⚡"} title={activitySport === "running" || activitySport === "treadmill" ? copy.intervals : activitySport === "trail" || activitySport === "hiking" ? (pickLegacyLocalizedText(lang, "CÔTES", "HILLS", "CUESTAS")) : activitySport === "walking" ? (pickLegacyLocalizedText(lang, "MARCHE ACTIVE", "ACTIVE WALK", "CAMINATA ACTIVA")) : (pickLegacyLocalizedText(lang, "TECHNIQUE", "TECHNIQUE", "TÉCNICA"))} subtitle={activitySport === "running" ? copy.intervalsSub : activitySport === "treadmill" ? (pickLegacyLocalizedText(lang, "Vitesse · récupérations · sans GPS", "Speed · recoveries · no GPS", "Velocidad · recuperación · sin GPS")) : (pickLegacyLocalizedText(lang, "Relief · cadence · endurance", "Elevation · cadence · endurance", "Relieve · cadencia · resistencia"))} accent={accent} onClick={() => go("games", presetParams(activitySport === "walking" ? "easy" : activitySport === "nordic-walking" ? "tempo" : activitySport === "treadmill" ? "intervals" : "hills", null, activitySport))}/>
        <RunAction icon={activitySport === "running" ? "🎯" : "🛣️"} title={activitySport === "running" ? copy.five : (pickLegacyLocalizedText(lang, "SORTIE LONGUE", "LONG OUTING", "SALIDA LARGA"))} subtitle={activitySport === "running" ? copy.fiveSub : (pickLegacyLocalizedText(lang, "Endurance · distance · temps de mouvement", "Endurance · distance · moving time", "Resistencia · distancia · tiempo en movimiento"))} accent={accent} onClick={() => go("games", presetParams(activitySport === "running" ? "distance" : "long", activitySport === "running" ? 5000 : null, activitySport))}/>
      </div>
      {OUTDOOR_SPORT_PROFILES[activitySport].supportsPacer ? <button type="button" onClick={() => go("games", presetParams("pacer", 5000, activitySport))} style={{ width: "100%", marginTop: 9, display: "grid", gridTemplateColumns: "42px 1fr auto", gap: 10, alignItems: "center", borderRadius: 14, border: `1px solid ${accent}48`, background: `linear-gradient(135deg,${accent}16,rgba(255,255,255,.025))`, color: "#fff", padding: "11px 12px", textAlign: "left", cursor: "pointer" }}><span style={{ fontSize: 23 }}>⏱️</span><span><b style={{ fontSize: 12 }}>{copy.pacer}</b><small style={{ display: "block", color: textSoft, marginTop: 3, lineHeight: 1.3 }}>{copy.pacerSub}</small></span><b style={{ color: accent }}>›</b></button> : null}
    </div></div>
    ...
    <div style={{ ...sectionWrap, marginTop: 12 }}><button type="button" onClick={() => go("stats")} className="card" style={{ width: "100%", display: "grid", gridTemplateColumns: "48px 1fr auto", gap: 11, alignItems: "center", padding: 13, color: "inherit", textAlign: "left", cursor: "pointer", borderColor: `${accent}32`, background: `linear-gradient(145deg,${accent}0c,rgba(255,255,255,.02))` }}><div style={{ width: 46, height: 46, borderRadius: 14, display: "grid", placeItems: "center", background: `${accent}14`, border: `1px solid ${accent}34`, fontSize: 22 }}>📊</div><div><div style={{ color: accent, fontSize: 9, fontWeight: 1000, letterSpacing: .8 }}>{pickLegacyLocalizedText(lang, "ANALYSES & STATISTIQUES", "ANALYSIS & STATS", "ANÁLISIS Y ESTADÍSTICAS")}</div><div style={{ marginTop: 3, fontSize: 10, color: textSoft, lineHeight: 1.35 }}>{pickLegacyLocalizedText(lang, "Courbes, zones d’allure, prédictions, records, segments et historique sont regroupés dans Stats.", "Charts, pace zones, predictions, records, segments and history are grouped in Stats.", "Gráficas, zonas de ritmo, predicciones, récords, segmentos e historial están agrupados en Stats.")}</div></div><b style={{ color: accent, fontSize: 18 }}>›</b></button></div>

    {activities.length ? <div style={{ ...sectionWrap, marginTop: 12 }}><div className="card" style={{ padding: 14 }}><div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}><SectionTitle text={copy.recent} accent={accent}/><button className="btn" style={{ minHeight: 34, fontSize: 9.5 }} onClick={() => go("stats", { runningStatsTab: "history" })}>{copy.allRuns}</button></div><div style={{ display: "grid", gap: 7, marginTop: 8 }}>{activities.slice(0, 3).map((a) => <RecentRun key={a.id} activity={a} accent={accent} textSoft={textSoft}/>)}</div></div></div> : null}

    <div style={{ ...sectionWrap, marginTop: 12 }}><div className="card" style={{ display: "grid", gridTemplateColumns: "48px 1fr auto", gap: 12, alignItems: "center", padding: 13, opacity: .86 }}><div style={{ width: 46, height: 46, borderRadius: 14, display: "grid", placeItems: "center", background: `${accent}16`, border: `1px solid ${accent}38`, fontSize: 22 }}>⌚</div><div><div style={{ fontWeight: 1000, fontSize: 11 }}>{copy.devices}</div><div style={{ fontSize: 10, color: textSoft, marginTop: 3, lineHeight: 1.35 }}>{copy.devicesSub}</div></div><button className="btn" onClick={() => go("stats")} style={{ minHeight: 32, fontSize: 8, fontWeight: 1000, color: accent, borderColor: `${accent}55` }}>{pickLegacyLocalizedText(lang, "OUVRIR", "OPEN", "ABRIR")}</button></div></div>
    </div>
  </div>;
}
