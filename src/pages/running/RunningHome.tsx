...
export default function RunningHome({ store, go }: Props) {
    ...
    const recommendation = useMemo(() => {
        if (activitySport === "trail") return stats.weekSessions >= 2 ? { id: "long", icon: "⛰️", title: lang === "fr" ? "SORTIE TRAIL LONGUE" : lang === "es" ? "TRAIL LARGO" : "LONG TRAIL", text: lang === "fr" ? "Endurance et D+ sur terrain naturel." : lang === "es" ? "Resistencia y desnivel en terreno natural." : "Endurance and elevation on natural terrain." } : { id: "hills", icon: "⛰️", title: lang === "fr" ? "CÔTES / TRAIL" : lang === "es" ? "CUESTAS / TRAIL" : "HILLS / TRAIL", text: lang === "fr" ? "Travaille la montée, la relance et la technique." : lang === "es" ? "Trabaja subida, reactividad y técnica." : "Build climbing, turnover and trail technique." };
        if (activitySport === "hiking") return { id: stats.weekSessions ? "long" : "easy", icon: "🥾", title: lang === "fr" ? "RANDONNÉE ENDURANCE" : lang === "es" ? "SENDERISMO DE RESISTENCIA" : "ENDURANCE HIKE", text: lang === "fr" ? "Privilégie le temps de mouvement et le dénivelé." : lang === "es" ? "Prioriza tiempo en movimiento y desnivel." : "Focus on moving time and elevation." };
        if (activitySport === "walking") return { id: "easy", icon: "🚶", title: lang === "fr" ? "MARCHE ACTIVE" : lang === "es" ? "CAMINATA ACTIVA" : "ACTIVE WALK", text: lang === "fr" ? "30 à 45 min régulières en aisance." : lang === "es" ? "30 a 45 min regulares y cómodos." : "30–45 steady comfortable minutes." };
        if (activitySport === "nordic-walking") return { id: stats.weekSessions >= 2 ? "long" : "tempo", icon: "🥢", title: lang === "fr" ? "MARCHE NORDIQUE" : lang === "es" ? "MARCHA NÓRDICA" : "NORDIC WALK", text: lang === "fr" ? "Cadence, coordination et endurance active." : lang === "es" ? "Cadencia, coordinación y resistencia activa." : "Cadence, coordination and active endurance." };
        if (activitySport === "treadmill") return stats.weekSessions >= 2 ? { id: "intervals", icon: "⚡", title: lang === "fr" ? "INTERVALLES TAPIS" : lang === "es" ? "INTERVALOS EN CINTA" : "TREADMILL INTERVALS", text: lang === "fr" ? "Alterne vitesse et récupération sans dépendre du GPS." : lang === "es" ? "Alterna velocidad y recuperación sin GPS." : "Alternate speed and recovery without GPS." } : { id: "easy", icon: "🏃‍♂️", title: lang === "fr" ? "ENDURANCE TAPIS" : lang === "es" ? "RESISTENCIA EN CINTA" : "TREADMILL ENDURANCE", text: lang === "fr" ? "30 à 45 min régulières, mesure FTMS/footpod ou vitesse manuelle." : lang === "es" ? "30–45 min regulares con FTMS, footpod o velocidad manual." : "30–45 steady minutes using FTMS, footpod or manual speed." };
        if (!stats.sessions)
...
{activitySport === "running" ? <>
    ...
    <div style={{ ...sectionWrap, marginTop: 14 }}><div className="card" style={{ padding: 14 }}><SectionTitle text={copy.start} accent={accent}/>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(2,minmax(0,1fr))", gap: 9 }}>
        <RunAction icon={OUTDOOR_SPORT_PROFILES[activitySport].icon} title={activitySport === "running" ? copy.free : outdoorSportLabel(activitySport, lang).toUpperCase()} subtitle={activitySport === "treadmill" ? (lang === "fr" ? "Sans GPS · vitesse · inclinaison · splits" : lang === "es" ? "Sin GPS · velocidad · inclinación · splits" : "No GPS · speed · incline · splits") : activitySport === "trail" ? (lang === "fr" ? "GPS · relief · parcours · Ghost" : lang === "es" ? "GPS · relieve · rutas · Ghost" : "GPS · elevation · routes · Ghost") : activitySport === "hiking" ? (lang === "fr" ? "GPS · D+ · temps de mouvement · carte" : lang === "es" ? "GPS · D+ · tiempo en movimiento · mapa" : "GPS · elevation · moving time · map") : copy.freeSub} accent={accent} onClick={() => go("games", presetParams("free", null, activitySport))} featured/>
        <RunAction icon="🌱" title={copy.easy} subtitle={copy.easySub} accent={accent} onClick={() => go("games", presetParams("easy", null, activitySport))}/>
        <RunAction icon={activitySport === "trail" || activitySport === "hiking" ? "⛰️" : "⚡"} title={activitySport === "running" || activitySport === "treadmill" ? copy.intervals : activitySport === "trail" || activitySport === "hiking" ? (lang === "fr" ? "CÔTES" : lang === "es" ? "CUESTAS" : "HILLS") : activitySport === "walking" ? (lang === "fr" ? "MARCHE ACTIVE" : lang === "es" ? "CAMINATA ACTIVA" : "ACTIVE WALK") : (lang === "fr" ? "TECHNIQUE" : lang === "es" ? "TÉCNICA" : "TECHNIQUE")} subtitle={activitySport === "running" ? copy.intervalsSub : activitySport === "treadmill" ? (lang === "fr" ? "Vitesse · récupérations · sans GPS" : lang === "es" ? "Velocidad · recuperación · sin GPS" : "Speed · recoveries · no GPS") : (lang === "fr" ? "Relief · cadence · endurance" : lang === "es" ? "Relieve · cadencia · resistencia" : "Elevation · cadence · endurance")} accent={accent} onClick={() => go("games", presetParams(activitySport === "walking" ? "easy" : activitySport === "nordic-walking" ? "tempo" : activitySport === "treadmill" ? "intervals" : "hills", null, activitySport))}/>
        <RunAction icon={activitySport === "running" ? "🎯" : "🛣️"} title={activitySport === "running" ? copy.five : (lang === "fr" ? "SORTIE LONGUE" : lang === "es" ? "SALIDA LARGA" : "LONG OUTING")} subtitle={activitySport === "running" ? copy.fiveSub : (lang === "fr" ? "Endurance · distance · temps de mouvement" : lang === "es" ? "Resistencia · distancia · tiempo en movimiento" : "Endurance · distance · moving time")} accent={accent} onClick={() => go("games", presetParams(activitySport === "running" ? "distance" : "long", activitySport === "running" ? 5000 : null, activitySport))}/>
      </div>
