...
export default function RunningTerrainStatsPanel({ activities, lang, accent, textSoft }: Props) {
  ...
  const copy = lang === "fr" ? { title: "RELIEF & CÔTES", no: "Aucune donnée d’altitude exploitable pour le moment.", route: "SORTIE ANALYSÉE", difficulty: "DIFFICULTÉ", gain: "D+", loss: "D−", maxGrade: "PENTE MAX", hills: "CÔTES", records: "RECORDS EN MONTÉE", biggest: "PLUS GROSSE MONTÉE", steepest: "PLUS RAIDE", vertical: "VITESSE ASCENSIONNELLE", pace: "ALLURE ↔ RELIEF", paceCell: "ALLURE", km: "KM", alt: "Δ ALT.", climb: "D+ KM" }
    : lang === "es" ? { title: "DESNIVEL Y CUESTAS", no: "Todavía no hay datos de altitud utilizables.", route: "CARRERA ANALIZADA", difficulty: "DIFICULTAD", gain: "D+", loss: "D−", maxGrade: "PENDIENTE MÁX.", hills: "CUESTAS", records: "RÉCORDS EN SUBIDA", biggest: "MAYOR SUBIDA", steepest: "MÁS EMPINADA", vertical: "VELOCIDAD ASCENSIONAL", pace: "RITMO ↔ DESNIVEL", paceCell: "RITMO", km: "KM", alt: "Δ ALT.", climb: "D+ KM" }
    : { title: "ELEVATION & HILLS", no: "No usable elevation data yet.", route: "ANALYSED RUN", difficulty: "DIFFICULTY", gain: "ELEV +", loss: "ELEV −", maxGrade: "MAX GRADE", hills: "HILLS", records: "HILL RECORDS", biggest: "BIGGEST CLIMB", steepest: "STEEPEST", vertical: "VERTICAL SPEED", pace: "PACE ↔ ELEVATION", paceCell: "PACE", km: "KM", alt: "Δ ELEV.", climb: "KM CLIMB" };
  ...
  return <div style={{ marginTop: 12 }}><Section title={copy.title}>
    ...
    <div className="running-metrics-4" style={{ marginTop: 8 }}><Mini label={copy.gain} value={`+${Math.round(analysis.gainM)} m`} accent={accent}/><Mini label={copy.loss} value={`−${Math.round(analysis.lossM)} m`} accent={accent}/><Mini label={copy.maxGrade} value={`${analysis.maxGradePct.toFixed(1)}%`} accent={accent}/><Mini label={copy.hills} value={String(analysis.hills.length)} accent={accent}/></div>

    {(records.biggestGain || records.steepest || records.fastestVertical) ? <div style={{ marginTop: 11 }}><div style={titleStyle(accent)}>{copy.records}</div><div className="running-records-3"><RecordCard label={copy.biggest} value={records.biggestGain ? `+${Math.round(records.biggestGain.gainM)} m` : "—"} sub={records.biggestGain ? `${formatDistance(records.biggestGain.distanceM)} · ${records.biggestGain.avgGradePct.toFixed(1)}%` : ""} accent={accent}/><RecordCard label={copy.steepest} value={records.steepest ? `${records.steepest.avgGradePct.toFixed(1)}%` : "—"} sub={records.steepest?.elapsedMs ? formatDuration(records.steepest.elapsedMs) : ""} accent={accent}/><RecordCard label={copy.vertical} value={records.fastestVertical?.verticalSpeedMph ? `${Math.round(records.fastestVertical.verticalSpeedMph)} m/h` : "—"} sub={records.fastestVertical?.elapsedMs ? formatDuration(records.fastestVertical.elapsedMs) : ""} accent={accent}/></div></div> : null}

    {rows.length ? <div style={{ marginTop: 11 }}><div style={titleStyle(accent)}>{copy.pace}</div><div className="running-terrain-rows">{rows.slice(0, 16).map((row) => <div key={row.km} className="running-terrain-row"><TerrainCell label={copy.km} value={String(row.km)} accent={accent}/><TerrainCell label={copy.paceCell} value={`${formatPace(row.paceSecPerKm)}/km`} accent={accent}/><TerrainCell label={copy.alt} value={`${row.elevationDeltaM >= 0 ? "+" : ""}${Math.round(row.elevationDeltaM)} m`} accent={accent}/><TerrainCell label={copy.climb} value={`+${Math.round(row.gainM)} m`} accent={accent}/></div>)}</div></div> : null}
  </Section></div>;
}

function TerrainCell({ label, value, accent }: { label: string; value: string; accent: string }) { return <div className="running-terrain-cell"><div style={{ fontSize: 6.4, opacity: .48, fontWeight: 1000 }}>{label}</div><div style={{ marginTop: 2, fontSize: 8.8, color: accent, fontWeight: 1000, whiteSpace: "nowrap" }}>{value}</div></div>; }
