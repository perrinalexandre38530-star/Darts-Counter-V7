...
export default function RunningModule({ go, params }: Props) {
    ...
    <div style={{ display: setupPanel === "route" ? "block" : "none" }}>
      ...
      {routePanelTab === "choose" ? <>
        ...
        {routeOptions.length ? <div style={{ marginTop: 4 }}><Section title={pickLegacyLocalizedText(lang, "CHOISIR UN PARCOURS", "CHOOSE A ROUTE", "ELEGIR UNA RUTA")} right={<div style={{ display: "flex", gap: 6, alignItems: "center" }}><button className="btn" onClick={() => setRouteListOpen((value) => !value)} style={{ minHeight: 30, padding: "4px 8px", fontSize: 8.3, fontWeight: 1000 }}>{routeListOpen ? pickLegacyLocalizedText(lang, "REPLIER", "COLLAPSE", "PLEGAR") : pickLegacyLocalizedText(lang, "DÉPLIER", "EXPAND", "DESPLEGAR")}</button>{selectedRoute ? <button className="btn" disabled={!selectedRouteHasReference} onClick={() => selectedRouteHasReference && setGhostEnabled((value) => !value)} style={{ minHeight: 30, padding: "4px 8px", fontSize: 8.5, fontWeight: 1000, opacity: selectedRouteHasReference ? 1 : .45, color: ghostEnabled ? accent : undefined, borderColor: ghostEnabled ? `${accent}77` : undefined }}>{selectedRouteHasReference ? `GHOST ${ghostEnabled ? "ON" : "OFF"}` : pickLegacyLocalizedText(lang, "PARCOURS SEUL", "ROUTE ONLY", "SOLO RUTA")}</button> : null}</div>}>
          <RunningSurface accent={accent} style={{ marginTop: 2 }}>
            <div style={{ display: "grid", gap: 8 }}>
              <div style={{ display: "grid", gridTemplateColumns: "1fr auto", gap: 8, alignItems: "center" }}>
                <div><div style={{ color: accent, fontSize: 9.1, fontWeight: 1000, letterSpacing: .5 }}>{pickLegacyLocalizedText(lang, "PARCOURS TROUVÉS", "ROUTES FOUND", "RUTAS ENCONTRADAS")}</div><div style={{ marginTop: 3, color: textSoft, fontSize: 8.1 }}>{routeOptions.length} {pickLegacyLocalizedText(lang, "parcours disponibles", "routes available", "rutas disponibles")}</div></div>
                <div style={{ padding: "5px 9px", borderRadius: 999, border: `1px solid ${accent}33`, color: accent, fontSize: 8.1, fontWeight: 1000 }}>{routeOptions.length}</div>
              </div>

              {routeListOpen ? <div style={{ display: "grid", gap: 7 }}>{routeOptions.slice(0, 12).map((route, index) => {
                ...
                const routeName = buildRouteDisplayName(route, routeTerrain, lang);
              ...
              {selectedRoute ? <div style={{ marginTop: 2, touchAction: "pan-y" }} onTouchStart={onRouteSwipeStart} onTouchEnd={onRouteSwipeEnd}>
                ...
                {routeDetailsTab === 'photos' ? <><OutdoorRoutePlaceInfoPanel route={selectedRoute} lang={lang} accent={accent} textSoft={textSoft} compact/><OutdoorRoutePhotoGallery route={selectedRoute} lang={lang} accent={accent} textSoft={textSoft} onSearchImages={() => searchRouteImages(selectedRoute)}/><button className="btn" onClick={() => openRouteInMaps(selectedRoute)} style={{ width: '100%', minHeight: 38, marginTop: 7, fontSize: 8.1, fontWeight: 1000, color: accent, borderColor: `${accent}55` }}>🗺️ {pickLegacyLocalizedText(lang, 'OUVRIR LE DÉPART DANS MAPS', 'OPEN START IN MAPS', 'ABRIR SALIDA EN MAPS')}</button></> : null}
