...
const PRESETS: Preset[] = [
    { id: "free", type: "free", icon: "🏃", fr: "SORTIE LIBRE", en: "FREE ACTIVITY", es: "SALIDA LIBRE", subFr: "Pars sans contrainte. GPS, carte, splits et tours.", subEn: "Start without constraints. GPS, map, splits and laps.", subEs: "Sal sin límites. GPS, mapa, splits y vueltas.", targetDistanceM: null },
...
export default function RunningModule({ go, params }: Props) {
    ...
    const initialView: View = params?.runningView === "detail" ? "detail" : params?.runningView === "history" ? "history" : params?.runningView === "records" ? "records" : params?.runningView === "plan" ? "plan" : params?.runningView === "goal" ? "goal" : "setup";
    ...
    const [routeDetailOpen, setRouteDetailOpen] = React.useState(false);
    const [routeDetailInitialTab, setRouteDetailInitialTab] = React.useState<OutdoorRouteDetailTab>("overview");
    ...
    const isOutdoorAdventureSport = ["trail", "hiking", "walking"].includes(activitySport);
    ...
    React.useEffect(() => {
        const requestedActivityId = String(params?.runningActivityId || "").trim();
        if (!requestedActivityId) return;
        let cancelled = false;
        void getActivity(requestedActivityId).then((activity) => {
            if (cancelled || !activity) return;
            setActivitySport(canonicalOutdoorPerformanceSport(activity.sport));
            setSelected(activity);
            setFinishBadges([]);
            setDetailMapFullscreen(false);
            setView("detail");
        });
        return () => { cancelled = true; };
    }, [params?.runningActivityId]);
    React.useEffect(() => {
        ...
        setActivitySport(canonicalOutdoorPerformanceSport(session.sport));
    ...
    React.useEffect(() => {
        ...
        const timer = window.setInterval(() => { void persistCurrentDraft(); }, 3000);
    ...
    React.useEffect(() => addNativeTrackingListener((snapshot) => {
        ...
        if (!shouldAcceptRunningPoint(previous, point, gpsMaxSpeedMpsForSport(activitySport))) return;
    ...
    const finishRun = React.useCallback(async () => {
        ...
        if (nativeTrackingActiveRef.current && activitySport !== "treadmill") {
            ...
            routeForSave = filterRouteOutliers(routeForSave, gpsMaxSpeedMpsForSport(activitySport));
        ...
        const route = activitySport === "treadmill" ? [] : filterRouteOutliers(routeForSave, gpsMaxSpeedMpsForSport(activitySport));
    ...
    const removeSelected = React.useCallback(async () => {
        if (!selected) return;
        const confirmed = typeof window === "undefined" || window.confirm(pickLegacyLocalizedText(lang, "Supprimer définitivement cette sortie et son parcours GPS ?", "Permanently delete this activity and its GPS route?", "¿Eliminar definitivamente esta salida y su ruta GPS?"));
        if (!confirmed) return;
        ...
        await refreshActivities();
        setView("history");
    }, [lang, refreshActivities, selected]);
    ...
    const publishSelectedRouteOnline = React.useCallback(async () => {
        if (!selected || selected.indoor || (selected.route || []).length < 2) return;
        setRoutePublishMessage(pickLegacyLocalizedText(lang, "PUBLICATION…", "PUBLISHING…", "PUBLICANDO…"));
        const result = await publishOutdoorActivityRoute({ ...selected, visibility: "public" }, selected.notes || "");
        if (!result.ok) {
            setRoutePublishMessage(pickLegacyLocalizedText(lang, "Publication indisponible. Vérifie ta connexion et que la migration Running V3 est installée.", "Publishing unavailable. Check your connection and that Running V3 migration is installed.", "Publicación no disponible. Comprueba la conexión y la migración Running V3."));
            return;
        }
        await updateSelected({ visibility: "public", publishedRouteId: result.id, publishedAt: Date.now() });
        setRoutePublishMessage(pickLegacyLocalizedText(lang, "✓ Parcours publié dans la communauté.", "✓ Route published to the community.", "✓ Ruta publicada en la comunidad."));
    }, [lang, selected, updateSelected]);
    const unpublishSelectedRouteOnline = React.useCallback(async () => {
        if (!selected) return;
        const result = await unpublishOutdoorActivityRoute(selected.id);
        if (!result.ok) {
            setRoutePublishMessage(pickLegacyLocalizedText(lang, "Impossible de retirer le parcours pour le moment.", "Unable to unpublish the route right now.", "No se puede retirar la ruta ahora."));
            return;
        }
        await updateSelected({ publishedRouteId: undefined, publishedAt: undefined });
        setRoutePublishMessage(pickLegacyLocalizedText(lang, "Parcours retiré de la bibliothèque publique.", "Route removed from the public library.", "Ruta retirada de la biblioteca pública."));
    }, [lang, selected, updateSelected]);
    ...
    if (routeDetailOpen && selectedRoute) {
        const favorite = savedRoutes.some((item) => item.id === selectedRoute.id || (!!selectedRoute.externalId && item.externalId === selectedRoute.externalId) || (!!selectedRoute.sourceActivityId && item.sourceActivityId === selectedRoute.sourceActivityId));
        return <OutdoorRouteDetailPage
          route={selectedRoute}
          sport={activitySport}
          lang={lang}
          accent={accent}
          textSoft={textSoft}
          favorite={favorite}
          localAttempts={selectedRouteAttempts}
          initialTab={routeDetailInitialTab}
          onBack={() => setRouteDetailOpen(false)}
          onClose={() => setRouteDetailOpen(false)}
          onGuide={() => { setRouteDetailOpen(false); setSetupPanel("route"); setRoutePanelTab("guide"); }}
          onToggleFavorite={() => toggleFavoriteRoute(selectedRoute)}
          onOpenMaps={() => openRouteInMaps(selectedRoute)}
          onSearchImages={() => searchRouteImages(selectedRoute)}
          onOfflineChanged={() => void refreshOfflineRoutes()}
        />;
    }
    if (countdown != null)
        return <div className="container running-page" style={{ maxWidth: PAGE_MAX_WIDTH, minHeight: "78vh", display: "grid", placeItems: "center" }}><div style={{ textAlign: "center" }}><div style={{ fontSize: 12, color: textSoft, fontWeight: 1000, letterSpacing: 2 }}>{copy.countdown}</div><div style={{ marginTop: 8, fontSize: "clamp(88px,28vw,150px)", lineHeight: 1, fontWeight: 1000, color: accent, textShadow: `0 0 34px ${accent}77` }}>{countdown === 0 ? copy.go : countdown}</div><div style={{ marginTop: 14, fontWeight: 900 }}>{presetLabel(effectivePreset, lang)}</div></div></div>;
    if (view === "record") {
      ...
      {livePage === "details" ? <>
        <RunningSurface accent={accent} active><div style={{ display: "grid", gridTemplateColumns: "repeat(2,minmax(0,1fr))", gap: 8 }}>{outdoorUsesSpeedMetric(activitySport) ? <><MiniStat label={outdoorAverageMetricLabel(activitySport, lang)} value={`${liveSpeed.toFixed(1)} km/h`} accent={accent}/><MiniStat label={pickLegacyLocalizedText(lang, "VITESSE LIVE", "LIVE SPEED", "VELOCIDAD LIVE")} value={rollingSpeedKmh > 0 ? `${rollingSpeedKmh.toFixed(1)} km/h` : "—"} accent={accent}/></> : <><MiniStat label={copy.avgPace} value={`${formatPace(livePace)}/km`} accent={accent}/><MiniStat label={copy.livePace} value={`${formatPace(rollingPace)}/km`} accent={accent}/></>}<MiniStat label={pickLegacyLocalizedText(lang, "ALLURE (MIN/KM)", "PACE (MIN/KM)", "RITMO (MIN/KM)")} value={`${formatPace(livePace)}/km`} accent={accent}/><MiniStat label={copy.elevation} value={`+${Math.round(liveElevation)} m`} accent={accent}/><MiniStat label={pickLegacyLocalizedText(lang, "CARDIO", "HEART RATE", "CARDIO")} value={liveHr} accent={accent}/><MiniStat label={pickLegacyLocalizedText(lang, "CADENCE", "CADENCE", "CADENCIA")} value={liveCadence} accent={accent}/></div></RunningSurface>
    ...
    if (view === "history") {
        ...
        return <div className="container running-page" style={{ maxWidth: PAGE_MAX_WIDTH }}><PageHeader title={copy.history} subtitle={`${outdoorSportLabel(activitySport, lang)} · ${activities.length} ${pickLegacyLocalizedText(lang, "sorties", "activities", "salidas")}`} left={<BackDot onClick={() => go("stats")}/>} right={infoDot}/>
      <div className="running-metrics-4" style={{ marginTop: 10 }}>{(["all", "free", "training", "pacer"] as const).map((key, i) => <button key={key} className="btn" onClick={() => setHistoryFilter(key)} style={{ minHeight: 34, padding: "5px 4px", fontSize: 8.5, fontWeight: 1000, borderColor: historyFilter === key ? `${accent}77` : undefined, color: historyFilter === key ? accent : undefined }}>{copy.filters[i]}</button>)}</div>
    ...
    if (view === "plan") {
        return <div className="container running-page" style={{ maxWidth: PAGE_MAX_WIDTH }}><PageHeader title={copy.plan} subtitle={pickLegacyLocalizedText(lang, "Construis ta progression semaine après semaine", "Build progress week by week", "Construye tu progresión semana a semana")} left={<BackDot onClick={() => go("running_plan")}/>} right={infoDot}/><div style={{ marginTop: 10 }}><RunningPlanView activities={activities} lang={lang} accent={accent} textSoft={textSoft} onStart={startPlanSession}/></div></div>;
    }
    if (view === "goal") {
        return <div className="container running-page" style={{ maxWidth: PAGE_MAX_WIDTH }}><PageHeader title={pickLegacyLocalizedText(lang, "OBJECTIF DE COURSE", "RACE GOAL", "OBJETIVO DE CARRERA")} subtitle={pickLegacyLocalizedText(lang, "Date · chrono cible · allure · prédiction", "Date · target time · pace · prediction", "Fecha · tiempo objetivo · ritmo · predicción")} left={<BackDot onClick={() => go("running_plan")}/>} right={infoDot}/><div style={{ marginTop: 10 }}><RunningGoalView stats={stats} lang={lang} accent={accent} textSoft={textSoft}/></div></div>;
    }
    ...
    if (view === "records") {
        ...
        return <div className="container running-page" style={{ maxWidth: PAGE_MAX_WIDTH }}><PageHeader title={copy.records} subtitle={copy.bestEfforts} left={<BackDot onClick={() => go("stats")}/>} right={infoDot}/>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3,minmax(0,1fr))", gap: 8, margin: "12px 0" }}><MiniStat label={copy.distance} value={formatDistance(stats.totalDistanceM)} accent={accent}/><MiniStat label={copy.longestLabel} value={formatDistance(stats.longestM)} accent={accent}/><MiniStat label={outdoorUsesSpeedMetric(activitySport) ? pickLegacyLocalizedText(lang, "MEILLEURE VITESSE", "BEST SPEED", "MEJOR VELOCIDAD") : copy.avgPace} value={outdoorUsesSpeedMetric(activitySport) ? (activities.length ? `${Math.max(...activities.map((activity) => Number(activity.avgSpeedMps || 0) * 3.6)).toFixed(1)} km/h` : "—") : `${formatPace(stats.bestPaceSecPerKm)}/km`} accent={accent}/></div>
    ...
    <div style={{ display: "grid", gridTemplateColumns: `repeat(${(sportProfile.supportsPacer || sportProfile.supportsIntervals) ? 3 : trainingPresetIds.length ? 2 : 1},minmax(0,1fr))`, gap: 6, marginTop: 10 }}>
      <button className="btn" onClick={() => { setSetupTab("goal"); selectManualPreset(`goal-${simpleGoalMode}`); }} style={{ minHeight: 42, fontSize: 9.2, fontWeight: 1000, color: setupTab === "goal" ? accent : undefined, borderColor: setupTab === "goal" ? `${accent}66` : undefined }}>{pickLegacyLocalizedText(lang, "OBJECTIF", "GOAL", "OBJETIVO")}</button>
      {trainingPresetIds.length ? <button className="btn" onClick={() => setSetupTab("training")} style={{ minHeight: 42, fontSize: 9.2, fontWeight: 1000, color: setupTab === "training" ? accent : undefined, borderColor: setupTab === "training" ? `${accent}66` : undefined }}>{pickLegacyLocalizedText(lang, "SÉANCES", "WORKOUTS", "SESIONES")}</button> : null}
      {(sportProfile.supportsPacer || sportProfile.supportsIntervals) ? <button className="btn" onClick={() => setSetupTab("advanced")} style={{ minHeight: 42, fontSize: 9.2, fontWeight: 1000, color: setupTab === "advanced" ? accent : undefined, borderColor: setupTab === "advanced" ? `${accent}66` : undefined }}>{pickLegacyLocalizedText(lang, "AVANCÉ", "ADVANCED", "AVANZADO")}</button> : null}
    </div>
    ...
    {setupTab === "training" && trainingPresetIds.length ? <div style={{ marginTop: 10 }}>
      <Section title={pickLegacyLocalizedText(lang, activitySport === "trail" ? "SÉANCES TRAIL" : activitySport === "walking" ? "SÉANCES MARCHE" : activitySport === "hiking" ? "SÉANCES RANDONNÉE" : activitySport === "treadmill" ? "SÉANCES TAPIS" : "SÉANCES RUNNING", "WORKOUTS", "SESIONES")}>
    ...
    <div style={{ display: setupPanel === "route" ? "block" : "none" }}>
      ...
      {routePanelTab === "choose" ? <>
        <div style={{ display: "grid", gap: 10, marginTop: 8 }}>
          <RunningSurface accent={accent} active style={{ padding: 8 }}>
          ...
          {routeChooseMode === "scout" && activitySport !== "treadmill" ? <OutdoorRouteScoutDiscover
            routes={routeOptions.filter((route) => !!route.scout)}
            ...
            textSoft={textSoft}
            busy={routeScoutBusy}
            message={routeScoutMessage}
            radiusKm={routeScoutRadiusKm}
            targetDistanceKm={routeGenerationDistanceKm}
            onRadiusChange={setRouteScoutRadiusKm}
            onTargetDistanceChange={setRouteGenerationDistanceKm}
            onSearch={() => void scoutExistingRoutes()}
            onSelect={(route) => selectRoute(route)}
            onOpenDetails={(route) => { selectRoute(route); setRouteDetailInitialTab("overview"); setRouteDetailOpen(true); }}
            onGuide={(route) => { selectRoute(route); setRoutePanelTab("guide"); }}
          ...
          {routeOptions.length && routeChooseMode !== "scout" ? <RunningSurface accent={accent} active style={{ overflow: "hidden" }}>
            <div style={{ display: "grid", gap: 10 }}>
              <div style={{ display: "grid", gridTemplateColumns: "1fr auto", gap: 9, alignItems: "center" }}>
                <div style={{ minWidth: 0 }}>
                  <div style={{ color: accent, fontSize: 9.3, fontWeight: 1000, letterSpacing: .7 }}>{pickLegacyLocalizedText(lang, "TON PARCOURS", "YOUR ROUTE", "TU RUTA")}</div>
                  <div style={{ marginTop: 2, color: textSoft, fontSize: 7.3 }}>{selectedRoute ? buildRouteDisplayName(selectedRoute, selectedTerrain, lang) : pickLegacyLocalizedText(lang, "Sélectionne un tracé", "Select a route", "Selecciona una ruta")}</div>
                </div>
                <div style={{ display: "flex", gap: 5, alignItems: "center" }}>
                  <button className="btn" title={pickLegacyLocalizedText(lang, "Comparer les parcours", "Compare routes", "Comparar rutas")} onClick={() => setRouteListOpen((value) => !value)} style={{ minWidth: 34, minHeight: 34, padding: 0, fontSize: 12, color: routeListOpen ? accent : undefined, borderColor: routeListOpen ? `${accent}55` : undefined }}>≡</button>
                  {selectedRoute && selectedRouteHasReference ? <button className="btn" title="Ghost" onClick={() => setGhostEnabled((value) => !value)} style={{ minWidth: 34, minHeight: 34, padding: 0, fontSize: 9, fontWeight: 1000, color: ghostEnabled ? accent : undefined, borderColor: ghostEnabled ? `${accent}55` : undefined }}>G</button> : null}
                </div>
              </div>
              ...
              {selectedRoute ? <div style={{ display: "grid", gap: 10 }}>
                  ...
                  <div style={{ position: "absolute", inset: 10, display: "grid", gridTemplateRows: "auto 1fr", pointerEvents: "none" }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 8 }}>
                      ...
                      <div style={{ display: "grid", gap: 6, pointerEvents: "auto" }}>
                        <button className="btn" onClick={(event) => { event.stopPropagation(); toggleFavoriteRoute(selectedRoute); }} style={{ minWidth: 36, minHeight: 36, padding: 0, background: "rgba(7,10,15,.82)", borderColor: `${accent}35`, color: savedRoutes.some((item) => item.id === selectedRoute.id || (!!selectedRoute.externalId && item.externalId === selectedRoute.externalId) || (!!selectedRoute.sourceActivityId && item.sourceActivityId === selectedRoute.sourceActivityId)) ? accent : undefined }}>{savedRoutes.some((item) => item.id === selectedRoute.id || (!!selectedRoute.externalId && item.externalId === selectedRoute.externalId) || (!!selectedRoute.sourceActivityId && item.sourceActivityId === selectedRoute.sourceActivityId)) ? "★" : "☆"}</button>
                        <button className="btn" onClick={(event) => { event.stopPropagation(); setRouteDetailInitialTab("overview"); setRouteDetailOpen(true); }} style={{ minWidth: 36, minHeight: 36, padding: 0, background: "rgba(7,10,15,.82)", borderColor: `${accent}35`, fontSize: 12 }} title={pickLegacyLocalizedText(lang, "Plein écran", "Fullscreen", "Pantalla completa")}>⛶</button>
                      </div>
                    </div>
                    <div style={{ display: "grid", gridTemplateColumns: "40px 1fr 40px", alignItems: "center" }}>
                      <div style={{ pointerEvents: "auto" }}><button className="btn" onClick={(event) => { event.stopPropagation(); selectAdjacentRoute(-1); }} style={{ minWidth: 40, minHeight: 40, padding: 0, background: "rgba(7,10,15,.82)", borderColor: `${accent}35`, fontSize: 16 }}>‹</button></div>
                      <div />
                ...
                <div style={{ padding: 10, borderRadius: 17, background: "linear-gradient(145deg,rgba(255,255,255,.04),rgba(5,8,13,.90))", border: `1px solid ${accent}28` }}>
                  <div style={{ display: "grid", gridTemplateColumns: "1fr auto", gap: 8, alignItems: "start" }}>
                    <div style={{ minWidth: 0 }}><div style={{ color: "#fff", fontSize: 10.5, fontWeight: 1000, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{buildRouteDisplayName(selectedRoute, selectedTerrain, lang)}</div>{selectedRoute.community ? <div style={{ marginTop: 3, color: accent, fontSize: 7.3, fontWeight: 900 }}>👥 {selectedRoute.community.ownerDisplayName || pickLegacyLocalizedText(lang, "Athlète", "Athlete", "Atleta")}{selectedRoute.community.description ? ` · ${selectedRoute.community.description}` : ""}</div> : null}<div style={{ marginTop: 3, color: textSoft, fontSize: 7.3 }}>{pickLegacyLocalizedText(lang, "Touchez la carte pour ouvrir la fiche complète et la carte interactive.", "Tap the map to open the full route page and interactive map.", "Toca el mapa para abrir la ficha completa y el mapa interactivo.")}</div></div>
                    <div style={{ padding: "5px 8px", borderRadius: 999, border: `1px solid ${accent}30`, color: accent, fontSize: 7.2, fontWeight: 1000 }}>{formatDistance(selectedRoute.distanceM)}</div>
                  </div>
                  <div style={{ display: "grid", gridTemplateColumns: "repeat(3,minmax(0,1fr))", gap: 6, marginTop: 8 }}><MiniStat label="D+" value={selectedTerrain?.hasElevation ? `+${Math.round(selectedTerrain.gainM)} m` : `+${Math.round(selectedRoute.elevationGainM || 0)} m`} accent={accent}/><MiniStat label={pickLegacyLocalizedText(lang, "SOMMET", "HIGH POINT", "CIMA")} value={selectedTerrain?.maxAltitudeM != null ? `${Math.round(selectedTerrain.maxAltitudeM)} m` : "—"} accent={accent}/><MiniStat label={pickLegacyLocalizedText(lang, "DURÉE", "TIME", "TIEMPO")} value={formatDuration(estimateOutdoorRouteDurationMs(selectedRoute, activitySport))} accent={accent}/></div>
                  <div className="running-map-actions" style={{ marginTop: 8 }}><button className="btn" onClick={() => { setRouteDetailInitialTab("overview"); setRouteDetailOpen(true); }} style={{ minHeight: 37, color: accent, borderColor: `${accent}55`, fontSize: 8.8, fontWeight: 1000 }}>▤ {pickLegacyLocalizedText(lang, "FICHE COMPLÈTE", "FULL ROUTE", "FICHA COMPLETA")}</button><button className="btn" onClick={() => setRoutePanelTab("guide")} style={{ minHeight: 37, fontSize: 7.2, fontWeight: 1000 }}>▶ {pickLegacyLocalizedText(lang, "GUIDER", "GUIDE", "GUIAR")}</button><button className="btn" onClick={() => { setRouteDetailInitialTab("plan"); setRouteDetailOpen(true); }} style={{ minHeight: 37, fontSize: 7.2, fontWeight: 1000 }}>◷ {pickLegacyLocalizedText(lang, "PLANIFIER", "PLAN", "PLANIFICAR")}</button></div>
                </div>

                <div style={{ display: "flex", justifyContent: "space-between", gap: 8, alignItems: "center", padding: "2px 1px" }}>
                  <div style={{ color: textSoft, fontSize: 8.8, lineHeight: 1.35 }}>{pickLegacyLocalizedText(lang, "Utilise les flèches sur la carte pour changer de parcours.", "Use the arrows on the map to switch routes.", "Usa las flechas del mapa para cambiar de ruta.")}</div>
                  <button className="btn" onClick={() => setRouteListOpen((value) => !value)} style={{ minHeight: 34, padding: "4px 9px", fontSize: 8.6, fontWeight: 1000, whiteSpace: "nowrap" }}>{routeListOpen ? pickLegacyLocalizedText(lang, "FERMER", "CLOSE", "CERRAR") : pickLegacyLocalizedText(lang, "COMPARER", "COMPARE", "COMPARAR")}</button>
                </div>
      ...
      {routePanelTab === "guide" && selectedRoute ? <>
        ...
        {["trail", "hiking", "walking"].includes(activitySport) && routeExtras ? <><OutdoorRoutePlannerPanel route={selectedRoute} lang={lang} accent={accent} textSoft={textSoft} onChange={setRouteExtras}/><OutdoorLongDistancePanel route={selectedRoute} sport={activitySport} extras={routeExtras} lang={lang} accent={accent} textSoft={textSoft}/></> : null}
      </> : null}
...
function HistoryCard({ activity, lang, accent, onClick }: {
    ...
    const sportIcon = sport === "trail" ? "⛰️" : sport === "hiking" ? "🥾" : sport === "walking" ? "🚶" : sport === "treadmill" ? "🏃‍♂️" : "🏃";
    const metric = outdoorAverageMetricValue(activity, sport);
    return <button onClick={onClick} className="card" style={{ width: "100%", display: "grid", gridTemplateColumns: "48px 1fr auto", gap: 10, alignItems: "center", padding: 11, color: "#fff", textAlign: "left", cursor: "pointer" }}>
        <div style={{ width: 46, height: 46, display: "grid", placeItems: "center", borderRadius: 14, background: `${accent}12`, border: `1px solid ${accent}30`, fontSize: 21 }}>{sportIcon}</div>
        <div>
            <div style={{ color: accent, fontSize: 8, fontWeight: 1000, letterSpacing: ".06em" }}>{outdoorSportLabel(sport, lang).toUpperCase()}</div>
            <b style={{ fontSize: 10.5 }}>{activity.title || outdoorActivityTitle(sport, lang)}</b>
            <div style={{ fontSize: 9, opacity: .56, marginTop: 3 }}>{activityDate(activity.startedAt, lang)} · {formatDistance(activity.distanceM)} · {formatDuration(activity.elapsedMs)}</div>
        </div>
        <div style={{ textAlign: "right", color: accent, fontSize: 10, fontWeight: 1000 }}>{metric}<div style={{ fontSize: 7, opacity: .7 }}>{outdoorAverageMetricLabel(sport, lang)}</div><div style={{ opacity: .5, color: "#fff", marginTop: 3 }}>›</div></div>
    </button>;
}
...
function buildSportRouteDetails(route: RunningRouteTemplate, terrain: ReturnType<typeof analyzeRunningTerrain> | null, sport: OutdoorPerformanceSport, lang: string) {
    ...
    if (sport === "walking" || sport === "nordic-walking") return {
        title: pickLegacyLocalizedText(lang, "LECTURE MARCHE", "WALK READOUT", "LECTURA MARCHA"),
        metrics: [
            { label: pickLegacyLocalizedText(lang, "DURÉE", "DURATION", "DURACIÓN"), value: estimated },
...
function gpsMaxSpeedMpsForSport(sport: OutdoorPerformanceSport): number {
    const canonical = canonicalOutdoorPerformanceSport(sport);
    if (canonical === "walking" || canonical === "hiking") return 5; // 18 km/h, au-delà d'une marche normale.
