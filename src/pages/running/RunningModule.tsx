...
function RouteMap({ points, accent, waiting, lang = "fr", textSoft = "#a8a8b3", showRouteNetwork = false, zoomable = false, immersive = false, fullscreen = false, route, terrain, activePointIndex = null, onActivePointChange, onOpen }: {
    ...
    if (mapMode === "3d" && zoomable && points.length >= 2) return <div style={{ width: "100%", height: fullscreen ? "100%" : undefined, aspectRatio: fullscreen ? undefined : immersive ? "4/3" : "5/3", maxHeight: fullscreen ? "none" : immersive ? 480 : 360, minHeight: fullscreen ? 0 : immersive ? 270 : 190, position: "relative", overflow: "hidden", borderRadius: fullscreen ? 12 : immersive ? 21 : 15, background: "#101821", border: "1px solid rgba(255,255,255,.08)", boxShadow: immersive ? "0 22px 52px rgba(0,0,0,.30)" : undefined }}>
        ...
        <div style={{ position: "absolute", left: 58, top: 8, zIndex: 30, display: "flex", gap: 3, padding: 3, borderRadius: 12, background: "rgba(5,8,13,.86)", border: "1px solid rgba(255,255,255,.12)", backdropFilter: "blur(12px)" }}>
          <button className="btn" onClick={() => setMapMode("2d")} style={{ minWidth: 34, minHeight: 30, padding: "0 7px", fontSize: 7, fontWeight: 1000 }}>2D</button>
          <button className="btn" style={{ minWidth: 34, minHeight: 30, padding: "0 7px", fontSize: 7, fontWeight: 1000, color: accent, borderColor: `${accent}55`, background: `${accent}10` }}>3D</button>
        </div>
    </div>;
