...
export default function RunningModule({ go, params }: Props) {
    ...
    if (view === "detail" && selected) {
      ...
      {detailSensors.sampleCount ? <div style={{ marginTop: 10 }}><Section title={lang === "fr" ? "CAPTEURS & PHYSIO" : lang === "es" ? "SENSORES Y FISIO" : "SENSORS & PHYSIO"}><div style={{ display: "grid", gridTemplateColumns: "repeat(2,minmax(0,1fr))", gap: 8 }}><MiniStat label={lang === "fr" ? "FC MOY." : lang === "es" ? "FC MEDIA" : "AVG HR"} value={detailSensors.avgHeartRateBpm == null ? "—" : `${Math.round(detailSensors.avgHeartRateBpm)} bpm`} accent={accent}/><MiniStat label={lang === "fr" ? "FC MAX" : "MAX HR"} value={detailSensors.maxHeartRateBpm == null ? "—" : `${Math.round(detailSensors.maxHeartRateBpm)} bpm`} accent={accent}/><MiniStat label={lang === "fr" ? "CADENCE MOY." : lang === "es" ? "CADENCIA MEDIA" : "AVG CADENCE"} value={detailSensors.avgCadenceSpm == null ? "—" : `${Math.round(detailSensors.avgCadenceSpm)} spm`} accent={accent}/><MiniStat label={lang === "fr" ? "VITESSE CAPTEUR" : lang === "es" ? "VELOCIDAD SENSOR" : "SENSOR SPEED"} value={detailSensors.avgSensorSpeedMps == null ? "—" : `${(detailSensors.avgSensorSpeedMps * 3.6).toFixed(1)} km/h`} accent={accent}/></div>{selected.sensorDevices?.length ? <div style={{ marginTop: 8, fontSize: 8.4, color: textSoft }}>{selected.sensorDevices.map((device) => device.name).join(" · ")}</div> : null}</Section></div> : null}
    ...
    {routeOptions.length ? <div style={{ marginTop: 10 }}><Section title={lang === "fr" ? "PARCOURS & GHOST" : lang === "es" ? "RUTAS & GHOST" : "ROUTES & GHOST"} right={selectedRoute ? <button className="btn" disabled={!selectedRouteHasReference} onClick={() => selectedRouteHasReference && setGhostEnabled((value) => !value)} style={{ minHeight: 30, padding: "4px 8px", fontSize: 8.5, fontWeight: 1000, opacity: selectedRouteHasReference ? 1 : .45, color: ghostEnabled ? accent : undefined, borderColor: ghostEnabled ? `${accent}77` : undefined }}>{selectedRouteHasReference ? `GHOST ${ghostEnabled ? "ON" : "OFF"}` : (lang === "fr" ? "PARCOURS SEUL" : lang === "es" ? "SOLO RUTA" : "ROUTE ONLY")}</button> : undefined}><div style={{ color: textSoft, fontSize: 9.2, lineHeight: 1.4, marginBottom: 8 }}>{lang === "fr" ? "Rejoue un ancien parcours et compare ton temps au même endroit, en direct." : lang === "es" ? "Repite una ruta anterior y compara tu tiempo en el mismo punto, en directo." : "Run a previous route again and compare your time at the same distance, live."}</div><div style={{ display: "grid", gap: 7 }}>{routeOptions.slice(0, 6).map((route) => { const active = selectedRouteId === route.id; const favorite = !!route.sourceActivityId && favoriteSourceIds.has(route.sourceActivityId); const routeTerrain = analyzeRunningTerrain(route.route); return <div key={route.id} className="card" style={{ padding: 9, display: "grid", gridTemplateColumns: "1fr auto", gap: 7, alignItems: "center", borderRadius: 15, borderColor: active ? `${accent}66` : undefined, background: active ? `linear-gradient(145deg,${accent}16,rgba(4,6,10,.82))` : "linear-gradient(145deg,rgba(255,255,255,.038),rgba(4,6,10,.70))", boxShadow: active ? `0 14px 28px ${accent}10, inset 0 1px 0 ${accent}18` : "0 11px 22px rgba(0,0,0,.24), inset 0 1px 0 rgba(255,255,255,.025)" }}><button type="button" onClick={() => selectRoute(route)} style={{ border: 0, background: "transparent", color: "inherit", padding: 0, textAlign: "left", cursor: "pointer" }}><div style={{ display: "flex", alignItems: "center", gap: 6, minWidth: 0 }}><div style={{ fontSize: 10, fontWeight: 1000, color: active ? accent : undefined, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{route.name}</div>{routeTerrain.hasElevation ? <span style={{ flex: "0 0 auto", padding: "2px 5px", borderRadius: 999, border: `1px solid ${accent}33`, color: accent, fontSize: 6.8, fontWeight: 1000 }}>{terrainLabel(routeTerrain.terrain, lang)} {routeTerrain.difficultyScore}</span> : null}</div><div style={{ marginTop: 3, fontSize: 8.5, color: textSoft }}>{formatDistance(route.distanceM)} · +{Math.round(routeTerrain.hasElevation ? routeTerrain.gainM : route.elevationGainM)} m · {routeTerrain.hasElevation ? `${routeTerrain.hills.length} ${lang === "fr" ? "côtes" : lang === "es" ? "cuestas" : "hills"} · ` : ""}{route.referenceElapsedMs > 0 ? formatDuration(route.referenceElapsedMs) : (lang === "fr" ? "sans chrono" : lang === "es" ? "sin tiempo" : "no timing")}{route.source && route.source !== "activity" ? ` · ${route.source.toUpperCase()}` : ""}</div></button><button className="btn" onClick={() => toggleFavoriteRoute(route)} style={{ minWidth: 34, minHeight: 34, padding: 0, color: favorite ? accent : undefined, borderColor: favorite ? `${accent}66` : undefined }}>{favorite ? "★" : "☆"}</button></div>; })}</div>{selectedRoute ? <div style={{ marginTop: 8 }}><RouteMap points={selectedRoute.route} accent={accent} waiting={copy.waiting}/>{selectedTerrain?.hasElevation ? <div style={{ marginTop: 8 }}><RunningElevationProfile points={selectedRoute.route} accent={accent} textSoft={textSoft}/><div className="card" style={{ marginTop: 7, padding: 10, borderColor: `${accent}33` }}><div style={{ display: "grid", gridTemplateColumns: "repeat(4,minmax(0,1fr))", gap: 6 }}><MiniStat label={lang === "fr" ? "DIFFICULTÉ" : lang === "es" ? "DIFICULTAD" : "DIFFICULTY"} value={`${selectedTerrain.difficultyScore}/100`} accent={accent}/><MiniStat label="D+" value={`+${Math.round(selectedTerrain.gainM)} m`} accent={accent}/><MiniStat label={lang === "fr" ? "CÔTES" : lang === "es" ? "CUESTAS" : "HILLS"} value={String(selectedTerrain.hills.length)} accent={accent}/><MiniStat label={lang === "fr" ? "PENTE MAX" : lang === "es" ? "PEND. MAX" : "MAX GRADE"} value={`${selectedTerrain.maxGradePct.toFixed(1)}%`} accent={accent}/></div><div style={{ marginTop: 8, fontSize: 9, lineHeight: 1.45, color: textSoft }}><b style={{ color: accent }}>{terrainLabel(selectedTerrain.terrain, lang)}</b> · {selectedTerrainAdvice?.text}</div>{selectedTerrainAdvice ? <button className="btn" onClick={applyTerrainRecommendation} style={{ width: "100%", minHeight: 38, marginTop: 8, fontSize: 8.5, fontWeight: 1000, color: accent, borderColor: `${accent}55` }}>{lang === "fr" ? "APPLIQUER LA SÉANCE CONSEILLÉE" : lang === "es" ? "APLICAR SESIÓN RECOMENDADA" : "APPLY RECOMMENDED WORKOUT"}</button> : null}</div></div> : null}</div> : null}</Section></div> : null}

    {!routeOptions.length ? <RunningSurface accent={accent} style={{ marginTop: 10 }}><div style={{ textAlign: "center", color: textSoft, fontSize: 9.5, lineHeight: 1.5, padding: 12 }}>{lang === "fr" ? "Aucun parcours enregistré pour le moment. Une sortie GPS ou un import GPX/TCX apparaîtra ici." : lang === "es" ? "Todavía no hay rutas guardadas. Una carrera GPS o una importación GPX/TCX aparecerá aquí." : "No saved routes yet. A GPS run or GPX/TCX import will appear here."}</div></RunningSurface> : null}
    </div>
