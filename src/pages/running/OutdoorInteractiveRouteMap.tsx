...
import { fetchOutdoorRoutePlaceContext, type OutdoorRoutePlace } from "../../activity/outdoorRoutePlaces";
import { OutdoorMapMarkerIcon, outdoorMapMarkerColor, outdoorMapMarkerLabel } from "./OutdoorMapMarkerIcon";
...
export default function OutdoorInteractiveRouteMap({ route = null, points, routeName, performanceActivity = null, terrain = null, showPlaces = true, showRouteNetwork = false, accent, lang, textSoft = "#a8a8b3", height = "clamp(350px,58svh,680px)", fullscreen = false, activePointIndex = null, onActivePointChange, onFullscreen, onCloseFullscreen }: Props) {
  ...
  return <div ref={hostRef} className="running-map-shell" onPointerDown={onPointerDown} onPointerMove={onPointerMove} onPointerUp={(event) => { selectNearestPoint(event); onPointerUp(event); }} onPointerCancel={onPointerUp} onWheel={onWheel} style={{ ...shellStyle, touchAction: "none", userSelect: "none", cursor: "grab" }}>
    {layout ? <>
      ...
      {layout.screen[0] ? <MapMarkerBadge x={layout.screen[0].x} y={layout.screen[0].y} kind="start" accent={accent} title={outdoorMapMarkerLabel("start", lang)}/> : null}
      {layout.screen[layout.screen.length - 1] ? <MapMarkerBadge x={layout.screen[layout.screen.length - 1].x} y={layout.screen[layout.screen.length - 1].y} kind="finish" accent={accent} title={outdoorMapMarkerLabel("finish", lang)}/> : null}
      {places.slice(0, fullscreen ? 18 : 10).map((place) => { const point = screenPoint(place.lat, place.lon, layout); if (point.x < -30 || point.y < -30 || point.x > layout.width + 30 || point.y > layout.height + 30) return null; return <MapMarkerBadge key={place.id} x={point.x} y={point.y} kind={place.category} accent={accent} title={place.name} selected={selectedPlace?.id === place.id} onClick={() => setSelectedPlace(place)}/>; })}
...
function MapMarkerBadge({ x, y, kind, accent, title, selected = false, onClick }: { x: number; y: number; kind: Parameters<typeof OutdoorMapMarkerIcon>[0]["kind"]; accent: string; title: string; selected?: boolean; onClick?: () => void }) {
  const color = outdoorMapMarkerColor(kind, accent);
  const shared: React.CSSProperties = { position: "absolute", left: x, top: y, transform: "translate(-50%,-50%)", zIndex: selected ? 12 : 8, width: kind === "start" || kind === "finish" ? 35 : 31, height: kind === "start" || kind === "finish" ? 35 : 31, minWidth: 0, minHeight: 0, padding: 0, borderRadius: 999, display: "grid", placeItems: "center", background: "rgba(6,10,16,.94)", color, border: `1.5px solid ${color}`, boxShadow: selected ? `0 0 0 3px ${color}25, 0 7px 18px rgba(0,0,0,.48)` : "0 6px 16px rgba(0,0,0,.42)", pointerEvents: "auto" };
  if (!onClick) return <span title={title} style={{ ...shared, pointerEvents: "none" }}><OutdoorMapMarkerIcon kind={kind} size={kind === "start" || kind === "finish" ? 19 : 17}/></span>;
  return <button type="button" className="btn" title={title} aria-label={title} onPointerDown={(event) => event.stopPropagation()} onPointerUp={(event) => event.stopPropagation()} onClick={(event) => { event.preventDefault(); event.stopPropagation(); onClick(); }} style={shared}><OutdoorMapMarkerIcon kind={kind} size={17}/></button>;
}
...
function MapToolbar({ accent, lang, mapMode, theme, themeMenu, setMapMode, setThemeMenu, setTheme, resetView, onFullscreen, onCloseFullscreen, fullscreen }: {
  ...
  return <div className="running-map-toolbar" onPointerDown={(event) => event.stopPropagation()} onPointerUp={(event) => event.stopPropagation()} style={{ position: "absolute", right: 9, top: 9, zIndex: 30, display: "flex", flexWrap: "wrap", justifyContent: "flex-end", gap: 5, pointerEvents: "auto" }}>
    <div style={{ display: "flex", gap: 3, padding: 3, borderRadius: 12, background: "rgba(5,8,13,.88)", border: "1px solid rgba(255,255,255,.12)", backdropFilter: "blur(12px)" }}><button type="button" className="btn" onClick={() => setMapMode("2d")} style={toolButton(mapMode === "2d", accent)}>2D</button><button type="button" className="btn" onClick={() => setMapMode("3d")} style={toolButton(mapMode === "3d", accent)}>3D</button></div>
    <div style={{ position: "relative" }}><button type="button" className="btn" title={pickText(lang,"Style de carte","Map style","Estilo de mapa")} onClick={() => setThemeMenu((value) => !value)} style={{ ...iconButton, color: accent }}>{themeIcon(theme)}</button>{themeMenu ? <div style={{ position: "absolute", right: 0, top: 44, width: 150, padding: 5, borderRadius: 13, background: "rgba(5,8,13,.96)", border: "1px solid rgba(255,255,255,.13)", boxShadow: "0 14px 34px rgba(0,0,0,.38)" }}>{themes.map(([id,label]) => <button type="button" key={id} className="btn" onClick={() => { setTheme(id); setThemeMenu(false); }} style={{ width: "100%", minHeight: 34, margin: "2px 0", textAlign: "left", padding: "5px 8px", color: theme === id ? accent : undefined, borderColor: theme === id ? `${accent}55` : undefined, fontSize: 8 }}>{themeIcon(id)} {label}</button>)}</div> : null}</div>
    {mapMode === "2d" ? <button type="button" className="btn" title={pickText(lang,"Recentrer","Recenter","Centrar")} onClick={resetView} style={iconButton}>◎</button> : null}
    {!fullscreen && onFullscreen ? <button type="button" className="btn" title={pickText(lang,"Plein écran","Full screen","Pantalla completa")} onClick={onFullscreen} style={iconButton}>⛶</button> : null}
    {fullscreen && onCloseFullscreen ? <button type="button" className="btn" title={pickText(lang,"Fermer","Close","Cerrar")} onClick={onCloseFullscreen} style={iconButton}>✕</button> : null}
  </div>;
}
