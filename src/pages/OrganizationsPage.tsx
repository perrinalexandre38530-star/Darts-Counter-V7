...
export default function OrganizationsPage({ go }: Props) {
  ...
  const renderWizardStep = () => {
    if (wizardStep === 0) return <div style={{ display: "grid", gridTemplateColumns: "repeat(2,minmax(0,1fr))", gap: 9 }}>{KIND_OPTIONS.map((kind) => <button key={kind.id} type="button" onClick={() => setKind(kind.id)} style={{ minHeight: 83, borderRadius: 14, border: `1px solid ${draft.kind === kind.id ? theme.primary : theme.borderSoft}`, background: draft.kind === kind.id ? `${theme.primary}15` : "rgba(255,255,255,.025)", color: draft.kind === kind.id ? theme.primary : theme.text, cursor: "pointer", textAlign: "left", padding: "11px" }}><div style={{ fontSize: 8.5, fontWeight: 1000, opacity: .72, letterSpacing: .8 }}>{kind.short}</div><div style={{ marginTop: 3, fontSize: 11.5, fontWeight: 1000 }}>{kind.label}</div><div style={{ marginTop: 5, color: theme.textSoft, fontSize: 8.6, lineHeight: 1.35 }}>{kind.detail}</div></button>)}</div>;
  ...
  const renderEntry = () => (
    <div style={{ display: "grid", gap: 12 }}>
      {entryMode === "create" ? renderCreateWizard() : <>
        ...
        <div style={{ ...card, padding: 15 }}><div style={{ color: theme.text, fontSize: 12.5, fontWeight: 1000 }}>{L("POUR QUI ?", "WHO IS IT FOR?", "¿PARA QUIÉN?")}</div><div style={{ marginTop: 10, display: "grid", gridTemplateColumns: "repeat(2,minmax(0,1fr))", gap: 8 }}>{KIND_OPTIONS.map((kind) => <div key={kind.id} style={{ minHeight: 54, borderRadius: 12, border: `1px solid ${theme.borderSoft}`, background: "rgba(255,255,255,.025)", padding: "9px 10px" }}><div style={{ color: theme.primary, fontSize: 8, fontWeight: 1000, letterSpacing: .8 }}>{kind.short}</div><div style={{ marginTop: 3, color: theme.text, fontSize: 10.5, fontWeight: 900 }}>{kind.label}</div></div>)}</div></div>
      </>}
    </div>
  );
