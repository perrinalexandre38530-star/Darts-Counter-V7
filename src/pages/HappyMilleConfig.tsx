...
export type HappyMilleConfigPayload = {
  players: number;
  botsEnabled: boolean;
  botLevel: BotLevel;
  rounds: number;
...
const INFO_TEXT = `Course au score : premier à 1000 (ou objectif) gagne.`;
...
export default function HappyMilleConfig(props: any) {
  ...
  const payload: HappyMilleConfigPayload = { players, botsEnabled, botLevel, rounds, objective };
  ...
  function start() {
    // App.tsx wiring: tab "happy_mille_play"
    if (props?.setTab) return props.setTab("happy_mille_play", { config: payload });
  ...
  return (
    <div className="page">
      <PageHeader
        ...
        right={<InfoDot title="Règles HAPPY MILLE" content={INFO_TEXT} />}
      />
