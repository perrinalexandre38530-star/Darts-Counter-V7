...
export async function beginMatch(init: AutosaveInit) {
  ...
  // 🔁 Sync event (best-effort)
  // On garde un payload compact: assez pour reconstruire des timelines multi-device.
  Promise.resolve(
    EventBuffer.push({
      sport: "darts",
      mode: init.kind,
      event_type: "MATCH_BEGIN",
      payload: {
        matchId: init.matchId,
        resumeId: init.resumeId,
        kind: init.kind,
        game: init.game,
        players: init.players.map((p) => ({ id: p.id, name: p.name ?? "" })),
        createdAt: t,
      },
    })
  ).catch(() => void 0);
...
export async function pushVisit(
  ...
  // 🔁 Visit event (coarse-grained, 1 par volée)
  // NB: volume OK, et bien plus "rejouable" qu'un snapshot complet.
  Promise.resolve(
    EventBuffer.push({
      sport: "darts",
      mode: "x01",
