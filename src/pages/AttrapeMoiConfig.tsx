...
function RulesContent({ primary }: { primary: string }) {
  return <div style={{ display: "grid", gap: 11, fontSize: 13, lineHeight: 1.45 }}>
    ...
    <div><strong style={{ color: ACCENT }}>FUYARD</strong><br />Il joue en premier à chaque round. Il gagne immédiatement par ÉVASION s’il atteint le score d’écart configuré, ou s’il n’est pas rattrapé à la fin des rounds max.</div>
...
export default function AttrapeMoiConfig(props: any) {
  ...
  const [escapeScore, setEscapeScore] = React.useState<number>(Math.max(Number(saved.escapeScore ?? 300), Number(saved.headStart ?? 100) + 1));
  ...
  React.useEffect(() => { if (escapeScore <= headStart) setEscapeScore(Math.max(headStart + 50, headStart + 1)); }, [headStart]);
  React.useEffect(() => {
    try { localStorage.setItem(LS_CFG_KEY, JSON.stringify({ participantMode, teamsSourceMode, selectedIds, teamAssignments, selectedStoredTeamIds, selectedBotTeamIds, savedTeamMemberSelections, botsPanelEnabled, botTeamsPanelEnabled, botLevel, headStart, escapeScore, pursuitRounds, legVictoryMode, legVictoryTarget, setVictoryMode, setVictoryTarget, legsBestOf, setsBestOf, startingRunner, scoreInputMethod, playerDartSets })); } catch {}
  }, [participantMode, teamsSourceMode, selectedIds, teamAssignments, selectedStoredTeamIds, selectedBotTeamIds, savedTeamMemberSelections, botsPanelEnabled, botTeamsPanelEnabled, botLevel, headStart, escapeScore, pursuitRounds, legVictoryMode, legVictoryTarget, setVictoryMode, setVictoryTarget, legsBestOf, setsBestOf, startingRunner, scoreInputMethod, playerDartSets]);
  ...
  function onStart() {
    ...
    const payload: CatchMeConfigPayload = {
      ...
      escapeScore: Math.max(Number(headStart) + 1, Math.min(10000, Number(escapeScore) || 300)),
  ...
  const pursuitBlock = <section style={guidedCard}><h3 style={{ margin: "0 0 8px", color: primary, fontSize: 13, textTransform: "uppercase", letterSpacing: 1 }}>Poursuite</h3><div style={panel}>
    ...
    <OptionRow label="Score d’évasion"><OptionSelect value={escapeScore} options={[150,200,250,300,400,500,600,750,1000].filter((v) => v > headStart)} onChange={(v: any) => setEscapeScore(Math.max(headStart + 1, Number(v) || 300))} /></OptionRow>
    ...
    <div style={{ marginTop: 9, padding: 10, borderRadius: 12, background: `${primary}0d`, border: `1px solid ${primary}30`, color: "#cfd4ea", fontSize: 10.8, lineHeight: 1.45 }}>Départ : Fuyard {headStart} pts, Chasseur 0. CAPTURE à 0 d’écart. ÉVASION immédiate à +{escapeScore} d’écart. Si aucun seuil n’est atteint, le Fuyard s’échappe aussi à la fin des {pursuitRounds} rounds.</div>
  </div></section>;
