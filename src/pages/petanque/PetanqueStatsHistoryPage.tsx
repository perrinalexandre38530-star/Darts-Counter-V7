...
// ✅ Copie conforme UI de HistoryPage (Darts) mais filtrée Pétanque
// - Reprise => petanque_play (resumeId)
// =============================================================
...
export default function PetanqueStatsHistoryPage({ store, go }: Props) {
  ...
  function goResume(e: any) {
    const resumeId = e?.resumeId || e?.id;
    go("petanque_play", { resumeId, from: "petanque_history" });
  }
  ...
  return (
    <div style={S.page(theme)}>
      ...
      <div style={S.title(theme)}>HISTORIQUE (PÉTANQUE)</div>
      ...
      <div style={{ marginTop: 10 }}>
        {filtered.length === 0 ? (
          <div style={S.empty(theme)}>Aucune partie Pétanque pour cette période.</div>
        ) : (
          filtered.map((e) => (
            <div key={String(e.id)} style={S.item(theme)}>
              ...
              <button style={S.smallBtn(theme)} onClick={() => goResume(e)}>
                Reprendre
              </button>
            </div>
          ))
...
const S: any = {
  ...
  kpiCard: (active: boolean, _glow: string) => ({
    flex: "1 1 150px",
    borderRadius: 16,
    padding: 12,
