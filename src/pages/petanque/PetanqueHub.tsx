// ============================================
// src/pages/petanque/PetanqueHub.tsx
// Pétanque — HUB/Menu (à afficher dans l’onglet "Games")
// ============================================

import React from "react";
import { useTheme } from "../../contexts/ThemeContext";
import { loadPetanqueState, resetPetanque } from "../../lib/petanqueStore";

type Props = { go: (route: any, params?: any) => void };

export default function PetanqueHub({ go }: Props) {
  const { theme } = useTheme();
  const [st, setSt] = React.useState(() => loadPetanqueState());
  ...
  const onContinue = () => go("petanque_play");
  ...
  return (
    <div style={wrap(theme)}>
      <div style={card(theme)}>
        ...
        <div style={row}>
          <button style={primary(theme)} onClick={onContinue}>Continuer</button>
          <button style={danger(theme)} onClick={onNew}>Nouvelle partie</button>
        </div>
      </div>

      <div style={card(theme)}>
        <div style={sub(theme)}>Accès rapide</div>
        <div style={row}>
          <button style={ghost(theme)} onClick={() => go("petanque_play")}>Jouer</button>
          <button style={ghost(theme)} onClick={() => go("tournaments")}>Tournois</button>
          <button style={ghost(theme)} onClick={() => go("stats_leaderboards")}>Classements</button>
        </div>
      </div>
    </div>
  );
...
function primary(theme: any): React.CSSProperties {
  return { flex: 1, borderRadius: 14, padding: "10px 12px", border: "1px solid rgba(255,255,255,0.18)", background: "rgba(255,255,255,0.10)", color: theme?.colors?.text ?? "#fff", fontWeight: 900, cursor: "pointer" };
}
function danger(theme: any): React.CSSProperties {
  return { borderRadius: 14, padding: "10px 12px", border: "1px solid rgba(255,255,255,0.14)", background: "rgba(255,60,60,0.18)", color: theme?.colors?.text ?? "#fff", fontWeight: 900, cursor: "pointer" };
}
function ghost(theme: any): React.CSSProperties {
  return { borderRadius: 14, padding: "10px 12px", border: "1px solid rgba(255,255,255,0.14)", background: "rgba(255,255,255,0.04)", color: theme?.colors?.text ?? "#fff", fontWeight: 800, cursor: "pointer", opacity: 0.92 };
}
...
function chip(theme: any): React.CSSProperties {
  return { maxWidth: 140, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", borderRadius: 999, padding: "6px 10px", border: "1px solid rgba(255,255,255,0.14)", background: "rgba(0,0,0,0.16)", fontWeight: 900, opacity: 0.95 };
}
...
function win(theme: any): React.CSSProperties {
  return { textAlign: "center", fontWeight: 900, padding: "10px 12px", borderRadius: 14, border: "1px solid rgba(255,255,255,0.14)", background: "rgba(0,255,180,0.10)" };
}
