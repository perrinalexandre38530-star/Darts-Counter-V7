...
export default function BabyFootPhasePanel({ state, liveContext }: Props) {
  const isPenalties = false;
  const statusLabel = state.finished ? "Terminé" : state.phase === "overtime" ? "Overtime" : "Live";
    ...
    >
      <div style={{ display: "flex", justifyContent: "space-between", gap: 12, alignItems: "center", flexWrap: "wrap" }}>
        <div>
          ...
          <div style={{ marginTop: 4, fontSize: 19, fontWeight: 1100 }}>{state.phase === "overtime" ? "Prolongation" : "Match en cours"}</div>
        </div>
        <div
          style={{
            display: "inline-flex",
