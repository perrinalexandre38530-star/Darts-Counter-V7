...
// - Clone de l'esprit Pétanque: accès rapide + état de match
// =============================================================
...
export default function BabyFootHome({ go }: Props) {
  const { theme } = useTheme();
  const [st, setSt] = React.useState(() => loadBabyFootState());

  React.useEffect(() => {
    const t = setInterval(() => setSt(loadBabyFootState()), 500);
  ...
  return (
    <div style={wrap(theme)}>
      <div style={card(theme)}>
        <div style={h1}>BABY-FOOT</div>
        <div style={sub}>Partie en cours</div>

        <div style={scoreLine}>
          <span style={chip(theme)}>{st.teamA}</span>
          <span style={score}>{st.scoreA}</span>
          <span style={sep}>—</span>
          <span style={score}>{st.scoreB}</span>
        ...
        <div style={row}>
          <button style={primary(theme)} onClick={() => go("babyfoot_play")}>Continuer</button>
          <button style={ghost(theme)} onClick={() => go("babyfoot_config")}>Nouvelle partie</button>
        </div>
      </div>
      ...
      <div style={card(theme)}>
        ...
        <div style={row}>
          <button style={ghost(theme)} onClick={() => go("babyfoot_config")}>Configurer</button>
          <button style={ghost(theme)} onClick={() => go("babyfoot_play")}>Jouer</button>
...
const scoreLine: React.CSSProperties = { display: "flex", alignItems: "center", justifyContent: "center", gap: 10, padding: "10px 0" };
