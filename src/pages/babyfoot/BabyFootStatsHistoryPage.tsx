...
export default function BabyFootStatsHistoryPage({ store, go }: Props) {
  ...
  const agg = useMemo(() => {
    ...
    const scorers: Record<string, { id: string; name: string; goals: number }> = {};
    let matches = 0;
    let goals = 0;
    ...
    for (const h of all) {
      ...
      for (const p of players) {
        ...
        if (!scorers[id]) scorers[id] = { id, name: p?.name || "", goals: 0 };
        if (!scorers[id].name && p?.name) scorers[id].name = p.name;
        byId[id].played += 1;
      }
