...
export default function BabyFootStatsShell({ go }: Props) {
  const { theme } = useTheme();

  return (
    <div style={wrap(theme)}>
      <div style={title}>STATS — BABY-FOOT</div>

      <div style={grid}>
        <button style={card(theme)} onClick={() => go("babyfoot_stats_history")}>
        ...
        <button style={card(theme)} onClick={() => go("home")}>
          ...
          <div style={cardSub}>Accueil baby-foot</div>
        </button>
      </div>
    </div>
  );
