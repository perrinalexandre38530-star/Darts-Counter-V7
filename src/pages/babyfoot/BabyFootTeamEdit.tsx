// @ts-nocheck
// =============================================================
// src/pages/babyfoot/BabyFootTeamEdit.tsx
// Équipe Baby-Foot — EDIT (UI refacto MOBILE SAFE)
...
export default function BabyFootTeamEdit({ go, params }: Props) {
  ...
  const existing = React.useMemo(() => {
    ...
    return loadBabyFootTeams().find((x) => x.id === teamId) ?? null;
  }, [teamId]);

  const [team, setTeam] = React.useState<BabyFootTeam>(() =>
    existing ?? createBabyFootTeam({ id: teamId })
  );
...
const availableProfiles = React.useMemo(() => {
  ...
  function save(next: BabyFootTeam) {
    const fixed: BabyFootTeam = {
      ...next,
  ...
  return (
    ...
    >
      ...
      >
        <button
          onClick={() => go("babyfoot_teams" as any)}
