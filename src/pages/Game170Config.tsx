...
export type Game170ConfigPayload = {
  players: number;
  botsEnabled: boolean;
  botLevel: BotLevel;
  rounds: number;
...
const INFO_TEXT = `MVP : base jouable. Version complète : départ 170 + bust rules.`;
...
export default function Game170Config(props: any) {
  ...
  const payload: Game170ConfigPayload = { players, botsEnabled, botLevel, rounds, objective };
  ...
  function start() {
    // App.tsx wiring: tab "game_170_play"
    if (props?.setTab) return props.setTab("game_170_play", { config: payload });
