...
export default function OceanControlEnd({ state, profilesById, onClose, onReplay, onStats, onHistory }: any) {
  ...
  const totalHits = rows.reduce((sum: number, row: any) => sum + Number(row.stats?.shipHits || 0), 0);
  const totalShips = rows.reduce((sum: number, row: any) => sum + Number(row.stats?.shipsSunk || 0), 0);
  const totalSonars = rows.reduce((sum: number, row: any) => sum + Number(row.stats?.sonarUses || 0), 0);
  const bestStreak = rows.reduce((best: number, row: any) => Math.max(best, Number(row.stats?.bestHitStreak || 0)), 0);
  ...
  return <div role="dialog" aria-modal="true" style={{ position: "fixed", inset: 0, zIndex: 10000, background: "radial-gradient(circle at 50% 0%,rgba(48,185,255,.28),rgba(0,5,10,.96) 48%,#000 100%)", overflowY: "auto", padding: "max(10px,env(safe-area-inset-top)) 9px max(12px,env(safe-area-inset-bottom))", boxSizing: "border-box" }}>
    <div style={{ width: "min(760px,100%)", margin: "0 auto" }}>
      ...
      <div style={{ marginTop: 9, display: "grid", gridTemplateColumns: "repeat(5,minmax(0,1fr))", gap: 6 }}>
        {[["FLÉCHETTES",totalDarts,"#fff"],["IMPACTS",totalHits,BLUE],["NAVIRES",totalShips,RED],["SONARS",totalSonars,CYAN],["SÉRIE",bestStreak,GOLD]].map(([label,value,color]:any)=><div key={label} style={{ minWidth:0,borderRadius:14,padding:"8px 5px",textAlign:"center",background:"rgba(255,255,255,.03)",border:"1px solid rgba(255,255,255,.08)" }}><div style={{color:SOFT,fontSize:6.5,fontWeight:1000}}>{label}</div><div style={{marginTop:3,color,fontSize:17,fontWeight:1150}}>{value}</div></div>)}
      </div>
      ...
      <div style={{ marginTop: 9, display: "grid", gap: 7 }}>{rows.map((row: any, rank: number) => <div key={row.player.id} style={{ display: "grid", gridTemplateColumns: "36px 44px minmax(0,1fr) auto", gap: 8, alignItems: "center", borderRadius: 17, padding: 9, background: row.won ? `linear-gradient(135deg,${BLUE}15,rgba(255,255,255,.035))` : "rgba(255,255,255,.03)", border: `1px solid ${row.won ? BLUE : "rgba(255,255,255,.09)"}66` }}>
        ...
        <div style={{ minWidth: 0 }}><div style={{ color: row.won ? CYAN : "#fff", fontSize: 10.5, fontWeight: 1100, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{playerName(row.profile)}{row.won ? " · VAINQUEUR" : ""}</div><div style={{ marginTop: 3, color: SOFT, fontSize: 8.2 }}>{row.stats?.shipsSunk || 0} navires · {row.stats?.sonarUses || 0} sonars · série {row.stats?.bestHitStreak || 0}</div><div style={{ marginTop: 3, display:"flex", gap:4, flexWrap:"wrap" }}>{Number(row.stats?.perfectVisits||0)>0?<span style={{borderRadius:999,padding:"2px 6px",fontSize:6.5,fontWeight:1000,color:GREEN,border:`1px solid ${GREEN}55`,background:`${GREEN}10`}}>✨ {row.stats.perfectVisits} parfaite{row.stats.perfectVisits>1?"s":""}</span>:null}{Number(row.stats?.multiHitVisits||0)>0?<span style={{borderRadius:999,padding:"2px 6px",fontSize:6.5,fontWeight:1000,color:GOLD,border:`1px solid ${GOLD}55`,background:`${GOLD}10`}}>💥 {row.stats.multiHitVisits} multi-impact{row.stats.multiHitVisits>1?"s":""}</span>:null}</div></div>
      ...
      {(state.battleHistory || []).length ? <div style={{ marginTop: 10, borderRadius:18, padding:11, background:"rgba(255,255,255,.025)", border:"1px solid rgba(255,255,255,.08)" }}><div style={{color:CYAN,fontSize:9,fontWeight:1100,letterSpacing:.7,marginBottom:8}}>RAPPORT DES MANCHES</div><div style={{display:"grid",gap:6}}>{state.battleHistory.map((battle:any)=>{const owner=state.owners.find((row:any)=>row.id===battle.winnerOwnerId);return <div key={`${battle.battle}-${battle.finishedAt}`} style={{display:"grid",gridTemplateColumns:"34px minmax(0,1fr) auto",gap:8,alignItems:"center",padding:8,borderRadius:12,background:"rgba(255,255,255,.03)",border:"1px solid rgba(255,255,255,.07)"}}><strong style={{color:BLUE}}>B{battle.battle}</strong><div style={{minWidth:0}}><div style={{fontSize:9,fontWeight:1050,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{owner?.name||"Flotte"}</div><div style={{marginTop:2,color:SOFT,fontSize:7.3}}>{battle.visits} volées · {battle.darts} fléchettes · {battle.hits} impacts</div></div><strong style={{color:GOLD,fontSize:10}}>{battle.shipsSunk} coulés</strong></div>})}</div></div>:null}
