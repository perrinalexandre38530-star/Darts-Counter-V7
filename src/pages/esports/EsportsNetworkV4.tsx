...
import EsportsCompetitiveSessionV5 from "./EsportsNetworkV5";
...
export function EsportsCompetitiveNetworkV4({ state, panelStyle, buttonStyle, inputStyle, textSoft, setToast, tr }: Common & { state: EsportsState }) {
  ...
  return <div className="esports-section-stack">
    ...
    <section className="esports-panel" style={{ ...panelStyle, padding: 14 }}>
      ...
      <div className="esports-form-grid" style={{ marginTop: 9 }}>
    ...
    <EsportsCompetitiveSessionV5 state={state} ticket={ticket} panelStyle={panelStyle} buttonStyle={buttonStyle} inputStyle={inputStyle} textSoft={textSoft} setToast={setToast} tr={tr} />

    <section className="esports-panel" style={{ ...panelStyle, padding: 14 }}>
      <div style={{ display: "flex", justifyContent: "space-between", gap: 8, alignItems: "center", flexWrap: "wrap" }}><div><div style={{ fontSize: 18, fontWeight: 1000 }}>🔔 {tr("Notifications E-SPORTS", "E-SPORTS notifications", "Notificaciones E-SPORTS")} {unread ? `· ${unread}` : ""}</div><div style={{ color: textSoft, fontSize: 9 }}>{tr("Candidatures, clans et matchmaking arrivent ici en temps réel.", "Applications, clans and matchmaking arrive here in realtime.", "Candidaturas, clanes y matchmaking llegan aquí en tiempo real.")}</div></div>{unread ? <button type="button" onClick={markAll} style={{ ...buttonStyle(false), minHeight: 30, padding: "4px 8px" }}>✓ {tr("Tout lire", "Mark all read", "Marcar todo")}</button> : null}</div>
...
export function EsportsTeamMembershipV4({ cloudTeams, panelStyle, buttonStyle, inputStyle, textSoft, setToast, tr, onRefresh }: Common & { cloudTeams: PublicEsportsTeam[]; onRefresh?: () => void }) {
  ...
  return <div style={{ display: "grid", gap: 10 }}>
    {error ? <section className="esports-panel" style={{ ...panelStyle, padding: 10, color: "#fb7185", fontSize: 9.5 }}>{error}</section> : null}
    <section className="esports-panel" style={{ ...panelStyle, padding: 12 }}><div style={{ fontSize: 17, fontWeight: 1000 }}>🧩 {tr("Adhésions & rôles V0.4", "Memberships & roles V0.4", "Membresías y roles V0.4")}</div><div style={{ color: textSoft, fontSize: 9, marginTop: 3 }}>{tr("OWNER · CAPTAIN · OFFICER · MEMBER, avec demandes et invitations réelles.", "OWNER · CAPTAIN · OFFICER · MEMBER, with real requests and invitations.", "OWNER · CAPTAIN · OFFICER · MEMBER, con solicitudes e invitaciones reales.")}</div>
      ...
      {ownedPending.length ? <div style={{ marginTop: 9 }}><strong style={{ fontSize: 10 }}>{tr("Demandes reçues", "Incoming requests", "Solicitudes recibidas")} · {ownedPending.length}</strong><div style={{ marginTop: 5, display: "grid", gap: 5 }}>{ownedPending.map((m) => <div key={m.id} style={{ borderRadius: 11, padding: 8, background: "rgba(255,255,255,.03)", display: "grid", gridTemplateColumns: "minmax(0,1fr) auto", gap: 6, alignItems: "center" }}><div><strong>{m.displayName}</strong><div style={{ color: textSoft, fontSize: 8 }}>{m.teamTag ? `[${m.teamTag}] ` : ""}{m.teamName}{m.message ? ` · ${m.message}` : ""}</div></div><div style={{ display: "flex", gap: 4 }}><button type="button" onClick={() => review(m,"active")} style={{ ...buttonStyle(true), minHeight: 27, padding: "2px 7px" }}>✓</button><button type="button" onClick={() => review(m,"declined")} style={{ ...buttonStyle(false), minHeight: 27, padding: "2px 7px" }}>×</button></div></div>)}</div></div> : null}
    </section>

    {mineTeams.length ? <section className="esports-panel" style={{ ...panelStyle, padding: 12 }}><div style={{ fontWeight: 1000 }}>✉ {tr("Inviter un gamer dans mon clan", "Invite a gamer to my clan", "Invitar gamer a mi clan")}</div><div className="esports-mobile-stack" style={{ marginTop: 7, display: "grid", gridTemplateColumns: "minmax(0,220px) minmax(0,1fr) auto", gap: 6 }}><select value={inviteTeamId} onChange={(e) => setInviteTeamId(e.target.value)} style={inputStyle}>{mineTeams.map((t) => <option key={t.id} value={t.id}>{t.tag ? `[${t.tag}] ` : ""}{t.name}</option>)}</select><input value={query} onChange={(e) => setQuery(e.target.value)} onKeyDown={(e) => e.key === "Enter" && void search()} placeholder={tr("Pseudo du gamer…", "Gamer nickname…", "Alias del gamer…")} style={inputStyle}/><button type="button" disabled={busy || query.trim().length < 2} onClick={search} style={buttonStyle(false)}>⌕</button></div>{players.length ? <div style={{ marginTop: 6, display: "grid", gap: 4 }}>{players.slice(0,8).map((p) => <div key={p.userId} style={{ display: "grid", gridTemplateColumns: "minmax(0,1fr) auto", gap: 6, alignItems: "center", padding: 7, borderRadius: 10, background: "rgba(255,255,255,.025)" }}><span>{p.displayName}</span><button type="button" onClick={() => invite(p)} style={{ ...buttonStyle(true), minHeight: 27, padding: "2px 7px" }}>＋ {tr("Inviter", "Invite", "Invitar")}</button></div>)}</div> : null}</section> : null}

    {manageableActive.length ? <section className="esports-panel" style={{ ...panelStyle, padding: 12 }}><div style={{ fontWeight: 1000 }}>🎖 {tr("Rôles de mes clans", "Roles in my clans", "Roles de mis clanes")}</div><div style={{ marginTop: 7, display: "grid", gap: 5 }}>{manageableActive.map((m) => <div key={m.id} className="esports-mobile-stack" style={{ display: "grid", gridTemplateColumns: "minmax(0,1fr) minmax(0,130px)", gap: 7, alignItems: "center", padding: 8, borderRadius: 11, background: "rgba(255,255,255,.025)" }}><div><strong>{m.displayName}</strong><div style={{ color: textSoft, fontSize: 8 }}>{m.teamTag ? `[${m.teamTag}] ` : ""}{m.teamName}</div></div><select value={m.role} onChange={(e) => void role(m,e.target.value as any)} style={{ ...inputStyle, minHeight: 32, padding: "4px 6px" }}><option value="captain">CAPTAIN</option><option value="officer">OFFICER</option><option value="member">MEMBER</option></select></div>)}</div></section> : null}

    <section className="esports-panel" style={{ ...panelStyle, padding: 12 }}><div style={{ fontWeight: 1000 }}>🌍 {tr("Clans rejoignables", "Joinable clans", "Clanes disponibles")} · {publicJoinable.length}</div><div style={{ marginTop: 7, display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(min(100%,220px),1fr))", gap: 6 }}>{publicJoinable.slice(0,16).map((team) => <div key={team.id} style={{ borderRadius: 12, padding: 9, background: "rgba(255,255,255,.025)" }}><strong>{team.tag ? `[${team.tag}] ` : ""}{team.name}</strong><div style={{ color: textSoft, fontSize: 8.5 }}>{team.ownerDisplayName || tr("Capitaine", "Captain", "Capitán")}</div><button type="button" disabled={busy} onClick={() => requestJoin(team)} style={{ ...buttonStyle(false), width: "100%", marginTop: 6, minHeight: 28 }}>＋ {tr("Demander à rejoindre", "Request to join", "Solicitar ingreso")}</button></div>)}</div></section>

    {myActive.filter((m) => m.role !== "owner").length ? <section className="esports-panel" style={{ ...panelStyle, padding: 12 }}><div style={{ fontWeight: 1000 }}>🛡 {tr("Mes adhésions", "My memberships", "Mis membresías")}</div><div style={{ marginTop: 6, display: "grid", gap: 5 }}>{myActive.filter((m) => m.role !== "owner").map((m) => <div key={m.id} style={{ display: "grid", gridTemplateColumns: "minmax(0,1fr) auto", gap: 6, alignItems: "center" }}><span><strong>{m.teamTag ? `[${m.teamTag}] ` : ""}{m.teamName}</strong> · <span style={{ color: textSoft, fontSize: 9 }}>{m.role.toUpperCase()}</span></span><button type="button" onClick={() => leave(m.teamId)} style={{ ...buttonStyle(false), minHeight: 27, padding: "2px 7px" }}>{tr("Quitter", "Leave", "Salir")}</button></div>)}</div></section> : null}
  </div>;
}
...
export function EsportsLfgApplicationsV4({ posts, panelStyle, buttonStyle, textSoft, setToast, tr, onRefresh }: Omit<Common,"inputStyle"> & { posts: PublicEsportsLfgPost[]; onRefresh?: () => void }) {
  ...
  return <section className="esports-panel" style={{ ...panelStyle, padding: 12 }}>
