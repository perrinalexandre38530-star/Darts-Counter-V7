...
export default function EsportsHub({ store, go, section = "overview" }: Props) {
  ...
  const shellStyle: React.CSSProperties = { background: `radial-gradient(circle at 82% 0%, ${selectedGame.accent}18, transparent 34%), ${bg}`, color: textMain };
  const panelStyle: React.CSSProperties = { width: "100%", maxWidth: "100%", borderRadius: 22, border: `1px solid ${cardSoft}`, background: `radial-gradient(circle at top, rgba(255,255,255,.045), transparent 44%), linear-gradient(145deg, ${card}, rgba(5,8,14,.97))`, boxShadow: "0 18px 40px rgba(0,0,0,.62)", overflow: "hidden" };
  const buttonStyle = (active = false): React.CSSProperties => ({ maxWidth: "100%", border: `1px solid ${active ? accent : cardSoft}`, borderRadius: 14, minHeight: 40, padding: "8px 12px", background: active ? `${accent}20` : "rgba(255,255,255,.035)", color: active ? accent : textMain, fontWeight: 900, cursor: "pointer", boxSizing: "border-box" });
  const inputStyle: React.CSSProperties = { width: "100%", maxWidth: "100%", minWidth: 0, minHeight: 42, borderRadius: 13, border: `1px solid ${cardSoft}`, background: "rgba(0,0,0,.28)", color: textMain, padding: "9px 11px", outline: "none", boxSizing: "border-box" };
  ...
  const totalWins = state.matches.filter((m: any) => m.winner === "A").length;
  const winRate = counts.matches ? Math.round((totalWins / counts.matches) * 100) : 0;
  const activeRank = state.gamer.rankByGame?.[selectedGame.id] || tr("Non classé", "Unranked", "Sin rango");

  return (
    <div className="esports-root" style={{ ...shellStyle, ["--es-accent" as any]: accent } as React.CSSProperties}>
      <div className="esports-page">
        <SportHomeWelcomeHeader
          sport="esports"
          title="E-SPORTS SCORING"
          welcome={tr("Bienvenue", "Welcome", "Bienvenido")}
          accent={accent}
          borderSoft={(theme as any)?.borderSoft || "rgba(255,255,255,.10)"}
        />

        <section
          className="esports-hero-card"
          style={{ ["--es-game-accent-soft" as any]: `${selectedGame.accent}33` } as React.CSSProperties}
        >
          <div className="esports-hero-main">
            <img className="esports-hero-logo" src={logoEsports} alt="E-SPORTS SCORING" />
            <div>
              <div style={{ color: accent, fontSize: 9, fontWeight: 1000, letterSpacing: 1.3 }}>{tr("JOUEUR ACTIF", "ACTIVE GAMER", "GAMER ACTIVO")}</div>
              <div className="esports-gamer-name">{state.gamer.displayName}</div>
              <div className="esports-subtitle">{selectedGame.icon} {selectedGame.name} · {activeRank} · {state.gamer.lookingForGroup ? tr("Recherche un groupe", "Looking for group", "Busca grupo") : tr("Prêt à jouer", "Ready to play", "Listo para jugar")}</div>
            </div>
          </div>
          <div className="esports-hero-kpis">
            <div className="esports-hero-kpi"><strong>{counts.matches}</strong><span>{tr("Matchs", "Matches", "Partidos")}</span></div>
            <div className="esports-hero-kpi"><strong>{winRate}%</strong><span>Win rate</span></div>
            <div className="esports-hero-kpi"><strong>{counts.rooms}</strong><span>{tr("Salons", "Rooms", "Salas")}</span></div>
            <div className="esports-hero-kpi"><strong>{counts.tournaments}</strong><span>{tr("Tournois", "Tournaments", "Torneos")}</span></div>
          </div>
        </section>

        <nav className="esports-section-nav" aria-label="E-SPORTS">
          {[
            ["overview", "⌂", tr("Vue", "Home", "Vista")],
            ["games", "🎮", tr("Jeux", "Games", "Juegos")],
            ["rooms", "◉", tr("Salons", "Rooms", "Salas")],
            ["matches", "VS", tr("Matchs", "Matches", "Partidos")],
            ["tournaments", "🏆", tr("Tournois", "Tournaments", "Torneos")],
            ["profile", "👤", tr("Communauté", "Community", "Comunidad")],
            ["stats", "▥", "Stats"],
          ].map(([key, icon, label]) => (
            <button key={key} type="button" className={section === key ? "is-active" : ""} onClick={() => navigateSection(key as EsportsSection)}>
      ...
      {toast ? <div className="esports-toast" style={{ background: "rgba(5,8,14,.96)", border: `1px solid ${accent}`, color: textMain, fontSize: 12, fontWeight: 900, boxShadow: `0 0 24px ${accent}40` }}>{toast}</div> : null}
    </div>
  );
}
...
function Overview({ state, selectedGame, counts, panelStyle, buttonStyle, textSoft, accent, navigateSection, tr }: any) {
  ...
  return <>
    <section style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(min(100%,150px),1fr))", gap: 9 }}>
      {[
      ...
      ].map(([icon, value, label, target]) => <button key={`${String(target)}_${String(label)}`} type="button" onClick={() => navigateSection(target)} style={{ ...panelStyle, padding: 13, textAlign: "left", color: "inherit", cursor: "pointer" }}><div style={{ fontSize: 18, color: accent, fontWeight: 1000 }}>{icon}</div><div style={{ marginTop: 8, fontSize: 24, fontWeight: 1000 }}>{value}</div><div style={{ color: textSoft, fontSize: 10.5 }}>{label}</div></button>)}
    </section>

    <section className="esports-panel esports-active-game-panel" style={{ ...panelStyle, padding: 14 }}>
      <div className="esports-active-game-head">
        <GameCover game={selectedGame} className="esports-active-game-cover" eager />
        <div className="esports-active-game-copy"><div style={{ fontSize: 11, color: textSoft, fontWeight: 900 }}>{tr("JEU ACTIF", "ACTIVE GAME", "JUEGO ACTIVO")}</div><div style={{ marginTop: 3, fontSize: 20, fontWeight: 1000 }}>{selectedGame.name}</div><div style={{ marginTop: 4, color: textSoft, fontSize: 9.5 }}>{selectedGame.publisher || ""} · {selectedGame.genre.toUpperCase()}</div></div>
        <button type="button" className="esports-active-game-change" onClick={() => navigateSection("games")} style={buttonStyle(false)}>{tr("Changer", "Change", "Cambiar")}</button>
      </div>
      <div style={{ marginTop: 12, display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(min(100%,120px),1fr))", gap: 8 }}>{[
    ...
    <section style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(min(100%,280px),1fr))", gap: 10 }}>
...
function GamesSection({ state, refresh, panelStyle, buttonStyle, textSoft, inputStyle, navigateSection, tr }: any) {
  ...
  return <section className="esports-panel" style={{ ...panelStyle, padding: 14 }}>
    <div style={{ display: "flex", alignItems: "end", justifyContent: "space-between", gap: 10, flexWrap: "wrap" }}><div><div style={{ color: textSoft, fontSize: 10, fontWeight: 900, letterSpacing: 1.5 }}>{tr("CATALOGUE E-SPORTS", "E-SPORTS CATALOG", "CATÁLOGO E-SPORTS")}</div><div style={{ fontSize: 24, fontWeight: 1000 }}>{ESPORTS_GAMES.length} {tr("jeux configurés avec jaquettes", "games configured with cover art", "juegos configurados con carátulas")}</div></div><button type="button" onClick={() => navigateSection("rooms")} style={buttonStyle(true)}>＋ {tr("Salon", "Room", "Sala")}</button></div>
    <div className="esports-mobile-stack" style={{ marginTop: 12, display: "grid", gridTemplateColumns: "minmax(0,1fr) repeat(2,minmax(0,190px))", gap: 8 }}><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder={tr("Rechercher un jeu...", "Search a game...", "Buscar un juego...")} style={inputStyle}/><select value={genre} onChange={(e) => setGenre(e.target.value)} style={inputStyle}><option value="all">{tr("Tous genres", "All genres", "Todos géneros")}</option>{[...new Set(ESPORTS_GAMES.map((g) => g.genre))].map((x) => <option key={x} value={x}>{x.toUpperCase()}</option>)}</select><select value={platform} onChange={(e) => setPlatform(e.target.value)} style={inputStyle}><option value="all">{tr("Toutes plateformes", "All platforms", "Todas plataformas")}</option>{["pc","playstation","xbox","switch","mobile","crossplay"].map((x) => <option key={x} value={x}>{displayPlatform(x)}</option>)}</select></div>
    <div className="esports-game-grid">{games.map((game) => { const selected = state.selectedGameId === game.id; const favorite = state.gamer.favoriteGameIds.includes(game.id); return <article key={game.id} className={`esports-game-card${selected ? " is-selected" : ""}`} style={{ ["--game-card-accent" as any]: game.accent } as React.CSSProperties}>
      <button type="button" className="esports-game-cover-button" onClick={() => pick(game)} aria-label={`${tr("Sélectionner", "Select", "Seleccionar")} ${game.name}`}><GameCover game={game} /></button>
      <div className="esports-game-card-body">
        <div className="esports-game-title-row"><div><div className="esports-game-title">{game.name}</div><div className="esports-game-meta">{game.publisher || ""}<br/>{game.genre.toUpperCase()} · {game.platforms.slice(0,3).map(displayPlatform).join(" · ")}</div></div><button type="button" aria-label="Favori" onClick={() => { toggleFavoriteGame(game.id); refresh(); }} className={`esports-favorite-button${favorite ? " is-favorite" : ""}`}>{favorite ? "★" : "☆"}</button></div>
        <div className="esports-game-tags">{[game.matchShape.toUpperCase(), `BO ${game.bestOf.join("/")}`, game.resultKind.toUpperCase(), game.crossplay ? "CROSSPLAY" : ""].filter(Boolean).map((tag) => <span key={tag}>{tag}</span>)}</div>
        <div className="esports-game-actions"><button type="button" onClick={() => pick(game)} style={buttonStyle(selected)}>{selected ? tr("Sélectionné", "Selected", "Seleccionado") : tr("Sélectionner", "Select", "Seleccionar")}</button><button type="button" onClick={() => { pick(game); navigateSection("rooms"); }} style={buttonStyle(false)}>＋ {tr("Salon", "Room", "Sala")}</button></div>
      </div>
    </article>; })}</div>
  </section>;
}
...
function RoomDetail({ room, state, refresh, buttonStyle, inputStyle, textSoft, setToast, tr }: any) {
  ...
  return <div>
    ...
    <div style={{ marginTop: 12, display: "grid", gap: 6 }}>{room.members.map((member: any, index: number) => <div key={member.id} className="esports-member-detail-row" style={{ borderRadius: 13, padding: 9, background: "rgba(255,255,255,.035)" }}><span style={{ width: 32, height: 32, borderRadius: 10, background: `${game.accent}18`, display: "grid", placeItems: "center", color: game.accent, fontWeight: 1000 }}>{index + 1}</span><div><strong>{member.name}</strong><div style={{ color: textSoft, fontSize: 9 }}>{member.role || "player"}</div></div><span style={{ fontSize: 9, color: member.team === "A" ? "#60a5fa" : member.team === "B" ? "#fb923c" : textSoft, fontWeight: 1000 }}>{member.team ? `TEAM ${member.team}` : "—"}</span><span className="esports-member-ready" style={{ fontSize: 9, color: member.ready ? "#34d399" : textSoft }}>{member.ready ? "READY" : "WAIT"}</span></div>)}</div>
    {room.source === "local" && room.members.length < room.maxPlayers ? <div style={{ marginTop: 9, display: "grid", gridTemplateColumns: "minmax(0,1fr) auto", gap: 7 }}><input value={name} onChange={(e) => setName(e.target.value)} onKeyDown={(e) => e.key === "Enter" && addLocalMember()} placeholder={tr("Ajouter un joueur / une équipe", "Add a player / team", "Añadir jugador / equipo")} style={inputStyle}/><button type="button" onClick={addLocalMember} style={buttonStyle(false)}>＋</button></div> : null}
...
function ProfileSection({ state, refresh, panelStyle, buttonStyle, inputStyle, textSoft, accent, go, setToast, tr }: any) {
  ...
  return <div style={{ display: "grid", gap: 10 }}>
    <section className="esports-panel" style={{ ...panelStyle, padding: 10 }}><div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(min(100%,105px),1fr))", gap: 6 }}>{tabs.map(([id, icon, label]) => <button type="button" key={id} onClick={() => setTab(id)} style={{ ...buttonStyle(tab === id), minWidth: 0, padding: "8px 6px" }}>{icon} <span style={{ marginLeft: 3 }}>{label}</span></button>)}</div></section>
    {tab === "profile" ? <>
      <section className="esports-panel" style={{ ...panelStyle, padding: 14 }}><div style={{ fontSize: 22, fontWeight: 1000 }}>{tr("Profil gamer", "Gamer profile", "Perfil gamer")}</div><div style={{ marginTop: 4, color: textSoft, fontSize: 10.5 }}>{tr("Ton identité gaming transverse : jeux, plateformes, disponibilité et recherche de groupe.", "Your cross-game identity: games, platforms, availability and group search.", "Tu identidad gaming: juegos, plataformas, disponibilidad y búsqueda de grupo.")}</div><div style={{ marginTop: 11, display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(min(100%,180px),1fr))", gap: 8 }}><input value={displayName} onChange={(e) => setDisplayName(e.target.value)} placeholder="Pseudo gamer" style={inputStyle}/><input value={country} maxLength={3} onChange={(e) => setCountry(e.target.value)} placeholder="FR" style={inputStyle}/><select value={availability} onChange={(e) => setAvailability(e.target.value)} style={inputStyle}><option value="available">🟢 {tr("Disponible", "Available", "Disponible")}</option><option value="busy">🟠 {tr("Occupé", "Busy", "Ocupado")}</option><option value="offline">⚫ Offline</option></select><label style={{ ...inputStyle, display: "flex", alignItems: "center", gap: 8 }}><input type="checkbox" checked={lookingForGroup} onChange={(e) => setLookingForGroup(e.target.checked)}/>{tr("Je cherche des joueurs / une équipe", "I'm looking for players / a team", "Busco jugadores / equipo")}</label></div><textarea value={bio} onChange={(e) => setBio(e.target.value)} placeholder="Bio gaming..." style={{ ...inputStyle, minHeight: 76, marginTop: 8, resize: "vertical" }}/><div style={{ marginTop: 8, display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(min(100%,180px),1fr))", gap: 8 }}><label style={{ display: "grid", gap: 4 }}><span style={{ color: textSoft, fontSize: 9, fontWeight: 900 }}>{tr("Plateforme principale", "Primary platform", "Plataforma principal")}</span><select value={primaryPlatform} onChange={(e) => setPrimaryPlatform(e.target.value as EsportsPlatform)} style={inputStyle}>{["pc","playstation","xbox","switch","mobile","crossplay"].map((p) => <option key={p} value={p}>{displayPlatform(p)}</option>)}</select></label><label style={{ display: "grid", gap: 4 }}><span style={{ color: textSoft, fontSize: 9, fontWeight: 900 }}>{tr("Jeu pour le niveau", "Game for rank", "Juego para rango")}</span><select value={rankGameId} onChange={(e) => setRankGameId(e.target.value)} style={inputStyle}>{ESPORTS_GAMES.map((g) => <option key={g.id} value={g.id}>{g.name}</option>)}</select></label><label style={{ display: "grid", gap: 4 }}><span style={{ color: textSoft, fontSize: 9, fontWeight: 900 }}>{tr("Niveau / rang", "Skill / rank", "Nivel / rango")}</span><input value={rankByGame[rankGameId] || ""} onChange={(e) => setRankByGame((prev) => ({ ...prev, [rankGameId]: e.target.value }))} placeholder={tr("Ex : Diamant III", "e.g. Diamond III", "Ej.: Diamante III")} style={inputStyle}/></label></div></section>
      <section className="esports-panel" style={{ ...panelStyle, padding: 14 }}><div style={{ fontWeight: 1000 }}>{tr("Identifiants plateformes", "Platform IDs", "IDs de plataformas")}</div><div style={{ marginTop: 9, display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(min(100%,190px),1fr))", gap: 8 }}>{["steam","epic","riot","ea","playstation","xbox","battlenet","switch"].map((key) => <label key={key} style={{ display: "grid", gap: 4 }}><span style={{ color: textSoft, fontSize: 9, fontWeight: 900 }}>{key.toUpperCase()}</span><input value={handles[key] || ""} onChange={(e) => setHandles((prev: any) => ({ ...prev, [key]: e.target.value }))} style={inputStyle}/></label>)}</div><div style={{ marginTop: 10, display: "flex", justifyContent: "space-between", gap: 8, flexWrap: "wrap" }}><button type="button" onClick={() => go("profiles")} style={buttonStyle(false)}>{tr("Ouvrir mon profil MULTISPORTS", "Open my MULTISPORTS profile", "Abrir mi perfil MULTISPORTS")}</button><button type="button" onClick={save} style={buttonStyle(true)}>✓ {tr("Enregistrer", "Save", "Guardar")}</button></div></section>
      <section className="esports-panel" style={{ ...panelStyle, padding: 14 }}><div style={{ fontWeight: 1000 }}>{tr("Jeux favoris", "Favorite games", "Juegos favoritos")}</div><div style={{ marginTop: 8, display: "flex", gap: 6, flexWrap: "wrap" }}>{state.gamer.favoriteGameIds.map((id: string) => { const game = getEsportsGame(id); return <span key={id} style={{ borderRadius: 999, padding: "7px 10px", border: `1px solid ${game.accent}50`, background: `${game.accent}12`, color: game.accent, fontSize: 10, fontWeight: 1000 }}>{game.icon} {game.shortName}</span>; })}</div></section>
    </> : null}
...
function EsportsFriendsPanel({ panelStyle, buttonStyle, inputStyle, textSoft, setToast, tr }: any) {
  ...
  return <div style={{ display: "grid", gap: 10 }}>
    <section className="esports-panel" style={{ ...panelStyle, padding: 14 }}><div style={{ fontSize: 22, fontWeight: 1000 }}>👥 {tr("Communauté gaming", "Gaming community", "Comunidad gaming")}</div><div style={{ marginTop: 4, color: textSoft, fontSize: 10.5 }}>{tr("Ce sont les mêmes amis que dans MULTISPORTS SCORING : pas de doublon de réseau social.", "These are the same friends as MULTISPORTS SCORING: no duplicate social network.", "Son los mismos amigos de MULTISPORTS SCORING: sin red social duplicada.")}</div><div style={{ marginTop: 10, display: "grid", gridTemplateColumns: "minmax(0,1fr) auto", gap: 7 }}><input value={query} onChange={(e) => setQuery(e.target.value)} onKeyDown={(e) => e.key === "Enter" && search()} placeholder={tr("Chercher un joueur / pseudo…", "Search player / nickname…", "Buscar jugador / alias…")} style={inputStyle}/><button type="button" disabled={loading || query.trim().length < 2} onClick={search} style={buttonStyle(true)}>⌕ {tr("Chercher", "Search", "Buscar")}</button></div>{error ? <div style={{ marginTop: 8, color: "#fb7185", fontSize: 10 }}>{error}</div> : null}</section>
    {results.length ? <section className="esports-panel" style={{ ...panelStyle, padding: 12 }}><div style={{ fontWeight: 1000 }}>{tr("Résultats", "Results", "Resultados")}</div><div style={{ marginTop: 8, display: "grid", gap: 6 }}>{results.map((user) => { const id = String(user.userId || user.id); return <div key={id} style={{ borderRadius: 13, padding: 9, background: "rgba(255,255,255,.035)", display: "grid", gridTemplateColumns: "minmax(0,1fr) auto", gap: 8, alignItems: "center" }}><div><strong>{user.displayName || user.nickname || id}</strong><div style={{ color: textSoft, fontSize: 9 }}>{user.countryCode || user.country || ""} · {user.status || "offline"}</div></div><button type="button" onClick={() => request(user)} style={buttonStyle(false)}>＋ {tr("Ami", "Friend", "Amigo")}</button></div>; })}</div></section> : null}
    <section className="esports-panel" style={{ ...panelStyle, padding: 12 }}><div style={{ display: "flex", justifyContent: "space-between", gap: 8, alignItems: "center" }}><div style={{ fontWeight: 1000 }}>{tr("Mes amis", "My friends", "Mis amigos")} · {friends.length}</div><button type="button" onClick={load} style={{ ...buttonStyle(false), minHeight: 30, padding: "4px 8px" }}>↻</button></div><div style={{ marginTop: 8, display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(min(100%,210px),1fr))", gap: 7 }}>{friends.length ? friends.map((friend) => { const id = String(friend.userId || friend.id); const status = friend.status === "online" ? "🟢" : friend.status === "away" ? "🟠" : "⚫"; return <div key={id} style={{ borderRadius: 14, padding: 10, background: "rgba(255,255,255,.035)" }}><div style={{ fontWeight: 1000 }}>{status} {friend.displayName || friend.nickname || id}</div><div style={{ marginTop: 3, color: textSoft, fontSize: 9 }}>{friend.countryCode || friend.country || ""} {friend.lastSeenAt ? `· ${friend.lastSeenAt}` : ""}</div></div>; }) : <Empty text={loading ? tr("Chargement…", "Loading…", "Cargando…") : tr("Aucun ami chargé.", "No friends loaded.", "No hay amigos cargados.")} textSoft={textSoft}/>}</div></section>
...
function EsportsPublicPlayerDiscovery({ panelStyle, buttonStyle, inputStyle, textSoft, setToast, tr }: any) {
  ...
  return <section className="esports-panel" style={{ ...panelStyle, padding: 14 }}>
    ...
    <div style={{ marginTop: 9, display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(min(100%,150px),1fr))", gap: 7 }}><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder={tr("Pseudo / mot-clé", "Nickname / keyword", "Alias / palabra clave")} style={inputStyle}/><select value={gameId} onChange={(e) => setGameId(e.target.value)} style={inputStyle}><option value="all">{tr("Tous les jeux", "All games", "Todos los juegos")}</option>{ESPORTS_GAMES.map((g) => <option key={g.id} value={g.id}>{g.name}</option>)}</select><select value={platform} onChange={(e) => setPlatform(e.target.value)} style={inputStyle}><option value="all">{tr("Toutes plateformes", "All platforms", "Todas plataformas")}</option>{["pc","playstation","xbox","switch","mobile","crossplay"].map((p) => <option key={p} value={p}>{displayPlatform(p)}</option>)}</select><input value={rank} onChange={(e) => setRank(e.target.value)} placeholder={tr("Niveau : Diamant, Gold…", "Rank: Diamond, Gold…", "Nivel: Diamante, Oro…")} style={inputStyle}/></div>
...
function EsportsTeamsPanel({ state, refresh, panelStyle, buttonStyle, inputStyle, textSoft, setToast, tr }: any) {
  ...
  return <div style={{ display: "grid", gap: 10 }}>
    ...
    <section className="esports-panel" style={{ ...panelStyle, padding: 12 }}><div style={{ fontWeight: 1000 }}>📱 {tr("Cache local / hors-ligne", "Local / offline cache", "Caché local / sin conexión")} · {state.teams.length}</div><div style={{ marginTop: 8, display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(min(100%,220px),1fr))", gap: 7 }}>{state.teams.length ? state.teams.map((team: any) => <div key={team.id} style={{ borderRadius: 14, padding: 9, background: "rgba(255,255,255,.025)" }}><div style={{ display: "flex", justifyContent: "space-between", gap: 8 }}><strong>{team.tag ? `[${team.tag}] ` : ""}{team.name}</strong><button type="button" onClick={() => { deleteEsportsTeam(team.id); refresh(); }} style={{ ...buttonStyle(false), minHeight: 25, padding: "2px 6px" }}>×</button></div></div>) : <Empty text={tr("Aucun cache local.", "No local cache.", "Sin caché local.")} textSoft={textSoft}/>}</div></section>
  </div>;
}
...
function StatsSection({ state, panelStyle, textSoft, tr }: any) {
  ...
  return <div className="esports-section-stack"><section className="esports-kpi-grid">{[[total,tr("Matchs", "Matches", "Partidos")],[wins,tr("Victoires", "Wins", "Victorias")],[total ? `${Math.round(wins/total*100)}%` : "0%",tr("Win rate", "Win rate", "Win rate")],[stats.length,tr("Jeux joués", "Games played", "Juegos jugados")]].map(([v,k]) => <div key={k} style={{ ...panelStyle, padding: 13 }}><div style={{ fontSize: 24, fontWeight: 1000 }}>{v}</div><div style={{ color: textSoft, fontSize: 10 }}>{k}</div></div>)}</section><section className="esports-panel" style={{ ...panelStyle, padding: 14 }}><div style={{ fontSize: 20, fontWeight: 1000 }}>{tr("Performance par jeu", "Performance by game", "Rendimiento por juego")}</div><div style={{ marginTop: 9, display: "grid", gap: 7 }}>{stats.length ? stats.map((row) => { const game = getEsportsGame(row.gameId); const rate = Math.round(row.wins / Math.max(1,row.played)*100); return <div key={row.gameId} className="esports-stat-performance-row"><div className="esports-stat-performance-head"><strong style={{ color: game.accent }}>{game.icon} {game.name}</strong><div style={{ marginTop: 5, height: 4, borderRadius: 99, background: "rgba(255,255,255,.07)", overflow: "hidden" }}><div style={{ width: `${rate}%`, height: "100%", background: game.accent }}/></div></div><div className="esports-stat-performance-kpis"><MiniStat label="P" value={row.played} textSoft={textSoft}/><MiniStat label="W" value={row.wins} textSoft={textSoft}/><MiniStat label="L" value={row.losses} textSoft={textSoft}/><MiniStat label="WR" value={`${rate}%`} textSoft={textSoft}/></div></div>; }) : <Empty text={tr("Enregistre quelques matchs pour alimenter les statistiques.", "Record a few matches to populate statistics.", "Guarda algunos partidos para alimentar las estadísticas.")} textSoft={textSoft}/>}</div></section></div>;
}
