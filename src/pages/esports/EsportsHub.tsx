...
function RoomsSection({ state, refresh, panelStyle, buttonStyle, inputStyle, textSoft, accent, setToast, tr }: any) {
  ...
  return <div style={{ display: "grid", gap: 10 }}>
    ...
    <section className="esports-panel" style={{ ...panelStyle, padding: 14 }}><div style={{ fontWeight: 1000 }}>{tr("Rejoindre un salon Online", "Join an Online room", "Unirse a una sala Online")}</div><div style={{ marginTop: 8, display: "grid", gridTemplateColumns: "1fr auto", gap: 8 }}><input value={joinCode} maxLength={8} onChange={(e) => setJoinCode(e.target.value.toUpperCase())} placeholder="CODE SALON" style={{ ...inputStyle, textTransform: "uppercase", letterSpacing: 3, fontWeight: 1000 }}/><button type="button" disabled={busy} onClick={joinOnline} style={buttonStyle(false)}>{tr("Rejoindre", "Join", "Unirse")}</button></div></section>
...
function ProfileSection({ state, refresh, panelStyle, buttonStyle, inputStyle, textSoft, accent, go, setToast, tr }: any) {
  ...
  return <div style={{ display: "grid", gap: 10 }}>
    <section className="esports-panel" style={{ ...panelStyle, padding: 10 }}><div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(105px,1fr))", gap: 6 }}>{tabs.map(([id, icon, label]) => <button type="button" key={id} onClick={() => setTab(id)} style={{ ...buttonStyle(tab === id), minWidth: 0, padding: "8px 6px" }}>{icon} <span style={{ marginLeft: 3 }}>{label}</span></button>)}</div></section>
    {tab === "profile" ? <>
...
function EsportsFriendsPanel({ panelStyle, buttonStyle, inputStyle, textSoft, setToast, tr }: any) {
  ...
  return <div style={{ display: "grid", gap: 10 }}>
    ...
    <section className="esports-panel" style={{ ...panelStyle, padding: 12 }}><div style={{ display: "flex", justifyContent: "space-between", gap: 8, alignItems: "center" }}><div style={{ fontWeight: 1000 }}>{tr("Mes amis", "My friends", "Mis amigos")} · {friends.length}</div><button type="button" onClick={load} style={{ ...buttonStyle(false), minHeight: 30, padding: "4px 8px" }}>↻</button></div><div style={{ marginTop: 8, display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(210px,1fr))", gap: 7 }}>{friends.length ? friends.map((friend) => { const id = String(friend.userId || friend.id); const status = friend.status === "online" ? "🟢" : friend.status === "away" ? "🟠" : "⚫"; return <div key={id} style={{ borderRadius: 14, padding: 10, background: "rgba(255,255,255,.035)" }}><div style={{ fontWeight: 1000 }}>{status} {friend.displayName || friend.nickname || id}</div><div style={{ marginTop: 3, color: textSoft, fontSize: 9 }}>{friend.countryCode || friend.country || ""} {friend.lastSeenAt ? `· ${friend.lastSeenAt}` : ""}</div></div>; }) : <Empty text={loading ? tr("Chargement…", "Loading…", "Cargando…") : tr("Aucun ami chargé.", "No friends loaded.", "No hay amigos cargados.")} textSoft={textSoft}/>}</div></section>
