// @ts-nocheck
// =============================================================
// HALVE-IT — configuration complète / guidée
...
import type { HalveItConfigPayload, HalveItParticipantMode, HalveItTeamConfig } from "../lib/gameEngines/halveItEngine";
...
export type { HalveItConfigPayload } from "../lib/gameEngines/halveItEngine";

const LS_CFG_KEY = "dc_modecfg_halveit_v4";
const GOLD = "#e4c06b";
...
function interleaveTeams(teams: HalveItTeamConfig[]) {
...
function RulesContent({ primary }: { primary: string }) {
  return <div style={{ display: "grid", gap: 11, fontSize: 13, lineHeight: 1.45 }}>
    <div><strong style={{ color: primary }}>OBJECTIF</strong><br />À chaque manche, tous les joueurs visent la même cible. Seules les fléchettes qui respectent le contrat de la manche ajoutent leurs points.</div>
    <div><strong style={{ color: primary }}>3 FLÉCHETTES</strong><br />Un 20 accepte S20, D20 et T20. Le round DOUBLE accepte n’importe quel double, TRIPLE n’importe quel triple, BULL accepte 25 et 50.</div>
    <div><strong style={{ color: "#ff9aae" }}>HALVE-IT</strong><br />Si aucune des 3 fléchettes ne touche la cible imposée, le score courant est divisé par deux. Par défaut l’arrondi se fait à l’entier inférieur.</div>
    <div><strong style={{ color: "#ffd76a" }}>DÉPART</strong><br />Départ à 0, capital fixe, ou volée libre initiale pour constituer le premier score avant les contrats.</div>
    <div><strong style={{ color: primary }}>VICTOIRE</strong><br />Après la dernière cible, le joueur ou l’équipe avec le total le plus élevé gagne. Une égalité de score reste une égalité.</div>
    <div><strong style={{ color: primary }}>ÉQUIPES</strong><br />Chaque joueur garde son propre score et ses statistiques ; le classement équipe additionne les scores de ses membres.</div>
  </div>;
}
...
export default function HalveItConfig(props: any) {
  ...
  const [configViewMode, setConfigViewMode] = React.useState<"guided" | "complete">(() => { try { return localStorage.getItem("dc_halveit_config_view_mode") === "complete" ? "complete" : "guided"; } catch { return "guided"; } });
  ...
  const selectConfigViewMode = React.useCallback((mode: "guided" | "complete") => { setConfigViewMode(mode); try { localStorage.setItem("dc_halveit_config_view_mode", mode); } catch {} }, []);
  ...
  const [participantMode, setParticipantMode] = React.useState<HalveItParticipantMode>(saved.participantMode === "teams" ? "teams" : "players");
  ...
  const [sequencePreset, setSequencePreset] = React.useState<"classic7" | "extended9" | "expert12" | "numbers7">(
    ["classic7", "extended9", "expert12", "numbers7"].includes(String(saved.sequencePreset)) ? saved.sequencePreset : "classic7"
  );
  const [startMode, setStartMode] = React.useState<"zero" | "fixed" | "opening_visit">(
    saved.startMode === "fixed" || saved.startMode === "opening_visit" ? saved.startMode : "zero"
  );
  const [fixedStartingScore, setFixedStartingScore] = React.useState<number>([20, 40, 50, 100].includes(Number(saved.fixedStartingScore)) ? Number(saved.fixedStartingScore) : 40);
  const [rounding, setRounding] = React.useState<"floor" | "ceil">(saved.rounding === "ceil" ? "ceil" : "floor");
  ...
  React.useEffect(() => {
    try { localStorage.setItem(LS_CFG_KEY, JSON.stringify({ participantMode, teamsSourceMode, selectedIds, teamAssignments, selectedStoredTeamIds, selectedBotTeamIds, savedTeamMemberSelections, botsPanelEnabled, botTeamsPanelEnabled, botLevel, sequencePreset, startMode, fixedStartingScore, rounding, randomOrder, scoreInputMethod, playerDartSets })); } catch {}
  }, [participantMode, teamsSourceMode, selectedIds, teamAssignments, selectedStoredTeamIds, selectedBotTeamIds, savedTeamMemberSelections, botsPanelEnabled, botTeamsPanelEnabled, botLevel, sequencePreset, startMode, fixedStartingScore, rounding, randomOrder, scoreInputMethod, playerDartSets]);
  ...
  function externalTeamConfig(team: any, index: number): HalveItTeamConfig {
  ...
  const activeTeamConfigs: HalveItTeamConfig[] = teamsSourceMode === "manual" ? manualTeamConfigs : savedTeamConfigs;
  ...
  function onStart() {
    ...
    const payload: HalveItConfigPayload = {
      mode: "halve_it", participantMode, players: orderedIds.length, selectedIds: orderedIds,
      ...
      sequencePreset, startMode, fixedStartingScore, rounding, randomOrder, scoreInputMethod,
    };
    try { recordProfileUsageForMode("halve_it", orderedIds); } catch {}
    if (typeof go === "function") go("halve_it_play", payload);
  }
  ...
  const targetSummary = sequencePreset === "classic7" ? "20 · 19 · TRIPLE · 18 · DOUBLE · 17 · BULL" : sequencePreset === "extended9" ? "20 · 19 · TRIPLE · 18 · DOUBLE · 17 · 16 · 15 · BULL" : sequencePreset === "expert12" ? "20 · 19 · TRIPLE · 18 · DOUBLE · 17 · 16 · 15 · 14 · 13 · 12 · BULL" : "20 · 19 · 18 · 17 · 16 · 15 · BULL";
  ...
  const participantsBlock = <>
    <section style={selectorCard}>
      ...
      {participantMode === "players" ? <>
        ...
        <PlayerPagedSelector usageMode="halve_it" profiles={humanProfiles} selectedIds={selectedIds} onToggle={togglePlayer} accent={primary} pageSize={9} modalTitle="Choisir des joueurs" showSelectedSummary={false} />
        <p style={{ fontSize: 11, color: "#7c80a0", marginBottom: 0 }}>1 à 12 profils. Même sélecteur de profils, sets de fléchettes, équipes et bots que X01.</p>
  ...
  const routeBlock = <section style={guidedCard}>
    <h3 style={{ margin: "0 0 8px", color: primary, fontSize: 13, textTransform: "uppercase", letterSpacing: 1 }}>Parcours HALVE-IT</h3>
    <div style={panel}>
      <OptionRow label="Parcours"><OptionSelect value={sequencePreset} options={[
        { value: "classic7", label: "Classique · 7 cibles" },
        { value: "extended9", label: "Étendu · 9 cibles" },
        { value: "numbers7", label: "Débutant · numéros + Bull" },
        { value: "expert12", label: "Expert · 12 cibles" },
      ]} onChange={setSequencePreset} /></OptionRow>
      <div style={{ marginTop: 9, color: themeTextSoft, fontSize: 10.8, lineHeight: 1.45 }}>{targetSummary}</div>
    </div>
  </section>;
  ...
  const rulesBlock = <section style={guidedCard}>
    ...
    <OptionRow label="Score de départ"><OptionSelect value={startMode} options={[
      { value: "zero", label: "0 point · classique simple" },
      { value: "fixed", label: "Capital fixe" },
      { value: "opening_visit", label: "Volée libre initiale" },
    ]} onChange={setStartMode} /></OptionRow>
    {startMode === "fixed" ? <OptionRow label="Capital initial"><OptionSelect value={fixedStartingScore} options={[20, 40, 50, 100]} onChange={(v: any) => setFixedStartingScore(Number(v) || 40)} /></OptionRow> : null}
    <OptionRow label="Arrondi après /2"><OptionSelect value={rounding} options={[{ value: "floor", label: "Inférieur · standard" }, { value: "ceil", label: "Supérieur" }]} onChange={setRounding} /></OptionRow>
    <OptionRow label="Ordre aléatoire"><OptionToggle value={randomOrder} onChange={setRandomOrder} /></OptionRow>
    <div style={{ marginTop: 9, padding: 10, borderRadius: 12, background: `${primary}0d`, border: `1px solid ${primary}30`, color: "#cfd4ea", fontSize: 10.8, lineHeight: 1.45 }}>3 fléchettes par cible. Les touches valides ajoutent leur valeur réelle. 0/3 sur la cible = score courant divisé par deux. La volée libre initiale, si choisie, ne déclenche jamais de division.</div>
  </section>;
  ...
  const inputBlock = <section style={guidedCard}>
    ...
    <div style={{ marginTop: 10, color: themeTextSoft, fontSize: 10.8, lineHeight: 1.45 }}>Chaque impact S/D/T/BULL/DBULL/MISS est conservé : précision par cible, halves, points gagnés/perdus, séries et meilleures volées.</div>
  </section>;
  ...
  const summaryBlock = <section style={{ ...guidedCard, border: `1px solid ${validSelection ? primary + "66" : "rgba(255,255,255,.08)"}` }}>
    ...
    <div style={{ display: "grid", gap: 7, padding: 11, borderRadius: 14, background: `${primary}0c`, border: `1px solid ${primary}33`, fontSize: 11.5 }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", gap: 10 }}><span style={{ color: "#8f94b5" }}>Départ</span><b>{startMode === "opening_visit" ? "Volée libre" : startMode === "fixed" ? `${fixedStartingScore} pts` : "0 pt"}</b></div>
      <div style={{ display: "flex", justifyContent: "space-between", gap: 10 }}><span style={{ color: "#8f94b5" }}>Parcours</span><b style={{ textAlign: "right", maxWidth: "70%" }}>{targetSummary}</b></div>
      <div style={{ display: "flex", justifyContent: "space-between", gap: 10 }}><span style={{ color: "#8f94b5" }}>Halve</span><b>/2 · arrondi {rounding === "floor" ? "inférieur" : "supérieur"}</b></div>
  ...
  return <div style={{ minHeight: "100dvh", width: "100%", maxWidth: "100%", overflowX: "hidden", paddingBottom: 92 }}>
    <PageHeader tickerSrc={tickerHalveIt} tickerAlt="HALVE-IT" left={<BackDot onClick={backToGames} color={primary} glow={`${primary}88`} title="Retour" />} right={<InfoDot title="Règles HALVE-IT" color={theme?.accent1 || primary} glow={`${theme?.accent1 || primary}77`} content={<RulesContent primary={primary} />} />} />
    <div style={{ width: "100%", maxWidth: "100%", minWidth: 0, boxSizing: "border-box", padding: "8px 8px 0", overflowX: "hidden" }}>
      <section style={{ ...selectorCard, border: `1px solid ${primary}66`, boxShadow: `0 0 24px ${primary}18, 0 14px 34px rgba(0,0,0,.48)` }}>
        <div style={{ color: primary, fontSize: 12, fontWeight: 950, textTransform: "uppercase", letterSpacing: 1, marginBottom: 8 }}>Configuration HALVE-IT</div>
