...
export default function FootConfig({ go, params, store }: Props) {
  ...
  function replaceAutoGeneratedTeamSelection(teamIdsRaw: any[], selectionsRaw: Record<string, string[]>) {
    const ids = (Array.isArray(teamIdsRaw) ? teamIdsRaw : []).map((id: any) => String(id || "")).filter(Boolean).slice(0, 2);
    const selections: Record<string, string[]> = {};
    for (const id of ids) {
      selections[id] = Array.from(new Set((selectionsRaw?.[id] || []).map((pid: any) => String(pid || "").trim()).filter(Boolean))).slice(0, spec.playersPerSide);
    }
    setSourceMode("saved");
    setSelectedTeamIds(ids);
    setSelectedTeamPlayerIds(selections);
  }
  ...
  const TeamsChoice = () => (
    <section style={cardStyle()}>
      ...
      <TeamSelectorV2 teams={savedTeams} selectedTeamIds={selectedTeamIds} selectedTeamPlayerIds={selectedTeamPlayerIds} onAdd={addSavedTeamSelection} onRemove={removeSavedTeamSelection} profilesById={profileById} primary={primary} primarySoft={primarySoft} maxPlayers={spec.playersPerSide} maxSelections={2} autoShufflePlayers={profiles} sport="foot" onReplaceSelectedTeams={replaceAutoGeneratedTeamSelection} />
    </section>
  );
