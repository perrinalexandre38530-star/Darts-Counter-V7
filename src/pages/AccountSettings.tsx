...
export default function ScramPlay(props: any) {
  ...
  const teamPlayers = useMemo(() => {
    // SCRAM: on garde un rendu "Cricket" (2 équipes), mais le moteur SCRAM n'expose pas teams/teamByPlayer.
    // Convention: index pair = Team A, index impair = Team B.
    const list = (state?.players ?? players ?? []) as any[];
    const A: any[] = [];
    const B: any[] = [];
    list.forEach((p, i) => {
      if (!p) return;
  ...
  const activeTeam: "A" | "B" = (Number(state?.activePlayerIndex ?? 0) % 2 === 0 ? "A" : "B");
