...
const MODE_META: Record<ModeKind, { title: string; accent: string; subtitle: string; primary: string; secondary: string; tertiary: string }> = {
  ...
  darts_racer: {
    title: "DARTS RACER",
    accent: "#42d6ff",
    subtitle: "Course arcade : distance, vitesse, boosts, attaques, boucliers, pièges et collisions.",
    primary: "Distance",
    secondary: "Précision",
    tertiary: "Best volée",
  },
  ...
  darts_firefighter: {
    ...
    accent: "#ff6b27",
    subtitle: "Intervention coopérative : extinction, protections, propagation, zones critiques et soutien Canadair.",
    primary: "Feu réduit",
...
function valueFor(mode: ModeKind, key: "primary" | "secondary" | "tertiary", row: any, rank: number): number | string {
  ...
  if (mode === "darts_racer") {
    if (key === "primary") {
      const distance = num(pick(row.position, row.distance, row.finalPosition, row.netDistance), 0);
      const total = num(pick(row.totalDistance, row.trackDistance), 0);
      return total > 0 ? `${distance}/${total}` : distance;
    }
    if (key === "secondary") {
      const darts = num(pick(row.darts, row.dartsThrown), 0);
    ...
    return `+${num(row.bestVisitDistance, 0)}`;
  }
  if (mode === "attrape_moi") {
...
export default function DartsModeSummaryPage({ go, params }: Props) {
  ...
  const racerSummary = mode === "darts_racer" ? (pick(rec?.summary, rec?.payload?.summary, {}) || {}) : {};
  const racerMatchStats = mode === "darts_racer" ? (pick(racerSummary?.matchStats, rec?.payload?.stats?.match, rec?.payload?.stats?.global, {}) || {}) : {};
  ...
  const totalDarts = mode === "bobs_27"
    ...
    : mode === "shooter"
      ...
      : mode === "darts_racer"
        ? num(racerMatchStats?.totalDarts, rows.reduce((sum, r) => sum + num(pick(r.raw?.darts, r.raw?.dartsThrown, r.raw?.totalThrows), 0), 0))
      : mode === "attrape_moi"
  ...
  const totalActions = mode === "capital"
    ...
    : mode === "bobs_27"
      ...
      : mode === "shooter"
        ...
        : mode === "darts_racer"
          ? num(racerMatchStats?.totalNetDistance, rows.reduce((sum, r) => sum + num(pick(r.raw?.netDistance, r.raw?.distance, r.raw?.position), 0), 0))
        : mode === "attrape_moi"
  ...
  return (
    <div style={pageStyle}>
      ...
      {mode === "five_lives" ? <FiveLivesSummaryTables rec={rec} rows={rows} accent={meta.accent} /> : mode === "scram" ? (
        ...
        <DartsRacerSummaryTables rec={rec} accent={meta.accent} />
      ) : mode === "attrape_moi" ? (
...
function DartsFirefighterSummaryTables({ rec, rows, accent }: { rec: any; rows: any[]; accent: string }) {
  const summary = pick(rec?.summary, rec?.payload?.summary, {}) || {};
  const match = pick(summary?.matchStats, rec?.payload?.stats?.match, rec?.payload?.stats?.global, {}) || {};
  const territories = asArray(pick(summary?.finalTerritories, rec?.payload?.finalTerritories));
  const visits = asArray(pick(summary?.visits, rec?.payload?.visits, rec?.payload?.visitHistory));
  const won = Boolean(pick(summary?.won, rec?.payload?.won, false));
  const score = num(pick(summary?.score, match?.score, rec?.payload?.state?.score), 0);
  const durationMs = num(pick(summary?.durationMs, match?.durationMs), 0);
  const duration = durationMs > 0 ? (() => { const sec = Math.round(durationMs / 1000); return `${Math.floor(sec / 60)}:${String(sec % 60).padStart(2, "0")}`; })() : "—";
  const statusColor = (zone: any) => zone?.destroyed ? "#30343c" : num(zone?.fireLevel) >= 3 ? "#ff343f" : num(zone?.fireLevel) === 2 ? "#ff6b27" : num(zone?.fireLevel) === 1 ? "#ffb13c" : zone?.smoke ? "#a9afb9" : num(zone?.protection) > 0 ? "#25c9ff" : "#5ce6a8";
  const difficulty = ({ recruit: "Recrue", firefighter: "Pompier", commander: "Commandant", inferno: "Enfer" } as any)[String(summary?.difficulty || rec?.payload?.config?.difficulty)] || String(summary?.difficulty || rec?.payload?.config?.difficulty || "—");
  return <>
    <section style={card(accent)}>
      <div style={sectionTitle(accent)}>Bilan de l’intervention</div>
      <div style={{ textAlign: "center", padding: 12, marginBottom: 10, borderRadius: 15, background: won ? "rgba(92,230,168,.08)" : "rgba(255,76,85,.08)", border: `1px solid ${won ? "#5ce6a8" : "#ff4c55"}55` }}>
        <div style={{ color: won ? "#5ce6a8" : "#ff4c55", fontWeight: 1000, fontSize: 18 }}>{won ? "✅ MISSION RÉUSSIE" : "🚨 MISSION ÉCHOUÉE"}</div>
        <div style={{ color: "#ffd76a", fontWeight: 1000, fontSize: 25, marginTop: 3 }}>{score} POINTS</div>
        <div style={{ color: "#aeb4c1", fontSize: 11 }}>{String(summary?.mapId || rec?.payload?.config?.mapId || "Carte")} · {difficulty} · {duration}</div>
      </div>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4,minmax(0,1fr))", gap: 7 }}>
        <Kpi label="Feux éteints" value={num(pick(summary?.totalExtinguished, match?.totalExtinguished))} accent="#5ce6a8" />
        <Kpi label="Feu réduit" value={num(match?.totalFireReduced)} accent="#25c9ff" />
        <Kpi label="Zones perdues" value={num(pick(summary?.totalDestroyed, match?.totalDestroyed))} accent="#ff4c55" />
        <Kpi label="Propagations" value={num(pick(summary?.totalSpread, match?.totalSpread))} accent="#ff6b27" />
        <Kpi label="Blocages" value={num(pick(summary?.propagationBlocked, match?.propagationBlocked))} accent="#25c9ff" />
        <Kpi label="Protections" value={num(match?.protectionsPlaced)} accent="#25c9ff" />
        <Kpi label="Canadairs" value={num(match?.canadairs)} accent="#ffd76a" />
        <Kpi label="Précision" value={`${num(match?.accuracy).toFixed(1)}%`} accent="#5ce6a8" />
      </div>
    </section>

    <section style={card(accent)}>
      <div style={sectionTitle(accent)}>Contribution de la brigade</div>
      <div style={{ overflowX: "auto" }}><table style={{ width: "100%", minWidth: 980, borderCollapse: "collapse", fontSize: 11 }}>
        <thead><tr style={{ color: accent, textAlign: "left" }}><th style={th}>Pompier</th><th style={th}>Score</th><th style={th}>Feu réduit</th><th style={th}>Extinctions</th><th style={th}>Blocages</th><th style={th}>Protections</th><th style={th}>Eau</th><th style={th}>Préc.</th><th style={th}>S/D/T</th><th style={th}>Bull/DB</th><th style={th}>MISS</th><th style={th}>Best</th></tr></thead>
        <tbody>{rows.map((row: any) => { const p = row.raw || {}; const darts = num(p.darts); const hits = num(p.hits); const accuracy = num(p.accuracy, darts > 0 ? (hits / darts) * 100 : 0); return <tr key={`firefighter-${row.id}`}><td style={td}>{row.name}</td><td style={td}>{num(p.score)}</td><td style={td}>{num(p.fireReduced)}</td><td style={td}>{num(p.firesExtinguished)}</td><td style={td}>{num(p.propagationBlocked)}</td><td style={td}>{num(p.protectionsPlaced)}</td><td style={td}>{num(p.waterApplied)}</td><td style={td}>{accuracy.toFixed(1)}%</td><td style={td}>{num(p.singles)}/{num(p.doubles)}/{num(p.triples)}</td><td style={td}>{num(p.bulls)}/{num(p.dbulls)}</td><td style={td}>{num(p.misses)}</td><td style={td}>{num(p.bestVisitScore)}</td></tr>; })}</tbody>
      </table></div>
    </section>

    {territories.length ? <section style={card(accent)}>
      <div style={sectionTitle(accent)}>État final du territoire</div>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(135px,1fr))", gap: 7 }}>{[...territories].sort((a,b) => num(a?.target)-num(b?.target)).map((zone: any) => { const color = statusColor(zone); const label = zone?.destroyed ? "DÉTRUIT" : num(zone?.fireLevel) > 0 ? `FEU N${num(zone.fireLevel)}` : zone?.smoke ? "FUMÉE" : num(zone?.protection) > 0 ? `PROTÉGÉ ${num(zone.protection)}` : "SAIN"; return <div key={String(zone?.id || zone?.target)} style={{ padding: 9, borderRadius: 13, background: `${color}0d`, border: `1px solid ${color}44` }}><div style={{ display: "flex", justifyContent: "space-between" }}><strong style={{ color: "#ffd76a" }}>{num(zone?.target)}</strong><span>{zone?.critical ? "⚠️" : ""}</span></div><div style={{ marginTop: 3, fontWeight: 1000, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{String(zone?.name || zone?.id || "Zone")}</div><div style={{ marginTop: 2, color, fontSize: 10, fontWeight: 950 }}>{label}</div></div>; })}</div>
    </section> : null}

    <section style={card(accent)}>
      <div style={sectionTitle(accent)}>Journal des volées</div>
      <div style={{ display: "grid", gap: 7 }}>{visits.length ? [...visits].reverse().slice(0, 40).map((visit: any, index: number) => { const row = rows.find((r:any) => String(r.id) === String(visit?.playerId)); return <div key={String(visit?.id || index)} style={{ padding: 9, borderRadius: 13, background: "rgba(255,255,255,.035)", border: "1px solid rgba(255,255,255,.08)" }}><div style={{ display: "flex", justifyContent: "space-between", gap: 8 }}><strong>{row?.name || visit?.playerName || "Pompier"} · R{num(visit?.round,1)} · {asArray(visit?.labels).join(" / ") || "—"}</strong><strong style={{ color: num(visit?.score) >= 0 ? "#5ce6a8" : "#ff4c55" }}>{num(visit?.score) >= 0 ? "+" : ""}{num(visit?.score)}</strong></div><div style={{ color: "#9fa5b2", fontSize: 10, marginTop: 3 }}>{asArray(visit?.events).slice(-4).map((event:any) => event?.label).filter(Boolean).join(" · ") || "Aucun effet"}</div></div>; }) : <div style={{ color: "#c9c9d4" }}>Aucune volée enregistrée.</div>}</div>
    </section>
  </>;
}

function AttrapeMoiSummaryTables({ rec, accent }: { rec: any; accent: string }) {
  const [tab, setTab] = React.useState<"resume" | "camps" | "players" | "roles" | "legs" | "charts">("resume");
  const summary = pick(rec?.summary, rec?.payload?.summary, {}) || {};
  ...
  const totalPoints = num(matchStats?.totalPoints, players.reduce((sum,p)=>sum+num(p?.points??p?.score),0));
  const totalVisits = num(matchStats?.totalVisits, players.reduce((sum,p)=>sum+num(p?.visits),0));
  const avg3 = num(matchStats?.avg3, totalDarts ? (totalPoints/totalDarts)*3 : 0);
  const hitRate = num(matchStats?.hitRate, totalDarts ? ((totalDarts-players.reduce((sum,p)=>sum+num(p?.misses),0))/totalDarts)*100 : 0);
  const bestVisit = num(matchStats?.bestVisit, players.reduce((best,p)=>Math.max(best,num(p?.bestVisit)),0));
  const fmtDuration = (ms: number) => { const sec = Math.max(0, Math.round(ms / 1000)); return `${Math.floor(sec / 60)}:${String(sec % 60).padStart(2, "0")}`; };
  const tabs = [["resume","RÉSUMÉ"],["camps","CAMPS"],["players","JOUEURS"],["roles","RÔLES"],["legs","MANCHES"],["charts","GRAPHIQUES"]] as const;
  const btn = (active:boolean,color:string):React.CSSProperties => ({minHeight:34,padding:"0 11px",borderRadius:999,border:`1px solid ${active?color:"rgba(255,255,255,.12)"}`,background:active?`${color}18`:"rgba(255,255,255,.035)",color:active?color:"rgba(255,255,255,.65)",fontWeight:1000,fontSize:9,whiteSpace:"nowrap"});
  const roleColor=(id:string)=>id==="roles"?"#ff5d9e":id==="legs"?"#42d6ff":accent;
  const barRows=(rows:Array<{label:string,value:number,color:string}>)=>{const max=Math.max(1,...rows.map(x=>x.value));return <div style={{display:"grid",gap:7}}>{rows.map(x=><div key={x.label} style={{display:"grid",gridTemplateColumns:"90px 1fr 42px",gap:7,alignItems:"center"}}><span style={{fontSize:10,color:"#aeb2bf"}}>{x.label}</span><div style={{height:9,borderRadius:999,background:"rgba(255,255,255,.06)",overflow:"hidden"}}><div style={{height:"100%",width:`${x.value/max*100}%`,background:x.color,borderRadius:999}}/></div><b style={{textAlign:"right",fontSize:11}}>{Math.round(x.value*10)/10}</b></div>)}</div>};
  const lineChart=(values:number[],color:string,label:string)=>{const clean=values.map(Number).filter(Number.isFinite);if(!clean.length)return null;const w=520,h=110,p=12,min=Math.min(0,...clean),max=Math.max(1,...clean),span=max-min||1;const pts=clean.map((v,i)=>({x:p+(clean.length===1?0:i/(clean.length-1)*(w-p*2)),y:h-p-((v-min)/span)*(h-p*2)}));const d=pts.map((q,i)=>`${i?"L":"M"}${q.x},${q.y}`).join(" ");return <div style={{padding:10,borderRadius:14,background:"rgba(255,255,255,.03)",border:"1px solid rgba(255,255,255,.08)"}}><div style={{fontSize:10,color, fontWeight:1000,marginBottom:4}}>{label}</div><svg viewBox={`0 0 ${w} ${h}`} style={{width:"100%",height:95,display:"block"}}><line x1={p} y1={h-p} x2={w-p} y2={h-p} stroke="rgba(255,255,255,.12)"/><path d={d} fill="none" stroke={color} strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"/>{pts.map((q,i)=><circle key={i} cx={q.x} cy={q.y} r="3" fill={color}/>)}</svg></div>};

  return <>
    <section style={card(accent)}>
      <div style={{display:"flex",gap:6,overflowX:"auto",paddingBottom:2}}>{tabs.map(([id,label])=><button key={id} onClick={()=>setTab(id as any)} style={btn(tab===id,roleColor(id))}>{label}</button>)}</div>
    </section>

    {tab === "resume" ? <section style={card(accent)}>
      <div style={sectionTitle(accent)}>Vue complète de la poursuite</div>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(125px,1fr))", gap: 8 }}>
        <Kpi label="Score cumulé" value={totalPoints} accent="#ffd76a" />
        <Kpi label="AVG/3 match" value={avg3.toFixed(1)} accent={accent} />
        <Kpi label="Best volée" value={bestVisit || "—"} accent="#ffd76a" />
        <Kpi label="Précision" value={`${Math.round(hitRate*10)/10}%`} accent="#f5f7fb" />
        <Kpi label="Avance Fuyard" value={`${headStart} pts`} accent="#ff5d9e" />
        <Kpi label="Rounds max" value={pursuitRounds} accent="#42d6ff" />
        ...
        <Kpi label="Flèches / Volées" value={`${totalDarts} / ${totalVisits}`} accent="#f5f7fb" />
        <Kpi label="Pts / volée" value={totalVisits ? (totalPoints/totalVisits).toFixed(1) : "—"} accent="#ffd76a" />
        <Kpi label="Capture + rapide" value={matchStats?.fastestCaptureRound ? `R${num(matchStats.fastestCaptureRound)}` : "—"} accent="#42d6ff" />
        <Kpi label="Capture moyenne" value={matchStats?.avgCaptureRound ? `R${num(matchStats.avgCaptureRound).toFixed(1)}` : "—"} accent="#42d6ff" />
        <Kpi label="Durée" value={duration ? fmtDuration(duration) : "—"} accent={accent} />
      </div>
    </section> : null}

    {tab === "camps" && entities.length ? <section style={card(accent)}>
      <div style={sectionTitle(accent)}>Comparaison des camps</div>
      <div style={{display:"grid",gap:9}}>{entities.slice().sort((a:any,b:any)=>num(b?.winner)-num(a?.winner)).map((e:any,index:number)=>{const score=num(e?.totalPoints??e?.points??e?.score,num(e?.runnerPoints)+num(e?.chaserPoints));return <div key={String(e?.id||index)} style={{padding:11,borderRadius:15,background:"rgba(255,255,255,.035)",border:`1px solid ${e?.winner?"#ffd76a":"rgba(255,255,255,.09)"}`}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:8}}><b style={{fontSize:16}}>{String(e?.name||`Camp ${index+1}`)}{e?.winner?" 🏆":""}</b><b style={{color:"#ffd76a",fontSize:22}}>{score} pts</b></div><div style={{marginTop:8,display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(100px,1fr))",gap:6}}><Kpi label="Sets" value={num(e?.setWins??e?.setsWon)} accent="#ffd76a"/><Kpi label="Manches" value={num(e?.legsWon)} accent={accent}/><Kpi label="Victoires Fuyard" value={num(e?.runnerLegWins)} accent="#ff5d9e"/><Kpi label="Victoires Chasseur" value={num(e?.chaserLegWins)} accent="#42d6ff"/><Kpi label="Captures" value={num(e?.captures)} accent="#42d6ff"/><Kpi label="Évasions" value={num(e?.escapes)} accent="#ff5d9e"/><Kpi label="Capture + rapide" value={e?.fastestCaptureRound?`R${num(e.fastestCaptureRound)}`:"—"} accent="#42d6ff"/><Kpi label="Capture moy." value={e?.avgCaptureRound?`R${num(e.avgCaptureRound).toFixed(1)}`:"—"} accent="#42d6ff"/><Kpi label="Distance max" value={num(e?.maxRunnerLead)} accent="#ff5d9e"/><Kpi label="Évasion max" value={num(e?.bestEscapeLead)} accent="#ff5d9e"/><Kpi label="Plus proche" value={e?.closestChaseGap??"—"} accent="#42d6ff"/><Kpi label="Taux rôle" value={`${num(e?.roleWinRate).toFixed(1)}%`} accent={accent}/></div></div>})}</div>
    </section> : null}

    {tab === "players" ? <section style={card(accent)}><div style={sectionTitle(accent)}>Statistiques joueurs</div><div style={{overflowX:"auto"}}><table style={{width:"100%",minWidth:1260,borderCollapse:"collapse",fontSize:11}}><thead><tr style={{color:accent,textAlign:"left"}}>{["Joueur","Score","Sets","Manches","AVG/3","Best","Pts/dart","Pts/volée","Préc.","AVG Fuyard","Best F.","AVG Chasseur","Best C.","Captures","Évasions","Darts","Volées","60+","100+","140+","180"].map(h=><th key={h} style={th}>{h}</th>)}</tr></thead><tbody>{players.map((p:any,index:number)=><tr key={String(p?.id||index)}><td style={td}>{String(p?.name||p?.playerName||`Joueur ${index+1}`)}{p?.winner?" 🏆":""}</td><td style={td}>{num(p?.points??p?.score)}</td><td style={td}>{num(p?.setsWon)}</td><td style={td}>{num(p?.legsWon)}</td><td style={td}>{num(p?.avg3).toFixed(1)}</td><td style={td}>{num(p?.bestVisit)}</td><td style={td}>{num(p?.pointsPerDart).toFixed(1)}</td><td style={td}>{num(p?.pointsPerVisit).toFixed(1)}</td><td style={td}>{num(p?.hitRate).toFixed(1)}%</td><td style={td}>{num(p?.runnerAvg3).toFixed(1)}</td><td style={td}>{num(p?.runnerBestVisit)}</td><td style={td}>{num(p?.chaserAvg3).toFixed(1)}</td><td style={td}>{num(p?.chaserBestVisit)}</td><td style={td}>{num(p?.captureCredits)}</td><td style={td}>{num(p?.escapeCredits)}</td><td style={td}>{num(p?.darts??p?.dartsThrown)}</td><td style={td}>{num(p?.visits)}</td><td style={td}>{num(p?.visits60Plus)}</td><td style={td}>{num(p?.visits100Plus)}</td><td style={td}>{num(p?.visits140Plus)}</td><td style={td}>{num(p?.visits180)}</td></tr>)}</tbody></table></div></section> : null}

    {tab === "roles" ? <section style={card(accent)}><div style={sectionTitle(accent)}>Fuyard vs Chasseur</div><div style={{display:"grid",gap:9}}>{players.map((p:any,index:number)=><div key={String(p?.id||index)} style={{padding:10,borderRadius:14,background:"rgba(255,255,255,.035)",border:"1px solid rgba(255,255,255,.08)"}}><div style={{display:"flex",justifyContent:"space-between",gap:8}}><b>{String(p?.name||`Joueur ${index+1}`)}</b><span style={{color:"#ffd76a",fontWeight:1000}}>{num(p?.points)} pts</span></div><div style={{marginTop:8,display:"grid",gridTemplateColumns:"repeat(2,minmax(0,1fr))",gap:7}}><div style={{padding:9,borderRadius:12,background:"rgba(255,93,158,.055)",border:"1px solid rgba(255,93,158,.22)"}}><b style={{color:"#ff5d9e"}}>FUYARD</b><div style={{marginTop:6,display:"grid",gridTemplateColumns:"repeat(3,minmax(0,1fr))",gap:5}}><Kpi label="AVG/3" value={num(p?.runnerAvg3).toFixed(1)} accent="#ff5d9e"/><Kpi label="Best" value={num(p?.runnerBestVisit)} accent="#ffd76a"/><Kpi label="Points" value={num(p?.runnerPoints)} accent="#fff"/></div></div><div style={{padding:9,borderRadius:12,background:"rgba(66,214,255,.055)",border:"1px solid rgba(66,214,255,.22)"}}><b style={{color:"#42d6ff"}}>CHASSEUR</b><div style={{marginTop:6,display:"grid",gridTemplateColumns:"repeat(3,minmax(0,1fr))",gap:5}}><Kpi label="AVG/3" value={num(p?.chaserAvg3).toFixed(1)} accent="#42d6ff"/><Kpi label="Best" value={num(p?.chaserBestVisit)} accent="#ffd76a"/><Kpi label="Points" value={num(p?.chaserPoints)} accent="#fff"/></div></div></div></div>)}</div></section> : null}

    {tab === "legs" && legs.length ? <section style={card(accent)}><div style={sectionTitle(accent)}>Frise des manches</div><div style={{display:"grid",gap:6}}>{legs.map((r:any,index:number)=><div key={String(r?.globalLegNo||index)} style={{display:"grid",gridTemplateColumns:"70px minmax(0,1fr) auto auto",gap:8,alignItems:"center",padding:9,borderRadius:12,background:"rgba(255,255,255,.04)",border:"1px solid rgba(255,255,255,.08)"}}><div style={{color:accent,fontWeight:1000}}>S{num(r?.setNo,1)} · M{num(r?.legNo,index+1)}</div><div><div style={{fontWeight:1000}}>{String(r?.winnerName||r?.winnerEntityId||"—")} · {String(r?.reason)==="capture"?"💥 CAPTURE":"🏁 ÉVASION"}</div><div style={{color:"#9da2b4",fontSize:10}}>{String(r?.runnerName||"Fuyard")} {num(r?.runnerScore)} — {num(r?.chaserScore)} {String(r?.chaserName||"Chasseur")}</div></div><div style={{color:String(r?.reason)==="capture"?"#42d6ff":"#ff5d9e",fontWeight:1000}}>R{num(r?.pursuitRound)}</div><div style={{color:"#ffd76a",fontWeight:1000}}>{num(r?.finalDistance)>0?`+${num(r.finalDistance)}`:num(r?.finalDistance)}</div></div>)}</div></section> : null}

    {tab === "charts" ? <section style={card(accent)}><div style={sectionTitle(accent)}>Graphiques du match</div><div style={{display:"grid",gap:10}}>{lineChart(legs.map((r:any)=>num(r?.finalDistance)),"#42d6ff","Écart final par manche")}{lineChart(legs.map((r:any)=>num(r?.pursuitRound)),"#ff5d9e","Round de fin par manche")}<div style={{padding:10,borderRadius:14,background:"rgba(255,255,255,.03)",border:"1px solid rgba(255,255,255,.08)"}}><b style={{color:"#ffd76a"}}>Répartition des impacts</b><div style={{marginTop:9}}>{barRows([["Simple",players.reduce((a,p)=>a+num(p?.singles),0),"#fff"],["Double",players.reduce((a,p)=>a+num(p?.doubles),0),"#42d6ff"],["Triple",players.reduce((a,p)=>a+num(p?.triples),0),"#c967ff"],["Bull",players.reduce((a,p)=>a+num(p?.bulls),0),"#65efb4"],["DBull",players.reduce((a,p)=>a+num(p?.dbulls),0),"#ffd76a"],["Miss",players.reduce((a,p)=>a+num(p?.misses),0),"#ff667e"]].map(([label,value,color]:any)=>({label,value,color})))}</div></div><div style={{padding:10,borderRadius:14,background:"rgba(255,255,255,.03)",border:"1px solid rgba(255,255,255,.08)"}}><b style={{color:accent}}>Distribution des volées</b><div style={{marginTop:9}}>{barRows([["0",players.reduce((a,p)=>a+num(p?.zeroVisits),0),"#ff667e"],["1-39",players.reduce((a,p)=>a+num(p?.score1_39),0),"#fff"],["40-59",players.reduce((a,p)=>a+num(p?.score40_59),0),"#ffd76a"],["60-99",players.reduce((a,p)=>a+num(p?.score60_99),0),"#42d6ff"],["100-139",players.reduce((a,p)=>a+num(p?.score100_139),0),"#c967ff"],["140-179",players.reduce((a,p)=>a+num(p?.score140_179),0),"#ff5d9e"],["180",players.reduce((a,p)=>a+num(p?.score180),0),"#ffd76a"]].map(([label,value,color]:any)=>({label,value,color})))}</div></div></div></section> : null}
  </>;
}

function DartsRacerSummaryTables({ rec, accent }: { rec: any; accent: string }) {
  const summary = pick(rec?.summary, rec?.payload?.summary, {}) || {};
  const matchStats = pick(summary?.matchStats, rec?.payload?.stats?.match, rec?.payload?.stats?.global, {}) || {};
  const players = asArray(pick(rec?.payload?.stats?.players, summary?.players, summary?.perPlayer, rec?.payload?.players, rec?.players));
  ...
  const totalDarts = num(matchStats?.totalDarts, players.reduce((sum, p) => sum + num(p?.darts ?? p?.dartsThrown), 0));
  const totalHits = num(matchStats?.totalHits, players.reduce((sum, p) => sum + num(p?.hits), 0));
  const accuracy = num(matchStats?.accuracy, totalDarts ? (totalHits / totalDarts) * 100 : 0);
  const distance = num(pick(summary?.totalDistance, matchStats?.totalDistance, rec?.payload?.state?.totalDistance), 0);
  const rounds = num(pick(summary?.roundsPlayed, matchStats?.roundsPlayed), 0);
  const duration = num(pick(matchStats?.durationMs, summary?.durationMs, summary?.duration), 0);
  const style = String(pick(summary?.raceStyle, rec?.payload?.config?.raceStyle, "arcade")).toUpperCase();
  const fmtDuration = (ms: number) => { const sec = Math.max(0, Math.round(ms / 1000)); return `${Math.floor(sec / 60)}:${String(sec % 60).padStart(2, "0")}`; };
  const sums = (key: string) => players.reduce((sum, p) => sum + num(p?.[key]), 0);

  return <>
    <section style={card(accent)}>
      <div style={sectionTitle(accent)}>Performance DARTS RACER</div>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(125px,1fr))", gap: 8 }}>
        <Kpi label="Style" value={style} accent={accent} />
        <Kpi label="Piste" value={`${distance || "—"} cases`} accent={accent} />
        <Kpi label="Tours" value={num(summary?.laps, 1)} accent="#ffd76a" />
        <Kpi label="Rounds" value={rounds || "—"} accent={accent} />
        <Kpi label="Précision" value={`${Math.round(accuracy * 10) / 10}%`} accent="#65efb4" />
        <Kpi label="Touches" value={`${totalHits}/${totalDarts}`} accent={accent} />
        <Kpi label="Boosts piste" value={sums("specialBoosts")} accent="#ff63b8" />
        <Kpi label="Attaques" value={sums("attacksLanded")} accent="#ff63b8" />
        <Kpi label="Boucliers" value={sums("shieldsPicked")} accent="#ffd76a" />
        <Kpi label="Pièges" value={sums("hazards")} accent="#ff7c93" />
        <Kpi label="Collisions" value={sums("collisions")} accent={accent} />
        <Kpi label="Durée" value={duration ? fmtDuration(duration) : "—"} accent={accent} />
      </div>
    </section>

    {standings.length ? <section style={card(accent)}>
      <div style={sectionTitle(accent)}>Classement de la course</div>
      <div style={{ display: "grid", gap: 7 }}>
        {standings.slice().sort((a:any,b:any)=>num(a?.rank,99)-num(b?.rank,99)).map((row: any, index: number) => {
          const team = teams.find((t: any) => String(t?.id) === String(row?.id));
          return <div key={String(row?.id || index)} style={{ display: "grid", gridTemplateColumns: "34px minmax(0,1fr) auto", gap: 8, alignItems: "center", padding: 9, borderRadius: 13, background: index === 0 ? `${accent}18` : "rgba(255,255,255,.04)", border: `1px solid ${index === 0 ? accent : "rgba(255,255,255,.09)"}` }}>
            <div style={{ color: index === 0 ? "#ffd76a" : "#d7d9e3", fontWeight: 1000 }}>#{num(row?.rank, index + 1)}</div>
            <div><div style={{ fontWeight: 1000 }}>{String(row?.name || team?.name || `Kart ${index + 1}`)}{index === 0 ? " 🏁" : ""}</div><div style={{ color: "#9da2b4", fontSize: 10.5 }}>Tour {num(row?.lap, 1)}/{num(summary?.laps, 1)} • {num(row?.progressPct)}%</div></div>
            <div style={{ color: accent, fontWeight: 1000, fontSize: 18 }}>{num(row?.position)}/{distance || "?"}</div>
          </div>;
        })}
      </div>
    </section> : null}
    ...
    <section style={card(accent)}>
      ...
      <div style={{ overflowX: "auto" }}><table style={{ width: "100%", minWidth: 1160, borderCollapse: "collapse", fontSize: 11.5 }}>
        <thead><tr style={{ color: accent, textAlign: "left" }}>{["Joueur","Rang","Case","Préc.","Darts","Distance","Best","D","T","BULL","DBULL","Boost","Att.","Shield","Piège","Coll.","Leader"].map((h) => <th key={h} style={th}>{h}</th>)}</tr></thead>
        <tbody>{players.slice().sort((a:any,b:any)=>num(a?.rank,99)-num(b?.rank,99)).map((p:any,index:number)=>{const darts=num(p?.darts??p?.dartsThrown);const hits=num(p?.hits);const acc=num(p?.accuracy,darts?(hits/darts)*100:0);return <tr key={String(p?.id||index)}><td style={td}>{String(p?.name||p?.playerName||`Joueur ${index+1}`)}{p?.winner?" 🏆":""}</td><td style={td}>{num(p?.rank,index+1)}</td><td style={td}>{num(p?.position??p?.distance)}/{distance||"?"}</td><td style={td}>{Math.round(acc*10)/10}%</td><td style={td}>{darts}</td><td style={td}>{num(p?.netDistance)}</td><td style={td}>+{num(p?.bestVisitDistance)}</td><td style={td}>{num(p?.doubles)}</td><td style={td}>{num(p?.triples)}</td><td style={td}>{num(p?.bulls)}</td><td style={td}>{num(p?.dbulls)}</td><td style={td}>{num(p?.specialBoosts)}</td><td style={td}>{num(p?.attacksLanded)}</td><td style={td}>{num(p?.shieldsPicked)}</td><td style={td}>{num(p?.hazards)}</td><td style={td}>{num(p?.collisions)}</td><td style={td}>{num(p?.leadVisits)}</td></tr>})}</tbody>
      </table></div>
    </section>

    <section style={card(accent)}>
      <div style={sectionTitle(accent)}>Impacts & interactions</div>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(125px,1fr))", gap: 8 }}>
        <Kpi label="Singles" value={sums("singles")} accent={accent} />
        <Kpi label="Doubles" value={sums("doubles")} accent={accent} />
        <Kpi label="Triples" value={sums("triples")} accent="#ff63b8" />
        <Kpi label="BULL" value={sums("bulls")} accent="#ffd76a" />
        <Kpi label="DBULL" value={sums("dbulls")} accent="#ffd76a" />
        <Kpi label="MISS" value={sums("misses")} accent="#ff7c93" />
        <Kpi label="Bonus distance" value={`+${sums("bonusDistance")}`} accent="#65efb4" />
        <Kpi label="Distance perdue" value={`−${sums("penaltyDistance")}`} accent="#ff7c93" />
        <Kpi label="Attaque infligée" value={sums("attackDistance")} accent="#ff63b8" />
