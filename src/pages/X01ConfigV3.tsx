...
type MatchVictoryModeV3 = "best_of" | "first_to";
type MatchFormatUnitV3 = "sets" | "legs";
...
function computeX01VariantForConfig(participantMode: ParticipantMode, matchMode: MatchModeV3, totalPlayers: number): "solo" | "duo" | "multi" | "team" {
  if (participantMode === "teams" || matchMode === "teams") return "team";
  if (matchMode === "multi" || totalPlayers > 2) return "multi";
  if (totalPlayers === 2) return "duo";
  return "solo";
}
...
function x01AutoBalanceLevelForProfile(profile: any, statsById: Record<string, any> = {}): number {
  const clamp = (value: number) => Math.max(0.5, Math.min(5, Math.round(value * 2) / 2));
  const star = x01ProfileStarRenderData(profile, statsById);
  if (star?.level && Number.isFinite(Number(star.level))) return clamp(Number(star.level));
  if (star?.avg3d && Number.isFinite(Number(star.avg3d))) return clamp(Number(star.avg3d) / 20);
  const fallback = playerBalanceLevel(profile);
  return fallback > 0 ? clamp(fallback) : 2.5;
}
...
export default function X01ConfigV3({ profiles, activeProfileId: activeProfileIdProp = null, onBack, onStart, go }: Props) {
  ...
  function x01TeamBaseId(value: any): string {
    return String(value?.baseTeamId || value?.sourceTeamId || value?.id || value || "").split("__slot_")[0];
  }

  function x01TeamSuffix(index: number): string {
    const letters = "ABCDEFGHIJKLMNOPQRSTUVWXYZ";
    return index < letters.length ? letters[index] : `#${index + 1}`;
  }

  const selectedStoredTeams = React.useMemo(() => {
    return (selectedStoredTeamIds || []).map((rawId: any, index: number) => {
      const baseId = x01TeamBaseId(rawId);
      const occurrence = (selectedStoredTeamIds || []).slice(0, index).filter((id: any) => x01TeamBaseId(id) === baseId).length;
      const team = storedDartsTeams.find((t: any) => String(t.id) === baseId) || findRememberedGeneratedTeam(baseId);
      if (!team) return null;
  ...
  const selectedBotTeams = React.useMemo(() => {
    ...
    return (selectedBotTeamIds || []).map((rawId: any, index: number) => {
      const baseId = x01TeamBaseId(rawId);
      const occurrence = (selectedBotTeamIds || []).slice(0, index).filter((id: any) => x01TeamBaseId(id) === baseId).length;
  ...
  function ensureSavedTeamMembersInitialized(tid: string, teamList: any[]) {
    setSavedTeamMemberSelections((prev) => {
      ...
      const baseTid = x01TeamBaseId(tid);
      const team = teamList.find((t: any) => String(t.id || t.baseTeamId) === baseTid) || findRememberedGeneratedTeam(baseTid);
      const allIds = (Array.isArray((team as any)?.playerIds) ? (team as any).playerIds : []).map((x: any) => String(x || "")).filter(Boolean);
  ...
  function buildExternalTeamConfigs(teamList: any[]) {
    return (teamList || [])
      .map((team: any, index: number) => {
        ...
        const baseTid = x01TeamBaseId(team);
        const allIds = (Array.isArray(team?.playerIds) ? team.playerIds : []).map((id: any) => String(id || "")).filter(Boolean);
        const occurrence = (teamList || []).slice(0, index).filter((t: any) => x01TeamBaseId(t) === baseTid).length;
        const defaultMembers = allIds.slice(occurrence * 2, occurrence * 2 + 2);
        const selectedMembers = Array.isArray(savedTeamMemberSelections[tid]) ? savedTeamMemberSelections[tid] : (defaultMembers.length ? defaultMembers : allIds);
              ...
              <section style={{ background: cardBg, borderRadius: 18, padding: 14, marginBottom: 14, border: "1px solid rgba(255,255,255,0.05)", boxShadow: "0 16px 40px rgba(0,0,0,0.55)" }}>
                ...
                <div style={{ marginBottom: 14 }}>
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8, marginBottom: 7 }}>
                    <div style={{ fontSize: 12, color: "#c8cbe4" }}>Formats Pro / avancés</div>
		...
		<div style={{ marginBottom: 10 }}>
            ...
            <div style={{ marginTop: 10 }}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8, marginBottom: 6 }}>
                <div style={{ fontSize: 12, color: "#c8cbe4" }}>{t("x01v3.proFormats", "Formats Pro / avancés")}</div>
...
export type TeamsSectionProps = {
  ...
  allowSaveGeneratedTeams?: boolean;
  primary: string;
  primarySoft: string;
};
...
export function TeamsSection({
  ...
  const [autoTeamsOpen, setAutoTeamsOpen] = React.useState(false);
  const [autoParticipantIds, setAutoParticipantIds] = React.useState<string[]>([]);
  const [autoGeneratedTeams, setAutoGeneratedTeams] = React.useState<any[]>([]);
  ...
  const allStoredTeams = React.useMemo(() => {
    // Liste affichée dans l'onglet "Équipes enregistrées" : uniquement les vraies
    // équipes sauvegardées. Les équipes de brassage auto restent temporaires et ne
    // doivent pas apparaître ici tant que l'utilisateur n'a pas cliqué sur
  ...
  const selectedStoredInstances = React.useMemo(() => {
    return (selectedStoredTeamIds || []).map((instanceId) => {
      const raw = String(instanceId || "");
      const baseId = raw.split("__slot_")[0];
      const team = (allStoredTeams || []).find((t: any) => String(t?.id || t?.baseTeamId || "") === baseId) || findRememberedGeneratedTeam(baseId);
      if (!team) return null;
  ...
  }, [selectedStoredTeamIds, allStoredTeams, savedTeamMemberSelections]);
  ...
  const autoSelectedIds = React.useMemo(() => {
    const valid = new Set(autoCandidatePlayers.map((player: any) => playerIdOf(player)).filter(Boolean));
    return (selectedIds || []).map(String).filter((id) => valid.has(id));
  }, [selectedIds, autoCandidateKey]);
  ...
  const autoSelectedPlayers = React.useMemo(() => {
    const ids = new Set((autoSelectedIds || []).map(String));
    return autoCandidatePlayers
      .filter((player: any) => ids.has(playerIdOf(player)))
      .map((player: any) => {
        const id = playerIdOf(player);
        const stats = autoStatsById[id];
        const withStats = stats ? { ...player, x01Stats: stats, stats: { ...(player?.stats || {}), x01: stats } } : player;
        const level = x01AutoBalanceLevelForProfile(withStats, autoStatsById);
        return {
          ...withStats,
          autoBalanceLevel: level,
          balanceLevel: level,
          level,
          stars: level,
          profileStarring: level,
          x01ProfileStarring: level,
        };
      });
  }, [autoCandidatePlayers, autoSelectedIds, autoStatsById]);

  const configuredTeamSize = Number.isFinite(Number(teamSizeOverride)) && Number(teamSizeOverride) > 0
    ? Math.floor(Number(teamSizeOverride))
    : 0;
  const configuredTeamCount = Number.isFinite(Number(teamCountOverride)) && Number(teamCountOverride) > 0
    ? Math.floor(Number(teamCountOverride))
    : 0;
  const autoTeamSize = configuredTeamSize || 2;
  const autoTeamCount = configuredTeamCount || Math.floor(autoSelectedPlayers.length / autoTeamSize);
  const autoRequiredPlayers = autoTeamSize * (configuredTeamCount || 2);
  const canAutoShuffleTeams = autoCandidatePlayers.length >= autoRequiredPlayers;
  const autoSelectedSetKey = React.useMemo(
    () => [...(autoSelectedIds || [])].map(String).sort().join("|"),
    [autoSelectedIds]
  );

  React.useEffect(() => {
    // En brassage auto, l'aperçu doit toujours correspondre STRICTEMENT aux
    // profils actuellement sélectionnés. Si on retire/ajoute un joueur, on vide
    // l'ancien brassage pour éviter d'afficher des équipes avec d'autres avatars.
    if (sourceMode !== "auto") return;
    const selected = new Set((autoSelectedIds || []).map(String));

    let staleGeneratedTeams = false;
    if ((autoGeneratedTeams || []).length > 0) {
      const generatedIds = Array.from(new Set(
        (autoGeneratedTeams || [])
          .flatMap((team: any) => Array.isArray(team?.playerIds) ? team.playerIds : [])
          .map((id: any) => String(id || ""))
          .filter(Boolean)
      ));
      const samePlayers = generatedIds.length === selected.size && generatedIds.every((id) => selected.has(id));
      staleGeneratedTeams = !samePlayers;
      if (staleGeneratedTeams) {
        updateAutoGeneratedTeams([]);
        setAutoShuffleError("");
        setAutoSavedHint("");
      }
    }

    if (staleGeneratedTeams && removeStoredTeamSelection && (selectedStoredTeamIds || []).length > 0) {
      (selectedStoredTeamIds || []).forEach((raw: any) => {
        const baseId = String(raw || "").split("__slot_")[0];
        if (baseId.startsWith("tmp_shuffle_")) removeStoredTeamSelection(String(raw));
      });
    }
  }, [sourceMode, autoSelectedSetKey, autoGeneratedTeams, selectedStoredTeamIds]);

  function openDartSetPickerAfterTeamPlayerSelection(idRaw: any, meta?: any) {
    const pid = String(idRaw || "").trim();
    if (!pid || meta?.selected === false) return;
    const humanPool = Array.isArray(allProfiles) && allProfiles.length ? allProfiles : (profiles || []);
    const isHuman = humanPool.some((p: any) => String(p?.id || p?.profileId || p?.localProfileId || "") === pid && p?.isBot !== true);
    if (!isHuman) return;
    setAutoDartSetPicker?.({ profileId: pid, seq: Date.now() });
  }

  function updateAutoGeneratedTeams(next: any[]) {
    setAutoGeneratedTeams(next);
    rememberGeneratedTeams(next);
  }

  function onAutoTogglePlayer(idRaw: any) {
    const id = String(idRaw || "");
    if (!id || !togglePlayer) return;
    setAutoShuffleError("");
    setAutoSavedHint("");
    updateAutoGeneratedTeams([]);
    togglePlayer(id);
  ...
  function selectAllAutoPlayers() {
    if (!togglePlayer) return;
    const current = new Set((selectedIds || []).map(String));
    setAutoShuffleError("");
    setAutoSavedHint("");
    updateAutoGeneratedTeams([]);
    for (const player of autoCandidatePlayers || []) {
      const id = playerIdOf(player);
      if (id && !current.has(id)) togglePlayer(id);
    }
  }

  function clearAutoPlayers() {
    if (!togglePlayer) return;
    const current = new Set((selectedIds || []).map(String));
    setAutoShuffleError("");
    setAutoSavedHint("");
  ...
  function runAutoTeamShuffle(mode: any) {
    setAutoSavedHint("");
    setAutoShuffleMode(mode === "random" ? "random" : "balanced");
    if (configuredTeamCount > 0 && autoSelectedPlayers.length !== autoTeamSize * configuredTeamCount) {
      const required = autoTeamSize * configuredTeamCount;
      setAutoShuffleError(`Sélectionne exactement ${required} joueurs pour créer ${configuredTeamCount} équipes de ${autoTeamSize}.`);
      return;
    }
    if (autoSelectedPlayers.length < autoTeamSize * 2) {
      setAutoShuffleError(`Sélectionne au moins ${autoTeamSize * 2} joueurs pour créer des équipes aléatoires.`);
      return;
    }
    if (autoSelectedPlayers.length % autoTeamSize !== 0) {
      setAutoShuffleError("Nombre de joueurs incompatible : sélectionne un nombre pair de joueurs.");
      return;
    }
    const next = generateShuffledTeams({
      players: autoSelectedPlayers,
      teamSize: autoTeamSize,
      teamCount: autoTeamCount,
      mode: mode === "random" ? "random" : "balanced",
      sport: "darts",
      seed: `x01-${mode}-${Date.now()}-${autoSelectedPlayers.map(playerIdOf).join("-")}`,
      accent: primary,
    });
    updateAutoGeneratedTeams(next);
    setAutoShuffleError("");
  }

  function renameAutoGeneratedTeam(id: string, name: string) {
    updateAutoGeneratedTeams((autoGeneratedTeams || []).map((team: any) => team.id === id ? { ...team, name, originalName: name, updatedAt: Date.now() } : team));
  }

  function rerollAutoGeneratedLogo(id: string) {
  ...
  function saveAutoGeneratedTeams() {
    if (!autoGeneratedTeams.length) return;
    const saved = saveGeneratedTeamsToTeamStore(autoGeneratedTeams, "darts");
    setAutoSavedHint(`${saved.length} équipe${saved.length > 1 ? "s" : ""} enregistrée${saved.length > 1 ? "s" : ""}.`);
  }
  ...
  const openStoredTeamPicker = (team: any) => {
    ...
    if (available.length <= 0) return;
    setTeamPicker(team);
    setTeamPickerPlayerIds(available.slice(0, configuredTeamSize || 1));
  };
  ...
  const validateStoredTeamPicker = () => {
    ...
    if (!teamId || ids.length <= 0 || (configuredTeamSize > 0 && ids.length !== configuredTeamSize)) return;
  ...
  const maxTeams = configuredTeamCount || (totalPlayers <= 4 ? 2 : totalPlayers <= 6 ? 3 : 4);
  const maxPerTeamBase = totalPlayers >= 8 ? 4 : totalPlayers >= 6 ? 3 : 2;
  const usedTeamsCount = orderedTeams.filter((tid) => counts[tid] > 0).length;
  const maxPerTeam = configuredTeamSize || (usedTeamsCount >= 3 ? 2 : maxPerTeamBase);
  const manualRequiredPlayers = configuredTeamCount > 0 && configuredTeamSize > 0
    ? configuredTeamCount * configuredTeamSize
    : 4;
  ...
  const filteredAutoLogos = React.useMemo(() => {
    const dartsLogos = TEAM_LOGO_LIBRARY.filter((logo) => teamLogoMatchesCategory(logo, "darts"));
    const sportPool = dartsLogos.length ? dartsLogos : TEAM_LOGO_LIBRARY;
    if (autoLogoCategory === "popular") {
      const popular = sportPool.filter((logo) => teamLogoMatchesCategory(logo, "popular"));
      return popular.length ? popular : sportPool;
    }
    return sportPool;
  }, [autoLogoCategory]);

  const x01AutoLogoCategories = React.useMemo(
    () => TEAM_LOGO_CATEGORIES.filter((cat) => cat.id === "popular" || cat.id === "darts"),
    []
  );
      ...
      {sourceMode === "auto" ? (
            ...
            <section style={{ borderRadius: 22, padding: 12, border: `1px solid ${primary}77`, background: `linear-gradient(180deg, ${primarySoft}, rgba(6,9,20,.72))`, boxShadow: `0 0 28px ${primary}18` }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 10, marginBottom: 10 }}>
                <div>
                  ...
                  <div style={{ color: "#dfe4ff", fontSize: 12, marginTop: 4, lineHeight: 1.35 }}>Sélectionne les joueurs avec le même sélecteur que le mode manuel, puis génère les équipes.</div>
                </div>
                <span style={{ color: "#dfe6ff", fontWeight: 950, fontSize: 12, whiteSpace: "nowrap" }}>{autoSelectedIds.length} sélectionné{autoSelectedIds.length > 1 ? "s" : ""}</span>
              </div>

              <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center", marginBottom: 10 }}>
                <button type="button" onClick={selectAllAutoPlayers} style={x01AutoSmallPill(primary, true)}>Tous les joueurs</button>
                <button type="button" onClick={clearAutoPlayers} style={x01AutoSmallPill(primary, false)}>Vider</button>
              </div>
              ...
              {togglePlayer ? (
                <PlayerPagedSelector
                  ...
                  selectedIds={selectedIds}
                  onToggle={onAutoTogglePlayer}
                  onAfterToggle={openDartSetPickerAfterTeamPlayerSelection}
                  accent={primary}
                  pageSize={9}
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(165px, 1fr))", gap: 8, marginTop: 12, marginBottom: 12 }}>
                <button type="button" onClick={() => runAutoTeamShuffle("random")} style={x01AutoActionButton(primary, autoShuffleMode === "random")}>
                  <span style={{ display: "inline-flex", alignItems: "center", justifyContent: "center", gap: 9 }}><X01AutoShuffleIcon type="random" /> Aléatoire</span>
                </button>
                <button type="button" onClick={() => runAutoTeamShuffle("balanced")} style={x01AutoActionButton(primary, autoShuffleMode === "balanced")}>
                  <span style={{ display: "inline-flex", alignItems: "center", justifyContent: "center", gap: 9 }}><X01AutoShuffleIcon type="balanced" /> Équilibré</span>
                </button>
              </div>
              ...
              {autoGeneratedTeams.length ? (
                <div style={{ display: "grid", gap: 10 }}>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(min(260px, 100%), 1fr))", gap: 12, minWidth: 0, maxWidth: "100%" }}>
                    {autoGeneratedTeams.map((team: any) => {
                      const members = (team.playerIds || []).map((id: string) => teamByPlayer.get(String(id))).filter(Boolean);
                      return (
                        <article key={team.id} style={{ borderRadius: 24, padding: 14, border: `1px solid ${primary}66`, background: "linear-gradient(180deg, rgba(5,10,24,.94), rgba(3,6,16,.86))", display: "grid", gap: 12, boxShadow: `0 0 18px ${primary}18` }}>
                          <div style={{ display: "flex", alignItems: "center", gap: 12, minWidth: 0 }}>
                            <button
                              type="button"
                              title="Changer le logo"
                              aria-label="Changer le logo de l'équipe"
                              onClick={() => setAutoLogoPickerTeamId(team.id)}
                              style={{ border: "none", background: "transparent", padding: 0, margin: 0, cursor: "pointer", flex: "0 0 auto", borderRadius: 999 }}
                            >
                              <ProfileAvatar name={team.name} dataUrl={team.logoDataUrl || team.logoUrl || undefined} size={74} />
                            </button>
                            <div style={{ flex: 1, minWidth: 0 }}>
                              <input value={team.name} onChange={(e) => renameAutoGeneratedTeam(team.id, e.target.value)} style={{ width: "100%", borderRadius: 14, border: `1px solid ${primary}66`, background: "rgba(0,0,0,.34)", color: "#fff", padding: "10px 11px", fontWeight: 1000, fontSize: 16, outline: "none" }} />
                              <div style={{ color: "#aab0ca", fontSize: 12, marginTop: 6, fontWeight: 900 }}>Force : {Number(team.teamPower || 0).toFixed(1)}</div>
                            </div>
                          </div>
                          <div style={{ display: "flex", alignItems: "center", gap: 7, flexWrap: "wrap", minHeight: 44 }}>
                            {members.map((p: any) => <ProfileAvatar key={p.id} profile={p} size={42} />)}
                          </div>
                        </article>
                      );
                    })}
              ...
              <div style={{ width: "min(680px, 96vw)", maxHeight: "90vh", overflow: "hidden", borderRadius: 24, background: "linear-gradient(180deg, rgba(7,18,35,.98), rgba(3,6,16,.98))", border: `1px solid ${primary}88`, boxShadow: `0 22px 70px rgba(0,0,0,.78), 0 0 28px ${primary}44`, display: "flex", flexDirection: "column" }} onClick={(e) => e.stopPropagation()}>
                ...
                <div className="dc-scroll-thin" style={{ padding: 14, overflowY: "auto" }}>
                          ...
                          <button
                            ...
                            onClick={() => {
                              setTeamPickerPlayerIds((prev) => {
                                ...
                                if (!wasChecked && configuredTeamSize > 0 && prev.length >= configuredTeamSize) return prev;
                  ...
                  <div style={{ display: "flex", justifyContent: "flex-end", gap: 10, marginTop: 16 }}>
                    ...
                    <button type="button" disabled={teamPickerPlayerIds.length <= 0 || (configuredTeamSize > 0 && teamPickerPlayerIds.length !== configuredTeamSize)} onClick={validateStoredTeamPicker} style={{ borderRadius: 999, padding: "10px 18px", border: `1px solid ${primary}`, background: teamPickerPlayerIds.length > 0 && (!configuredTeamSize || teamPickerPlayerIds.length === configuredTeamSize) ? `${primary}22` : "rgba(255,255,255,.04)", color: teamPickerPlayerIds.length > 0 && (!configuredTeamSize || teamPickerPlayerIds.length === configuredTeamSize) ? primary : "#777", fontWeight: 950, cursor: teamPickerPlayerIds.length > 0 && (!configuredTeamSize || teamPickerPlayerIds.length === configuredTeamSize) ? "pointer" : "not-allowed" }}>Valider</button>
                  </div>
                </div>
              </div>
            </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          <div style={{ color: "#c8cbe4", fontSize: 12, lineHeight: 1.35 }}>
            Choisir au minimum {manualRequiredPlayers} joueurs, puis assigne-les manuellement à Gold, Pink, Blue ou Green.
          </div>
          {togglePlayer ? (
            <PlayerPagedSelector
                  usageMode="x01"
          ...
          {selectedIds.length < manualRequiredPlayers && (
            <div style={{ color: "#8f94b2", fontSize: 12, marginBottom: 4 }}>
              Sélectionne au moins {manualRequiredPlayers} joueurs pour composer des équipes manuelles.
            </div>
          )}
          {selectedIds.map((pid) => {
              ...
              <div key={pid} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8 }}>
                ...
                <div style={{ display: "flex", gap: 4, flexWrap: "wrap", justifyContent: "flex-end" }}>
                  {orderedTeams.map((tid, idx) => {
                    ...
                    const disabled = !allowedTeamSlot || full || selectedIds.length < manualRequiredPlayers;
...
export function PillButton({ label, active, onClick, primary, primarySoft, compact, disabled }: PillProps) {
...
function X01AutoShuffleIcon({ type, size = 22 }: { type: "random" | "balanced"; size?: number }) {
  const p = {
    fill: "none",
    stroke: "currentColor",
    strokeWidth: 2,
  ...
  if (type === "balanced") {
    return (
      <svg width={size} height={size} viewBox="0 0 24 24" aria-hidden="true">
        <path {...p} d="M12 4v15" />
        <path {...p} d="M6 7h12" />
        <path {...p} d="M7 7l-3 6h6L7 7Z" />
        <path {...p} d="M17 7l-3 6h6l-3-6Z" />
        <path {...p} d="M8 21h8" />
      </svg>
    );
  }
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" aria-hidden="true">
      <rect {...p} x="4" y="4" width="16" height="16" rx="4" />
      <circle fill="currentColor" cx="8" cy="8" r="1.3" />
      <circle fill="currentColor" cx="12" cy="12" r="1.3" />
      <circle fill="currentColor" cx="16" cy="16" r="1.3" />
      <circle fill="currentColor" cx="16" cy="8" r="1.3" />
      <circle fill="currentColor" cx="8" cy="16" r="1.3" />
    </svg>
  );
}

function x01AutoSmallPill(primary: string, active: boolean): React.CSSProperties {
  return {
    borderRadius: 999,
    border: `1px solid ${primary}77`,
    background: active ? `${primary}1f` : "rgba(255,255,255,.04)",
    color: primary,
    padding: "7px 10px",
    fontSize: 10,
    fontWeight: 950,
...
function x01AutoActionButton(primary: string, active: boolean, disabled = false): React.CSSProperties {
  return {
    borderRadius: 999,
    border: disabled ? "1px solid rgba(255,255,255,.08)" : `1px solid ${primary}88`,
    background: disabled ? "rgba(255,255,255,.035)" : active ? `${primary}22` : "rgba(255,255,255,.055)",
    color: disabled ? "#777" : active ? "#fff" : primary,
    padding: "10px 12px",
    fontSize: 11,
    fontWeight: 1000,
    cursor: disabled ? "not-allowed" : "pointer",
    boxShadow: active && !disabled ? `0 0 18px ${primary}33` : "none",
  };
}
