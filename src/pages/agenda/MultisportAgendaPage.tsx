  ...
  return (
    <div className="container" style={{ maxWidth: 700, paddingBottom: 100 }}>
      <style>{`
        ...
        .msa-event{width:100%;text-align:left;border-radius:17px;border:1px solid rgba(255,255,255,.08);background:linear-gradient(145deg,rgba(255,255,255,.045),rgba(5,8,14,.96));padding:10px;display:grid;grid-template-columns:42px minmax(0,1fr) auto;gap:10px;align-items:center;color:#fff;cursor:pointer;min-width:0;position:relative;overflow:hidden;isolation:isolate}.msa-event>*{position:relative;z-index:2}.msa-event-watermark{position:absolute!important;z-index:0!important;right:54px;top:50%;transform:translateY(-50%);font-size:58px;line-height:1;opacity:.075;filter:saturate(1.25);pointer-events:none}
        ...
        .msa-week{display:grid;gap:8px}.msa-week-day{position:relative;overflow:hidden}.msa-week-day-logo{position:absolute;right:8px;top:3px;font-size:52px;line-height:1;opacity:.065;pointer-events:none}.msa-week-day>*{position:relative;z-index:1}.msa-month{display:grid;grid-template-columns:repeat(7,minmax(0,1fr));gap:5px}.msa-month-cell{min-height:66px;border-radius:12px;border:1px solid rgba(255,255,255,.065);background:rgba(255,255,255,.025);padding:5px;overflow:hidden}.msa-month-cell.off{opacity:.28}.msa-dot{width:6px;height:6px;border-radius:999px;display:inline-block;margin-right:3px}
...
function EventCard({ event, locale, onOpen, onDelete, conflict = false }: { event: MultisportAgendaEvent; locale: string; onOpen: () => void; onDelete?: () => void; conflict?: boolean }) {
  ...
  return <div className={`msa-event${conflict ? " conflict" : ""}`} role="button" tabIndex={0} onClick={onOpen} onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") onOpen(); }} style={{ borderColor: `${hot}38`, background: `linear-gradient(145deg,${hot}0b,rgba(5,8,14,.965))` }}>
    <div className="msa-event-watermark" style={{ color: hot }}>{meta.icon}</div>
