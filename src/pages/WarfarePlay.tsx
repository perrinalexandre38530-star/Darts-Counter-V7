    ...
    >
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10 }}>
          <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
            <div style={{ fontWeight: 900 }}>
              {turnIndex === 0 ? p1.name : p2.name}
              <span style={{ marginLeft: 8, fontSize: 12, color: theme.textSoft }}>
                {t("warfare.turn", "à toi")}
              </span>
            </div>
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
              <ArmyBadge army={p1Army} label={`${p1.name} • ${p1Army}`} />
              <ArmyBadge army={p2Army} label={`${p2.name} • ${p2Army}`} />
            </div>
          </div>

          <div style={{ textAlign: "right" }}>
            <div style={{ fontSize: 12, color: theme.textSoft }}>{t("warfare.zoneRule", "Zones")}</div>
            <div style={{ fontWeight: 900, color: theme.primary, textShadow: `0 0 10px ${theme.primary}44` }}>
      ...
      <div style={{ position: "fixed", left: 0, right: 0, bottom: 0, padding: 12, zIndex: 30 }}>
        <Keypad
          ...
          onTriple={() => setMult(3)}
          onCancel={() => {
            // comportement cohérent: annule la dernière volée validée
            undoLast();
          }}
          ...
          centerSlot={
            <div style={{ textAlign: "center" }}>
              <div style={{ fontSize: 12, color: theme.textSoft }}>{t("warfare.throw", "Volée")}</div>
