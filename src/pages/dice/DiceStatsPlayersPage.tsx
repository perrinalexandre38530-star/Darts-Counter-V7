// @ts-nocheck
// ============================================
// src/pages/dice/DiceStatsPlayersPage.tsx
...
// - Données: agrégées depuis lib/diceStats (fallback 0 si vide)
// ============================================
...
export default function DiceStatsPlayersPage({ go }: Props) {
    ...
    <div style={pageWrap}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
              ...
              <div style={{ minWidth: 140 }}>
                ...
                <div style={smallMuted}>
                  ...
                  {stats.matches} • {(t?.("stats.winrate") || "Winrate") + ": "}
                  {stats.winrate.toFixed?.(0) || 0}%
                </div>
              </div>
            </div>
          ) : (
            <div style={{ opacity: 0.65, textAlign: "center" }}>{t?.("profiles.none") || "Aucun profil."}</div>
          )}
        </div>
      ...
      <div style={grid}>
        <div style={{ ...card, padding: 14 }}>
          <div style={{ fontWeight: 900, fontSize: 15, marginBottom: 6 }}>{t?.("stats.stats") || "Statistiques"}</div>
          <div style={{ fontSize: 11, opacity: 0.75 }}>{t?.("stats.desc") || "Analyse des performances — Dice."}</div>

          <div style={{ display: "grid", gridTemplateColumns: "1fr", gap: 10, marginTop: 12 }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10 }}>
              <div style={{ fontSize: 11, opacity: 0.75 }}>{t?.("stats.favoriteMode") || "Mode de jeu préféré"}</div>
              <div
                style={{
                  padding: "4px 10px",
                  borderRadius: 999,
        ...
        <div style={statCard}>
          <div style={statLeft}>
            <div style={statLabel}>{t?.("stats.avgPts") || "Moyenne (pts / lancer)"}</div>
            <div style={statValue}>{stats.avgPts.toFixed?.(1) || stats.avgPts} pts</div>
            <div style={{ fontSize: 11, opacity: 0.75 }}>{t?.("stats.avgLabel") || "Visites moyennes"}</div>
          </div>
          <div style={badge}>⌀</div>
        </div>
        ...
        <div style={statCard}>
          <div style={statLeft}>
            <div style={statLabel}>{t?.("stats.best") || "Meilleure visite"}</div>
            <div style={statValue}>{stats.bestPts.toFixed?.(0) || stats.bestPts} pts</div>
            <div style={{ fontSize: 11, opacity: 0.75 }}>{t?.("stats.record") || "Record personnel"}</div>
          </div>
          <div style={badge}>★</div>
        </div>
        ...
        <div style={statCard}>
          <div style={statLeft}>
            <div style={statLabel}>{t?.("stats.winrate") || "Taux de victoire"}</div>
            <div style={statValue}>{stats.winrate.toFixed?.(0) || stats.winrate} %</div>
            <div style={{ fontSize: 11, opacity: 0.75 }}>{t?.("stats.total") || "Tous matchs"}</div>
          </div>
          <div style={badge}>%</div>
        </div>
        ...
        <div style={{ ...card, padding: 14 }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10 }}>
            <div style={{ fontWeight: 900 }}>{t?.("stats.evolution") || "Évolution"}</div>
            <div style={{ fontSize: 11, opacity: 0.75 }}>{t?.("stats.perGame") || "Moyenne par partie"}</div>
          </div>
          <div
            style={{
              marginTop: 10,
          ...
          >
            {t?.("stats.chartSoon") || "Graphique"}
          </div>
          <div style={{ marginTop: 10, fontSize: 11, opacity: 0.75 }}>
            {(t?.("stats.avgDuration") || "Durée moyenne") + ": "}
            {formatDuration(stats.avgDurationMs)} • {(t?.("stats.avgTurns") || "Tours moyens") + ": "}
