...
import tickerTicTacToe from "../assets-webp/tickers/ticker_tic_tac_toe.webp";
...
export type TicTacToeConfigPayload = {
  players: number;
  botsEnabled: boolean;
  botLevel: BotLevel;
  rounds: number;
...
export default function TicTacToeConfig(props: any) {
  const { t } = useLang();
  useTheme();
  ...
  const payload: TicTacToeConfigPayload = { players, botsEnabled, botLevel, rounds, objective };
  ...
  function start() {
    // App.tsx wiring: tab "tic_tac_toe_play"
    if (props?.setTab) return props.setTab("tic_tac_toe_play", { config: payload });
