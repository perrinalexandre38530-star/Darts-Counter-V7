...
assert.ok(review.includes('--approve')&&review.includes('--reject')&&review.includes('APPROVED'),"human review command incomplete");
assert.ok(review.includes('stepGuide.status')&&stepJobs.includes('dedicated step illustration'),"arbitrary video frames can still be promoted as exercise steps");
