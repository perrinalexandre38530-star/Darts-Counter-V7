...
if (includeApproved) console.warn("--include-approved est ignoré en V113 : les composants APPROVED validés ne sont jamais régénérés automatiquement.");
...
for(const exercise of catalog.exercises){
  ...
  const approvedPartialMissingVideo = registryState.status===AWENA_STATUS.APPROVED
    ...
    && (registryState.missingComponents||[]).includes("video");
  const approvedComplete = registryState.status===AWENA_STATUS.APPROVED
    && registryState.completeness===AWENA_COMPLETENESS.COMPLETE;
  if(approvedComplete) continue;
