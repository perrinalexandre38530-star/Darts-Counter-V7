...
const authLogin = read("src/pages/AuthV7Login.tsx");
const cloudAutoRestore = read("src/lib/cloudAutoRestore.ts");
const directR2 = read("src/lib/directR2BackupApi.ts");
const r2Cors = read("functions/api/storage/backups/_middleware.ts");
...
expect(
  "Le login public restaure directement depuis Cloudflare R2",
  /maybeAutoRestoreCloudForSignedInUser\(uid,\s*\{\s*force:\s*true\s*\}\)/.test(authLogin),
);
expect(
  "Le login public ne tente plus pullStoreSnapshot/NAS",
  !/pullStoreSnapshot\(/.test(authLogin),
);
expect(
  "Le client R2 accepte une session Supabase",
  /supabase\.auth\.getSession\(\)/.test(directR2) && /kind:\s*["']supabase["']/.test(directR2),
);
expect(
  "Le restaurateur R2 sait retomber sur le miroir NAS stocké dans R2",
  /downloadDirectR2NasUserMirror\(\)/.test(cloudAutoRestore) && /restoreFromR2NasMirrorFallback/.test(cloudAutoRestore),
);
expect(
  "Le CORS R2 autorise localhost avec port Vite variable",
  /host === ["']localhost["']/.test(r2Cors) && /url\.protocol === ["']http:["']/.test(r2Cors),
);
expect(
  "Le middleware R2 conserve les fallbacks média/avatar 404 souples",
  /backups\/media\//.test(r2Cors) && /backups\/avatar\//.test(r2Cors),
);
