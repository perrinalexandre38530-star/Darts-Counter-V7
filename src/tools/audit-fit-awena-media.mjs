...
import { AWENA_COMPLETENESS, AWENA_STATUS, listGeneratedArtifactKeys, resolveAwenaRegistryState } from "./fit-awena-registry.mjs";
...
for(const ex of catalog.exercises){
  ...
  rows.push({
    ...
    awenaCompleteness:state.completeness||AWENA_COMPLETENESS.NONE,missingComponents:(state.missingComponents||[]).join(","),
...
const approvedRows=rows.filter(r=>r.awenaStatus===AWENA_STATUS.APPROVED);
...
const summary={
  ...
  missingVideo:approvedRows.filter(r=>!r.awenaVideo).length,
  missingPoster:approvedRows.filter(r=>!r.awenaPoster).length,
  missingSteps:approvedRows.filter(r=>r.awenaSteps<4).length,
  ...artifactCounts,
  ...
  policy:{onlyApprovedRenderable:true,manualAuthoritative:true,reviewNeverPublished:true,rejectedNeverPublished:true,approvedCompleteNeverRegenerated:true,approvedPartialMissingComponentsOnly:true},
