...
async function syncJobOutputs(job,h){
  ...
  try{
    ...
    const requestedComponents=Array.isArray(job.requestedComponents)&&job.requestedComponents.length?job.requestedComponents:["video","poster"];
    if(requestedComponents.includes("poster")) makeAlphaStill(rgbFiles[posterIndex],maskFiles[posterIndex],path.join(outDir,"awena-poster.webp"));
    ...
    const metadata={exerciseId:job.exerciseId,name:job.name,assetKey:job.assetKey,status:AWENA_STATUS.REVIEW,generatedAt:new Date().toISOString(),generationMode:job.generationMode||"MOTION_PACK",requestedComponents,existingApprovedCoverage:job.existingApprovedCoverage||null,preserveApprovedComponents:true,transparentVideo:true,alphaVerified:true,alphaSource:"SAM2 final-frame matte",workflow:path.basename(workflowFile),frames:rgbFiles.length,fps,reviewFrameIndices,posterFrameIndex:requestedComponents.includes("poster")?posterIndex:null,motionDriver:job.__driverMeta||null,sourceReferenceImages:job.existingReferenceImages||[],sourceReferenceVideos:job.existingReferenceVideos||[],technicalQuality:quality,humanReviewRequired:true,stepGuide:{status:"MISSING_DEDICATED_GUIDE",requiredSteps:4,policy:"Review keyframes are inspection aids only. They are never published as pedagogical AWENA steps."}};
...
for(const [index,jobOriginal] of jobs.entries()){
  ...
  try{
    ...
    if(!h){
      ...
      console.log(`[${index+1}/${jobs.length}] ${job.name} · driver=${driver.mode} · mode=${job.generationMode||"MOTION_PACK"} · destination=REVIEW`); const prompt=replaceTokens(structuredClone(workflowBase),tokens); promptId=await submit(prompt);
