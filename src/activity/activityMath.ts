...
export function buildKilometerSplits(points: GeoPoint[], startedAt: number): ActivitySplit[] {
  ...
  for (let i = 1; i < points.length; i += 1) {
    ...
    while (cumulative >= nextBoundary) {
      ...
      const boundaryTs = a.timestamp + (b.timestamp - a.timestamp) * ratio;
      const elapsedMs = Math.max(0, boundaryTs - startedAt);
...
export function formatPace(secPerKm: number | null | undefined): string {
  if (!Number.isFinite(secPerKm) || Number(secPerKm) <= 0) return "--:--";
...
export function averagePaceSecPerKm(distanceM: number, elapsedMs: number): number | null {
  if (distanceM < 1 || elapsedMs <= 0) return null;
  return elapsedMs / 1_000 / (distanceM / 1_000);
}

export function averageSpeedMps(distanceM: number, elapsedMs: number): number {
  if (distanceM <= 0 || elapsedMs <= 0) return 0;
  return distanceM / (elapsedMs / 1_000);
}
...
export function shouldAcceptRunningPoint(previous: GeoPoint | undefined, next: GeoPoint): boolean {
  ...
  if (Number.isFinite(next.accuracy) && Number(next.accuracy) > 100) return false;
  if (!previous) return true;

  const dt = Math.max(0.001, (next.timestamp - previous.timestamp) / 1_000);
  const distance = haversineMeters(previous, next);
  if (distance < 2 && dt < 5) return false;

  // Garde-fou anti-saut GPS : 72 km/h est déjà largement au-delà d'une course à pied.
  if (distance / dt > 20) return false;
  return true;
}
