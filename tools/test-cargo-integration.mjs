...
for (const token of ["cargo_classic","free_load","full_pallet","exact_load","fragile_cargo","cargo_rush","convoy","long_haul","parcel_delivery","playCargoVisit","buildCargoMatchStats"]) assert.ok(engine.includes(token), `moteur incomplet: ${token}`);
...
for (const token of ["CargoEnd","History as any).upsert(buildHistoryRecord(\"in_progress\")","onFinish?.(buildHistoryRecord(\"finished\"), { navigate: false })","Keypad","MANIFESTE DE CHARGEMENT","JOURNAL DES CHARGEMENTS"]) assert.ok(play.includes(token), `écran Play incomplet: ${token}`);
...
assert.ok(registry.includes(`"cargo",
  "killer"`), "CARGO absent de READY_IDS");
