...
const checks = [
  ...
  [social.includes("const SOCIAL_MODEL = '@cf/meta/llama-3.1-8b-instruct-fast'") && !social.includes("const AI_MODEL = '@cf/zai-org/glm-4.7-flash'"), 'Social Growth uses the proven fast Workers AI model'],
  [social.includes('SOCIAL_DRAFT_RESPONSE_FORMAT') && social.includes('SOCIAL_QA_RESPONSE_FORMAT') && social.includes('response_format: SOCIAL_DRAFT_RESPONSE_FORMAT') && social.includes('response_format: SOCIAL_QA_RESPONSE_FORMAT'), 'Social Growth generation and QA use structured JSON mode'],
  [social.includes('max_tokens: 1100') && social.includes('max_tokens: 420') && social.includes('temperature: 0.2') && social.includes('temperature: 0'), 'Social Growth bounds output size and latency'],
